...
SUPER_VERSION="5.1.0-rc1"
readonly SUPER_VERSION
SUPER_DATE="2025/10/02"
readonly SUPER_DATE
...
# Output the usage documentation to standard out.
show_usage() {
	echo "
  S.U.P.E.R.M.A.N.
  ...
  [--usage]  [--help]  [--version]  [--config-status]
  ...
  Non-system Installation Options:
  ...
  [--install-safari-update-without-restarting-off]
  [--install-prioritize-non-restart-updates]
  [--install-prioritize-non-restart-updates-off]
  
  Jamf Policy Installation Options:
  ...
  [--workflow-reset-super-after-completion-off]
  ...
  [--schedule-deferred-start-days=number]
  ...
  [--display-accessory-safari-update-file=/local/path or URL]
  ...
  [--config-edit-main]  [--config-edit=ConfigurationName]
  [--config-clone=SourceConfigurationName]
  [--config-delete=ConfigurationName]  [--config-delete-all]
  ...
  [--usage]  [--help]  [--version]  [--config-status]
  [--open-logs]  [--reset-super]
  [--verbose-mode]  [--verbose-mode-off]
  
  ** Managed preferences override local options via domain: com.macjutsu.super
  
  <key>InstallMacOSMinorVersionTarget</key> <string>macOSVersionNumber</string>
  <key>InstallMacOSMajorUpgrades</key> <true/> | <false/>
  <key>InstallMacOSMajorVersionTarget</key> <string>macOSVersionNumber</string>
  <key>InstallRapidSecurityResponses</key> <true/> | <false/>
  <key>InstallNonSystemUpdatesWithoutRestarting</key> <true/> | <false/>
  <key>InstallSafariUpdateWithoutRestarting</key> <true/> | <false/>
  <key>InstallPrioritizeNonRestartUpdates</key> <true/> | <false/>
  ...
  <key>WorkflowRequireActiveUser</key> <true/> | <false/>
  ...
  <key>ScheduleDeferredStartFile</key> <string>path</string>
  <key>ScheduleDeferredStartDays</key> <string>number</string>
  ...
  <key>DisplayAccessorySafariUpdateFile</key> <string>path or URL</string>
...
show_help() {
	log_echo "**** S.U.P.E.R.M.A.N. ${SUPER_VERSION} - SHOW HELP ****"
	check_current_user
	if [[ "${current_user_account_name}" != "FALSE" ]]; then
		...
		log_echo "**** S.U.P.E.R.M.A.N. ${SUPER_VERSION} - EXIT SHOW HELP ****"
...
# Output only the ${SUPER_VERSION} to standard out.
show_version() {
	echo "${SUPER_VERSION}"
	exit 0
}

# Output various configuration status to standard out.
show_config_status() {
	log_echo "**** S.U.P.E.R.M.A.N. ${SUPER_VERSION} - CONFIGURATION STATUS ****"
	check_system
	check_macos_msu_download
	[[ "${macos_msu_downloaded_version}" == "FALSE" ]] && echo "Config Status: Mac softwareupdate system cache is empty (no downloaded macOS update/upgrade)."
	[[ "${macos_msu_downloaded_version}" != "FALSE" ]] && echo "Config Status: Mac softwareupdate system cache is macOS ${macos_msu_downloaded_version}-${macos_msu_downloaded_build}."
	local super_previous_pid
	super_previous_pid=$(pgrep -F "${SUPER_PID_FILE}" 2>/dev/null)
	local super_launch_daemon_active
	[[ $(launchctl print "system/${SUPER_LAUNCH_DAEMON_LABEL}" 2>&1 | grep -c "system/${SUPER_LAUNCH_DAEMON_LABEL}") -gt 0 ]] && super_launch_daemon_active="TRUE"
	if [[ -n "${super_previous_pid}" ]]; then
		[[ "${super_launch_daemon_active}" == "TRUE" ]] && echo "Config Status: The super workflow is RUNNING and LaunchDaemon system/${SUPER_LAUNCH_DAEMON_LABEL} is ENABLED."
		[[ "${super_launch_daemon_active}" != "TRUE" ]] && echo "Config Status: The super workflow is RUNNING and LaunchDaemon system/${SUPER_LAUNCH_DAEMON_LABEL} is NOT ENABLED."
	elif [[ "${super_launch_daemon_active}" == "TRUE" ]]; then
		local next_auto_launch
		next_auto_launch=$(/usr/libexec/PlistBuddy -c "Print :NextAutoLaunch" "${SUPER_MAIN_PLIST}.plist" 2> /dev/null)
		[[ "${next_auto_launch}" != "FALSE" ]] && echo "Config Status: LaunchDaemon system/${SUPER_LAUNCH_DAEMON_LABEL} is ENABLED and the super workflow is scheduled to automatically relaunch at ${next_auto_launch}."
		[[ "${next_auto_launch}" == "FALSE" ]] && echo "Config Status: LaunchDaemon system/${SUPER_LAUNCH_DAEMON_LABEL} is ENABLED but the super workflow is NOT scheduled to automatically relaunch."
	else
		echo "Config Status: LaunchDaemon system/${SUPER_LAUNCH_DAEMON_LABEL} is NOT ENABLED."
	fi
	
	if [[ -d "${SUPER_FOLDER}" ]]; then
		echo -e "\n**** Config Status: Contents of the super working folder located at ${SUPER_FOLDER}:"
		ls -lah "${SUPER_FOLDER}" | tail -n +4
	else
		echo -e "\n**** Config Error: No super working folder in the expected location at ${SUPER_FOLDER}."
	fi
	
	if [[ -d "${SUPER_CONFIGS_FOLDER}" ]]; then
		echo -e "\n**** Config Status: Contents of the super configs folder located at ${SUPER_CONFIGS_FOLDER}:"
		ls -lah "${SUPER_CONFIGS_FOLDER}" | tail -n +4
	else	
		echo -e "\nConfig Error: No super working log folder in the expected location at ${SUPER_LOG_FOLDER}."
	fi
	
	if [[ -d "${SUPER_LOG_FOLDER}" ]]; then
		echo -e "\n**** Config Status: Contents of the super logs folder located at ${SUPER_LOG_FOLDER}:"
		ls -lah "${SUPER_LOG_FOLDER}" | tail -n +4
	else
		echo -e "\nConfig Error: No super working log folder in the expected location at ${SUPER_LOG_FOLDER}."
	fi
	
	alternate_temporary_plist=$(get_pref "AlternateTemporaryPlist")
	config_temp_override=$(get_pref_managed "ConfigTempOverride")
	[[ -z "${config_temp_override}" ]] && config_temp_override="FALSE"
	if [[ -f "${SUPER_MANAGED_PLIST}.plist" ]]; then
		[[ "${config_temp_override}" == "FALSE" ]] || { [[ "${config_temp_override}" == "TRUE" ]] && [[ -z "${alternate_temporary_plist}" ]]; } && echo -e "\n**** Config Status: Contents of the CONFIGURATION PROFILE MANAGED super settings file located at ${SUPER_MANAGED_PLIST}.plist:"
		[[ "${config_temp_override}" == "FALSE" ]] || { [[ "${config_temp_override}" == "TRUE" ]] && [[ -z "${alternate_temporary_plist}" ]]; } && defaults read "${SUPER_MANAGED_PLIST}" | tail -n +2 | sed -e 's/    //g' -e 's/;//g' -e '$d'
	else
		echo -e "\n**** Config Status: No CONFIGURATION PROFILE MANAGED super settings file in the expected location at ${SUPER_MANAGED_PLIST}.plist."
	fi
	
	if [[ -n "${alternate_temporary_plist}" ]]; then
		if [[ -f "${alternate_temporary_plist}.plist" ]]; then
			{ [[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && [[ "${config_temp_override}" == "TRUE" ]]; } && echo -e "\n**** Config Status: The temporary workflow configuration will override conflicting settings in the managed workflow configuration."
			{ [[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && [[ "${config_temp_override}" == "FALSE" ]]; } && echo -e "\n**** Config Status: The managed workflow configuration will override conflicting settings in the temporary workflow configuration. You can reverse this behavior via the <key>ConfigTempOverride</key> <true/> managed preference setting."
			echo -e "\n**** Config Status: Contents of the ALTERNATE TEMPORARY super settings file located at ${alternate_temporary_plist}.plist:"
			defaults read "${alternate_temporary_plist}" | tail -n +2 | sed -e 's/    //g' -e 's/;//g' -e '$d'
			{ [[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && [[ "${config_temp_override}" == "TRUE" ]]; } && echo -e "\n**** Config Status: Contents of the CONFIGURATION PROFILE MANAGED super settings file located at ${SUPER_MANAGED_PLIST}.plist:"
			{ [[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && [[ "${config_temp_override}" == "TRUE" ]]; } && defaults read "${SUPER_MANAGED_PLIST}" | tail -n +2 | sed -e 's/    //g' -e 's/;//g' -e '$d'
		else
			echo -e "\n**** Config Error: No ALTERNATE TEMPORARY super settings file in the expected location at ${alternate_temporary_plist}.plist."
		fi
	else
		echo -e "\n**** Config Status: No ALTERNATE TEMPORARY super settings file."
	fi
	
	alternate_default_plist=$(get_pref "AlternateDefaultPlist")
	if [[ -n "${alternate_default_plist}" ]]; then
		if [[ -f "${alternate_default_plist}.plist" ]]; then
			echo -e "\n**** Config Status: Contents of the ALTERNATE DEFAULT super settings file located at ${alternate_default_plist}.plist:"
			defaults read "${alternate_default_plist}" | tail -n +2 | sed -e 's/    //g' -e 's/;//g' -e '$d'
		else
			echo -e "\n**** Config Error: No ALTERNATE DEFAULT super settings file in the expected location at ${alternate_default_plist}.plist."
		fi
	else
		echo -e "\n**** Config Status: No ALTERNATE DEFAULT super settings file."
	fi
	
	if [[ -f "${SUPER_MAIN_PLIST}.plist" ]]; then
		echo -e "\n**** Config Status: Contents of the MAIN super settings file located at ${SUPER_MAIN_PLIST}.plist:"
		defaults read "${SUPER_MAIN_PLIST}" | tail -n +2 | sed -e 's/    //g' -e 's/;//g' -e '$d'
	else
		echo -e "\n**** Config Error: No MAIN super settings file in the expected location at ${SUPER_MAIN_PLIST}.plist."
	fi
	
	if [[ $(ls "${SUPER_CONFIGS_FOLDER}") ]]; then
		for super_inactive_alt_plist in "${SUPER_CONFIGS_FOLDER:?}"/*.plist; do
			if [[ "${super_inactive_alt_plist}" != "${alternate_temporary_plist}.plist" ]] && [[ "${super_inactive_alt_plist}" != "${alternate_default_plist}.plist" ]]; then
				echo -e "\n**** Config Status: Contents of the INACTIVE super settings file located at ${super_inactive_alt_plist}:"
				defaults read "${super_inactive_alt_plist}" | tail -n +2 | sed -e 's/    //g' -e 's/;//g' -e '$d'
			fi
		done
	fi
	
	local system_profiler_install_history_json
	system_profiler_install_history_json=$(system_profiler SPInstallHistoryDataType -json)
	if [[ $(echo "${system_profiler_install_history_json}" | grep -c '_name') -ge 1 ]]; then
		echo -e "\n**** Config Status: Installation history of macOS system updates:"
		echo "${system_profiler_install_history_json}" | grep -e '_name' -e 'install_date' | awk -F '"' '{print $4}' | tr '\n' ',' | sed -e 's/Z,/\n/g' -e 's/,/\t/g' | grep -e 'macOS'
		echo -e "\n**** Config Status: Installation history of macOS security updates:"
		echo "${system_profiler_install_history_json}" | grep -e '_name' -e 'install_date' | awk -F '"' '{print $4}' | tr '\n' ',' | sed -e 's/Z,/\n/g' -e 's/,/\t\t/g' -e 's/MRTConfigData/MRTConfigData\t/g' | grep -e 'XProtect' -e 'MRTConfigData'
	else
		echo -e "\n**** Config Error: The system_profiler command did not return a valid installation history list."
	fi
	
	echo ""
	log_echo "**** S.U.P.E.R.M.A.N. ${SUPER_VERSION} - EXIT CONFIGURATION STATUS ****"
	exit 0
}
...
set_defaults() {
	...
	# Path to the main property list file:
	SUPER_MAIN_PLIST="${SUPER_FOLDER}/com.macjutsu.super" # No trailing ".plist"
	readonly SUPER_MAIN_PLIST
	...
	# Path to the alternate configuration folder:
	SUPER_CONFIGS_FOLDER="${SUPER_FOLDER}/configs"
	readonly SUPER_CONFIGS_FOLDER
	...
	IBM_NOTIFIER_DOWNLOAD_URL="https://github.com/IBM/mac-ibm-notifications/releases/download/v-3.2.2-b-128/IBM.Notifier.zip"
	readonly IBM_NOTIFIER_DOWNLOAD_URL
	
	# Target version for IBM Notifier.app:
	IBM_NOTIFIER_TARGET_VERSION="3.2.2"
	...
	MIST_CLI_DOWNLOAD_URL="https://github.com/ninxsoft/mist-cli/releases/download/v2.2/mist-cli.2.2.pkg"
	readonly MIST_CLI_DOWNLOAD_URL
	
	# Target version for mist-cli:
	MIST_CLI_TARGET_VERSION="2.2"
	...
	# Path to the local Apple Setup (Assistant) Done file.
	APPLE_SETUP_DONE_FILE="/var/db/.AppleSetupDone"
	readonly APPLE_SETUP_DONE_FILE
	# IMPORTANT DETAIL: Changing this path provides no benefit as this is the default (and only) location for this file.
	...
	# The maximum number of minutes between full software update status checks when the results were previously cached.
	SOFTWARE_STATUS_CACHE_AGE_MINUTES=360
	readonly SOFTWARE_STATUS_CACHE_AGE_MINUTES
	...
	# The minimum number of seconds after a recent system startup where it's safe to start a super workflow. If the system has started up too soon, the the workflow is automatically deferred.
	RECENT_STARTUP_AUTO_DEFERRAL_SECONDS=300
	readonly RECENT_STARTUP_AUTO_DEFERRAL_SECONDS
	...
	readonly TIMEOUT_INSTALLER_WORKFLOW_SECONDS
	...
	REGEX_MACOS_TARGET_VERSION="^([1-9][0-9])(\.[0-9]{1,2})?(\.[0-9]{1,2})?$"
	readonly REGEX_MACOS_TARGET_VERSION
	...
	REGEX_SCHEDULE_WORKFLOW_ACITVE_TIME_FRAME="^(MON|TUE|WED|THU|FRI|SAT|SUN):(2[0-3]|[01][0-9]):[0-5][0-9]-(2[0-3]|[01][0-9]):[0-5][0-9]$"
	readonly REGEX_SCHEDULE_WORKFLOW_ACITVE_TIME_FRAME
...
get_options() {
	...
	while [[ -n "$1" ]]; do
		...
		--version)
			show_version
			;;
		--install-macos-minor-version-target=*)
			install_macos_minor_version_target_option="${1##*=}"
			;;
		...
		--install-non-system-updates-without-restarting-off)
			...
			;;
		--install-safari-update-without-restarting)
			install_safari_update_without_restarting_option="TRUE"
			;;
		--install-safari-update-without-restarting-off)
			install_safari_update_without_restarting_option="FALSE"
			;;
		--install-prioritize-non-restart-updates)
			install_prioritize_non_restart_updates_option="TRUE"
			;;
		--install-prioritize-non-restart-updates-off)
			install_prioritize_non_restart_updates_option="FALSE"
			;;
		...
		--workflow-only-download-off)
			...
			;;
		--workflow-require-active-user)
			workflow_require_active_user_option="TRUE"
			;;
		--workflow-require-active-user-off)
			workflow_require_active_user_option="FALSE"
			;;
		...
		--workflow-disable-relaunch-off)
			...
			;;
		--workflow-reset-super-after-completion)
			workflow_reset_super_after_completion_option="TRUE"
			;;
		--workflow-reset-super-after-completion-off)
			workflow_reset_super_after_completion_option="FALSE"
			;;
		...
		--schedule-deferred-start-file=*)
			schedule_deferred_start_file_option="${1##*=}"
			;;
		--schedule-deferred-start-days=*)
			schedule_deferred_start_days_option="${1##*=}"
			;;
		...
		--display-accessory-non-system-updates-file=*)
			...
			;;
		--display-accessory-safari-update-file=*)
			display_accessory_safari_update_file_option="${1##*=}"
			;;
		...
		--test-battery-level=*)
			...
			;;
		--config-edit-main)
			config_edit_main_option="TRUE"
			;;
		--config-edit=*)
			config_edit_option="${1##*=}"
			;;
		--config-clone=*)
			config_clone_option="${1##*=}"
			;;
		--config-delete=*)
			config_delete_option="${1##*=}"
			;;
		--config-delete-all)
			config_delete_all_option="TRUE"
			;;
		--config-start-default=*)
			config_start_default_option="${1##*=}"
			;;
		--config-start-temp=*)
			config_start_temp_option="${1##*=}"
			;;
		--config-status)
			config_status_option="TRUE"
			;;
		--auth-ask-user-to-save-password)
			auth_ask_user_to_save_password_option="TRUE"
			;;
		--auth-ask-user-to-save-password-off)
			auth_ask_user_to_save_password_option="FALSE"
			;;
		--auth-local-account=*)
...
# Save a preference by name ${1} and value ${2} to the main ${SUPER_MAIN_PLIST}.
set_pref_main() {
	[[ "${2}" == "FALSE" ]] && defaults write "${SUPER_MAIN_PLIST}" "${1}" -bool false && return 0
	[[ "${2}" == "TRUE" ]] && defaults write "${SUPER_MAIN_PLIST}" "${1}" -bool true && return 0
	{ [[ "${2}" != "FALSE" ]] && [[ "${2}" != "TRUE" ]]; } && defaults write "${SUPER_MAIN_PLIST}" "${1}" -string "${2}" && return 0
}

# Save a preference by name ${1} and value ${2} to either the current ${super_alternate_plist} or the ${SUPER_MAIN_PLIST}.
set_pref() {
	local set_plist
	set_plist="${SUPER_MAIN_PLIST}"
	[[ -n "${super_alternate_plist}" ]] && set_plist="${super_alternate_plist}"
	[[ "${2}" == "FALSE" ]] && defaults write "${set_plist}" "${1}" -bool false && return 0
	[[ "${2}" == "TRUE" ]] && defaults write "${set_plist}" "${1}" -bool true && return 0
	{ [[ "${2}" != "FALSE" ]] && [[ "${2}" != "TRUE" ]]; } && defaults write "${set_plist}" "${1}" -string "${2}" && return 0
}

# Delete a preference by name ${1} from the main ${SUPER_MAIN_PLIST}.
delete_pref_main() {
...
delete_pref() {
	if [[ -n "${super_alternate_plist}" ]]; then
		defaults delete "${super_alternate_plist}" "${1}" 2>/dev/null
	else
		defaults delete "${SUPER_MAIN_PLIST}" "${1}" 2>/dev/null
	fi
}
...
get_pref_managed() {
	...
	if [[ -n "${preference_result}" ]]; then
		...
		if [[ $(echo "${preference_type}" | grep -c 'boolean') -gt 0 ]]; then
			...
			echo "${preference_result}" && return 0
		fi
	fi
	return 1
}

# Gets the value of a preference by name ${1}. This searches for values in ${super_alternate_plist}, and ${SUPER_MAIN_PLIST}. The first found in this order is returned.
get_pref() {
	local preference_result
	[[ -n "${super_alternate_plist}" ]] && preference_result=$(defaults read "${super_alternate_plist}" "${1}" 2>/dev/null)
	[[ -z "${preference_result}" ]] && preference_result=$(defaults read "${SUPER_MAIN_PLIST}" "${1}" 2>/dev/null)
	if [[ -n "${preference_result}" ]]; then
		local preference_type
		[[ -n "${super_alternate_plist}" ]] && preference_type=$(defaults read-type "${super_alternate_plist}" "${1}" 2>/dev/null)
		[[ -z "${preference_type}" ]] && preference_type=$(defaults read-type "${SUPER_MAIN_PLIST}" "${1}" 2>/dev/null)
		if [[ $(echo "${preference_type}" | grep -c 'boolean') -gt 0 ]]; then
			{ [[ "${preference_result}" == "0" ]] || [[ "${preference_result}" == "FALSE" ]]; } && echo "FALSE" && return 0
			{ [[ "${preference_result}" == "1" ]] || [[ "${preference_result}" == "TRUE" ]]; } && echo "TRUE" && return 0
		else
...
# If needed, reset any parameters stored in ${SUPER_MAIN_PLIST}.
manage_preference_resets() {
	# First handle any main preference deletion requests.
	if [[ "${reset_super_option}" == "TRUE" ]] || [[ $(get_pref "WorkflowResetSuperAfterCompletionNow") == "TRUE" ]]; then
		log_super_audit "Status: Resetting all local (non-managed and non-authentication) preferences."
		unset super_status_backup
		...
		auth_ask_user_to_save_password_backup=$(get_pref "AuthAskUserToSavePassword")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_ask_user_to_save_password_backup: ${auth_ask_user_to_save_password_backup}"
		local auth_local_account_backup
		auth_local_account_backup=$(get_pref "AuthLocalAccount")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_account_backup: ${auth_local_account_backup}"
		local auth_service_account_backup
		auth_service_account_backup=$(get_pref "AuthServiceAccount")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_account_backup: ${auth_service_account_backup}"
		local auth_jamf_client_backup
		auth_jamf_client_backup=$(get_pref "AuthJamfClient")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client_backup: ${auth_jamf_client_backup}"
		local auth_jamf_account_backup
		auth_jamf_account_backup=$(get_pref "AuthJamfAccount")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account_backup: ${auth_jamf_account_backup}"
		local auth_legacy_local_account_backup
		auth_legacy_local_account_backup=$(get_pref "LocalAccount")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_legacy_local_account_backup: ${auth_legacy_local_account_backup}"
		local auth_legacy_super_account_backup
		auth_legacy_super_account_backup=$(get_pref "SuperAccount")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_legacy_super_account_backup: ${auth_legacy_super_account_backup}"
		local auth_legacy_jamf_account_backup
		auth_legacy_jamf_account_backup=$(get_pref "JamfAccount")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_legacy_jamf_account_backup: ${auth_legacy_jamf_account_backup}"
	
		# Delete and/or reset locally saved items.
		defaults delete "${SUPER_MAIN_PLIST}" >/dev/null 2>&1
		set_pref_main "SuperVersion" "${SUPER_VERSION}"
		defaults read "${SUPER_MAIN_PLIST}" >/dev/null 2>&1
		...
		[[ "${verbose_mode}" == "TRUE" ]] && set_pref_main "VerboseMode" "TRUE"
	
		# Restore any saved preferences from backup.
		[[ "${auth_ask_user_to_save_password_backup}" == "TRUE" ]] && set_pref_main "AuthAskUserToSavePassword" "TRUE"
		[[ "${auth_local_account_backup}" == "TRUE" ]] && set_pref_main "AuthLocalAccount" "TRUE"
		[[ "${auth_service_account_backup}" == "TRUE" ]] && set_pref_main "AuthServiceAccount" "TRUE"
		[[ "${auth_jamf_client_backup}" == "TRUE" ]] && set_pref_main "AuthJamfClient" "TRUE"
		[[ "${auth_jamf_account_backup}" == "TRUE" ]] && set_pref_main "AuthJamfAccount" "TRUE"
		[[ -n "${auth_legacy_local_account_backup}" ]] && set_pref_main "LocalAccount" "${auth_legacy_local_account_backup}"
		[[ -n "${auth_legacy_super_account_backup}" ]] && set_pref_main "SuperAccount" "${auth_legacy_super_account_backup}"
		[[ -n "${auth_legacy_jamf_account_backup}" ]] && set_pref_main "JamfAccount" "${auth_legacy_jamf_account_backup}"
	else # Lesser main preference delete/reset options.
		if [[ "${deferral_timer_reset_all_option}" == "TRUE" ]]; then
			log_super_audit "Status: Resetting all deferral timer options in main preferences."
			delete_pref_main "DeferralTimerDefault"
			delete_pref_main "DeferralTimerMenu"
			delete_pref_main "DeferralTimerFocus"
			delete_pref_main "DeferralTimerError"
			delete_pref_main "DeferralTimerWorkflowRelaunch"
		fi
		if [[ "${scheduled_install_delete_all_option}" == "TRUE" ]]; then
			log_super_audit "Status: Deleting all scheduled installation options in main preferences."
			delete_pref_main "ScheduledInstallDays"
			delete_pref_main "ScheduledInstallDate"
			delete_pref_main "ScheduledInstallUserChoice"
			delete_pref_main "ScheduledInstallReminder"
			delete_pref_main "WorkflowScheduledInstall"
		fi
		if [[ "${deadline_count_delete_all_option}" == "TRUE" ]]; then
			log_super_audit "Status: Deleting all deadline count options in main preferences."
			delete_pref_main "DeadlineCountFocus"
			delete_pref_main "DeadlineCountSoft"
			delete_pref_main "DeadlineCountHard"
			delete_pref_main "DeadlineCounterFocus"
			delete_pref_main "DeadlineCounterSoft"
			delete_pref_main "DeadlineCounterHard"
		fi
		if [[ "${deadline_days_delete_all_option}" == "TRUE" ]]; then
			log_super_audit "Status: Deleting all deadline days options in main preferences."
			delete_pref_main "DeadlineDaysFocus"
			delete_pref_main "DeadlineDaysSoft"
			delete_pref_main "DeadlineDaysHard"
		fi
		if [[ "${deadline_date_delete_all_option}" == "TRUE" ]]; then
			log_super_audit "Status: Deleting all deadline date options in main preferences."
			delete_pref_main "DeadlineDateFocus"
			delete_pref_main "DeadlineDateSoft"
			delete_pref_main "DeadlineDateHard"
		fi
		if [[ "${dialog_timeout_delete_all_option}" == "TRUE" ]]; then
			log_super_audit "Status: Deleting all dialog timeout options in main preferences."
			delete_pref_main "DialogTimeoutDefault"
			delete_pref_main "DialogTimeoutUserChoice"
			delete_pref_main "DialogTimeoutSoftDeadline"
			delete_pref_main "DialogTimeoutUserAuth"
			delete_pref_main "DialogTimeoutInsufficientStorage"
			delete_pref_main "DialogTimeoutPowerRequired"
		fi
	fi
	
	# Handle any alternate workflow configuration delete requests.
	alternate_default_plist=$(get_pref "AlternateDefaultPlist")
	alternate_temporary_plist=$(get_pref "AlternateTemporaryPlist")
	config_delete_reset_workflow="FALSE"
	if [[ "${config_delete_all_option}" == "TRUE" ]]; then
		log_super_audit "Status: Deleting all workflow configurations in ${SUPER_CONFIGS_FOLDER}."
		for delete_alt_plist in "${SUPER_CONFIGS_FOLDER:?}"/*; do
			log_super_audit "Status: Deleting alternate workflow configuration ${delete_alt_plist}."
			rm -Rf "${delete_alt_plist}" >/dev/null 2>&1
		done
		if [[ -n "${alternate_default_plist}" ]]; then
			log_super_audit "Status: Workflow is no longer using deleted alternate default configuration at ${alternate_default_plist}.plist."
			delete_pref_main "AlternateDefaultPlist"
			config_delete_reset_workflow="TRUE"
		fi
		if [[ -n "${alternate_temporary_plist}" ]]; then
			log_super_audit "Status: Workflow is no longer using deleted alternate temporary configuration at ${alternate_temporary_plist}.plist."
			delete_pref_main "AlternateTemporaryPlist"
			config_delete_reset_workflow="TRUE"
		fi
	elif [[ -n "${config_delete_option}" ]]; then # Only deleting an individual alternate plist.
		if [[ -f "${SUPER_CONFIGS_FOLDER}/com.macjutsu.super.${config_delete_option}.plist" ]]; then
			log_super_audit "Status: Deleting alternate workflow configuration ${SUPER_CONFIGS_FOLDER}/com.macjutsu.super.${config_delete_option}.plist."
			rm -Rf "${SUPER_CONFIGS_FOLDER:?}/com.macjutsu.super.${config_delete_option}.plist" >/dev/null 2>&1
		else
			log_super "Error: Unable to locate alternate workflow configuration for deletion at ${SUPER_CONFIGS_FOLDER}/com.macjutsu.super.${config_delete_option}.plist."
		fi
		if [[ "${alternate_default_plist}" == "${SUPER_CONFIGS_FOLDER}/com.macjutsu.super.${config_delete_option}" ]]; then
			log_super_audit "Status: Workflow is no longer using deleted alternate default configuration at ${alternate_default_plist}.plist."
			delete_pref_main "AlternateDefaultPlist"
			config_delete_reset_workflow="TRUE"
		fi
		if [[ "${alternate_temporary_plist}" == "${SUPER_CONFIGS_FOLDER}/com.macjutsu.super.${config_delete_option}" ]]; then
			log_super_audit "Status: Workflow is no longer using deleted alternate temporary configuration at ${alternate_temporary_plist}.plist."
			delete_pref_main "AlternateTemporaryPlist"
			config_delete_reset_workflow="TRUE"
		fi
	fi
	unset alternate_default_plist
	unset alternate_temporary_plist
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: config_delete_reset_workflow is: ${config_delete_reset_workflow}"
}

# Validate non-authentication parameters and if necessary modifications to local preferences. Any errors set ${parameter_error}.
manage_parameter_options() {
	...
	config_temp_override=$(get_pref_managed "ConfigTempOverride")
	[[ -z "${config_temp_override}" ]] && config_temp_override="FALSE"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: config_temp_override: ${config_temp_override}"
	[[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && log_super "Status: Active managed workflow configuration located at ${SUPER_MANAGED_PLIST}.plist."
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Managed preference file: ${SUPER_MANAGED_PLIST}:\n$(defaults read "${SUPER_MANAGED_PLIST}" 2>/dev/null)"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Main preference file before startup validation: ${SUPER_MAIN_PLIST}:\n$(defaults read "${SUPER_MAIN_PLIST}" 2>/dev/null)"
	
	# First handle incompatible combinations of alternate config options.
	{ [[ "${config_edit_main_option}" == "TRUE" ]] && [[ -n "${config_edit_option}" ]]; } && log_super "Parameter Error: You can not use both the --config-edit-main and the --config-edit=ConfigurationName options a the same time. Instead, use multiple seperate runs of super for each configuration you want to edit." && parameter_error="TRUE"
	{ [[ -n "${config_clone_option}" ]] && [[ -z "${config_edit_option}" ]]; } && log_super "Parameter Error: To use the --config-clone=SourceConfigurationName option you must also use the --config-edit=ConfigurationName option in order to specify a desination for the cloned configuration." && parameter_error="TRUE"
	{ { [[ "${config_edit_main_option}" == "TRUE" ]] || [[ -n "${config_edit_option}" ]]; } && { [[ -n "${config_start_default_option}" ]] && [[ -n "${config_start_temp_option}" ]]; }; } && log_super "Parameter Error: You can not use both a --config-edit* option and a --config-start* option a the same time. Instead, first run super to edit configurations and then run super again to start configurations." && parameter_error="TRUE"
	{ { [[ -n "${config_delete_option}" ]] || [[ -n "${config_delete_all_option}" ]]; } && { [[ -n "${config_start_default_option}" ]] && [[ -n "${config_start_temp_option}" ]]; }; } && log_super "Parameter Error: You can not use both a --config-delete* option and a --config-start* option a the same time. Instead, first run super to delete configurations and then run super again to start configurations." && parameter_error="TRUE"
	[[ "${parameter_error}" == "TRUE" ]] && return 0
	
	# Evaluate ${config_edit_option} and ${config_clone_option}, if appropriate then set ${super_alternate_plist}. This also logs changes to configuration files and sets a new ConfigEditTimestamp.
	if [[ -n "${config_clone_option}" ]]; then
		local alternate_clone_plist
		alternate_clone_plist="${SUPER_CONFIGS_FOLDER}/com.macjutsu.super.${config_clone_option}"
		super_alternate_plist="${SUPER_CONFIGS_FOLDER}/com.macjutsu.super.${config_edit_option}"
		if [[ -f "${alternate_clone_plist}.plist" ]]; then
			if [[ ! -f "${super_alternate_plist}.plist" ]]; then
				log_super_audit "Status: Cloning workflow configuration ${config_clone_option} to new alternate preferences at ${super_alternate_plist}.plist."
			else
				log_super_audit "Status: Cloning workflow configuration ${config_clone_option} to replace alternate preferences at ${super_alternate_plist}.plist."
				rm -Rf "${SUPER_CONFIGS_FOLDER:?}/com.macjutsu.super.${config_edit_option}.plist" >/dev/null 2>&1
			fi
			cp "${alternate_clone_plist}.plist" "${super_alternate_plist}.plist" >/dev/null 2>&1
			set_pref "ConfigEditTimestamp" "$(date +%Y-%m-%d:%H:%M:%S)"
		else
			log_super "Parameter Error: Unable to clone workflow configuration because the preferences could not be located at ${alternate_clone_plist}.plist."
			parameter_error="TRUE"
		fi
	elif [[ -n "${config_edit_option}" ]]; then
		super_alternate_plist="${SUPER_CONFIGS_FOLDER}/com.macjutsu.super.${config_edit_option}"
		log_super_audit "Status: Saving workflow configuration to alternate preferences at ${super_alternate_plist}.plist."
		set_pref "ConfigEditTimestamp" "$(date +%Y-%m-%d:%H:%M:%S)"
	fi
	if [[ -z "${super_alternate_plist}" ]] && [[ "${config_update}" == "TRUE" ]]; then
		log_super_audit "Status: Saving workflow configuration to main preferences at ${SUPER_MAIN_PLIST}.plist."
		set_pref_main "ConfigEditTimestamp" "$(date +%Y-%m-%d:%H:%M:%S)"
	fi
	[[ "${parameter_error}" == "TRUE" ]] && return 0
	
	# If the workflow should start, then evaluate ${config_start_default_option} and ${config_start_temp_option} and possibly save to ${SUPER_MAIN_PLIST} and set ${super_alternate_plist}.
	if [[ "${config_edit_main_option}" != "TRUE" ]] && [[ -z "${config_edit_option}" ]] && [[ -z "${config_delete_option}" ]] && [[ -z "${config_delete_all_option}" ]]; then
		if [[ "${config_start_default_option}" == "X" ]]; then
			unset_alternate_plist=$(get_pref "AlternateDefaultPlist")
			if [[ -n "${unset_alternate_plist}" ]]; then
				delete_pref_main "AlternateDefaultPlist"
				log_super_audit "Status: Workflow is no longer using alternate default configuration at ${unset_alternate_plist}.plist."
				unset unset_alternate_plist
			fi
			unset config_start_default_option
		fi
		if [[ -n "${config_start_default_option}" ]]; then
			alternate_default_plist="${SUPER_CONFIGS_FOLDER}/com.macjutsu.super.${config_start_default_option}"
			if [[ -f "${alternate_default_plist}.plist" ]]; then
				log_super_audit "Status: Setting default workflow configuration to ${alternate_default_plist}.plist."
				set_pref_main "AlternateDefaultPlist" "${alternate_default_plist}"
			else
				log_super "Parameter Error: Unable to set new default workflow configuration because the preferences could not be located at ${alternate_default_plist}.plist."
				parameter_error="TRUE"
			fi
		fi
		if [[ "${config_start_temp_option}" == "X" ]]; then
			unset_alternate_plist=$(get_pref "AlternateTemporaryPlist")
			if [[ -n "${unset_alternate_plist}" ]]; then
				delete_pref_main "AlternateTemporaryPlist"
				log_super_audit "Status: Workflow is no longer using alternate temporary configuration at ${unset_alternate_plist}.plist."
				unset unset_alternate_plist
			fi
			unset config_start_temp_option
		fi
		if [[ -n "${config_start_temp_option}" ]]; then
			alternate_temporary_plist="${SUPER_CONFIGS_FOLDER}/com.macjutsu.super.${config_start_temp_option}"
			if [[ -f "${alternate_temporary_plist}.plist" ]]; then
				log_super_audit "Status: Setting temporary workflow configuration to ${alternate_temporary_plist}.plist."
				set_pref_main "AlternateTemporaryPlist" "${alternate_temporary_plist}"
			else
				log_super "Parameter Error: Unable to set new temporary workflow configuration because the preferences could not be located at ${alternate_temporary_plist}.plist."
				parameter_error="TRUE"
			fi
		fi
		{ [[ -n "${config_start_default_option}" ]] && [[ -n "${config_start_temp_option}" ]]; } && log_super "Parameter Warning: When starting with both the --config-start-default=* and the --config-start-temp=* options at the same time, the temporary workflow configuration always starts first. The default workflow configuration will only start after the temporary workflow has completed."
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: alternate_default_plist is: ${alternate_default_plist}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: alternate_temporary_plist is: ${alternate_temporary_plist}"
		[[ "${parameter_error}" == "TRUE" ]] && return 0
		
		# Evaluate previously saved AlternateDefaultPlist and AlternateTemporaryPlist. If valid set ${alternate_default_plist} and/or ${alternate_temporary_plist}.
		[[ -z "${alternate_default_plist}" ]] && alternate_default_plist=$(get_pref "AlternateDefaultPlist")
		{ [[ -n "${alternate_default_plist}" ]] && [[ ! -f "${alternate_default_plist}.plist" ]]; } && log_super "Parameter Error: Unable to locate default workflow configuration at ${alternate_default_plist}.plist." && parameter_error="TRUE"
		[[ -z "${alternate_temporary_plist}" ]] && alternate_temporary_plist=$(get_pref "AlternateTemporaryPlist")
		{ [[ -n "${alternate_temporary_plist}" ]] && [[ ! -f "${alternate_temporary_plist}.plist" ]]; } && log_super "Parameter Error: Unable to locate temporary workflow configuration at ${alternate_temporary_plist}.plist." && parameter_error="TRUE"
		[[ "${parameter_error}" == "TRUE" ]] && return 0
		
		# At this point if there are valid ${alternate_default_plist} or ${alternate_temporary_plist} options, evaluate both and set ${super_alternate_plist}.
		if [[ -n "${alternate_temporary_plist}" ]]; then
			super_alternate_plist="${alternate_temporary_plist}"
			log_super "Status: Active temporary workflow configuration located at ${super_alternate_plist}.plist."
			{ [[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && [[ "${config_temp_override}" == "TRUE" ]]; } && log_super "Warning: The temporary workflow configuration will override conflicting settings in the managed workflow configuration."
			{ [[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && [[ "${config_temp_override}" == "FALSE" ]]; } && log_super "Warning: The managed workflow configuration will override conflicting settings in the temporary workflow configuration. You can reverse this behavior via the <key>ConfigTempOverride</key> <true/> managed preference setting."
		elif [[ -n "${alternate_default_plist}" ]]; then
			super_alternate_plist="${alternate_default_plist}"
			log_super "Status: Active default workflow configuration located at ${super_alternate_plist}.plist."
			[[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && log_super "Warning: The managed workflow configuration will override conflicting settings in the default workflow configuration."
		fi
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: super_alternate_plist is: ${super_alternate_plist}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${super_alternate_plist}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Alternate preference file before startup validation: ${super_alternate_plist}:\n$(defaults read "${super_alternate_plist}" 2>/dev/null)"
	
	# Validate ${install_macos_minor_version_target_option} and if a valid set the ${install_macos_minor_version_target} parameter.
	...
	[[ -z "${install_macos_minor_version_target}" ]] && install_macos_minor_version_target=$(get_pref "InstallMacOSMinorVersionTarget")
	{ [[ -n "${install_macos_minor_version_target}" ]] && [[ ! "${install_macos_minor_version_target}" =~ ${REGEX_MACOS_TARGET_VERSION} ]]; } && log_super "Parameter Error: The --install-macos-minor-version-target=number value must be a contemporary macOS version number (15 | 15.1 | 15.1.1)." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_macos_minor_version_target}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_minor_version_target is: ${install_macos_minor_version_target}"
	[[ -n "${install_macos_minor_version_target}" ]] && install_macos_minor_version_normalized_target=$(echo "${install_macos_minor_version_target}" | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $install_macos_minor_version_normalized_target ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_minor_version_normalized_target is: ${install_macos_minor_version_normalized_target}"
	...
	install_macos_minor_updates="TRUE"
	if [[ -n $install_macos_minor_version_normalized_target ]] && [[ ${install_macos_minor_version_normalized_target:0:2} -gt $macos_version_major ]]; then
		log_super "Parameter Warning: The --install-macos-minor-version-target=${install_macos_minor_version_target} option will be ignored as it represents a major upgrade comparted to the current ${macos_version_full}."
		unset install_macos_minor_version_target
		unset install_macos_minor_version_normalized_target
	elif [[ -n $install_macos_minor_version_normalized_target ]] && [[ $install_macos_minor_version_normalized_target -le $macos_version_normalized ]]; then
		[[ $install_macos_minor_version_normalized_target -lt $macos_version_normalized ]] && log_super "Parameter Warning: The --install-macos-minor-version-target=${install_macos_minor_version_target} option is less than current ${macos_version_full}. Disabling macOS minor update workflow."
		[[ $install_macos_minor_version_normalized_target -eq $macos_version_normalized ]] && log_super "Status: The --install-macos-minor-version-target=${install_macos_minor_version_target} option is the same as current ${macos_version_full}. Disabling macOS minor update workflow."
		install_macos_minor_updates="FALSE"
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_minor_updates is: ${install_macos_minor_updates}"
	
	# Manage ${install_macos_major_upgrades} parameter.
	install_macos_major_upgrades=$(get_pref_managed "InstallMacOSMajorUpgrades")
	{ [[ -z "${install_macos_major_upgrades}" ]] && [[ -n "${install_macos_major_upgrades_option}" ]]; } && install_macos_major_upgrades="${install_macos_major_upgrades_option}" && set_pref "InstallMacOSMajorUpgrades" "${install_macos_major_upgrades_option}"
	[[ -z "${install_macos_major_upgrades}" ]] && install_macos_major_upgrades=$(get_pref "InstallMacOSMajorUpgrades")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_macos_major_upgrades}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_upgrades is: ${install_macos_major_upgrades}"
	
	# Validate ${install_macos_major_version_target_option} and if a valid set the ${install_macos_major_version_target} parameter.
	install_macos_major_version_target=$(get_pref_managed "InstallMacOSMajorVersionTarget")
	if [[ -z "${install_macos_major_version_target}" ]] && [[ "${install_macos_major_version_target_option}" == "X" ]]; then
		delete_pref "InstallMacOSMajorVersionTarget"
	elif [[ -z "${install_macos_major_version_target}" ]] && [[ -n "${install_macos_major_version_target_option}" ]]; then
		install_macos_major_version_target="${install_macos_major_version_target_option}"
		set_pref "InstallMacOSMajorVersionTarget" "${install_macos_major_version_target}"
	fi
	[[ -z "${install_macos_major_version_target}" ]] && install_macos_major_version_target=$(get_pref "InstallMacOSMajorVersionTarget")
	{ [[ -n "${install_macos_major_version_target}" ]] && [[ ! "${install_macos_major_version_target}" =~ ${REGEX_MACOS_TARGET_VERSION} ]]; } && log_super "Parameter Error: The --install-macos-major-version-target=number value must be a contemporary macOS version number (15 | 15.1 | 15.1.1)." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_macos_major_version_target}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_version_target is: ${install_macos_major_version_target}"
	[[ -n "${install_macos_major_version_target}" ]] && install_macos_major_version_normalized_target=$(echo "${install_macos_major_version_target}" | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $install_macos_major_version_normalized_target ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_version_normalized_target is: ${install_macos_major_version_normalized_target}"
	
	# Validate the ${install_macos_major_upgrades} and ${install_macos_major_version_normalized_target} as compared to the current macOS ${macos_version_normalized}.
	if [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ -n $install_macos_major_version_normalized_target ]] && [[ $install_macos_major_version_normalized_target -le $macos_version_normalized ]]; then
		[[ $install_macos_major_version_normalized_target -lt $macos_version_normalized ]] && log_super "Parameter Warning: The --install-macos-major-version-target=${install_macos_major_version_target} option is less than current ${macos_version_full}. Disabling macOS major upgrade workflow."
		[[ $install_macos_major_version_normalized_target -eq $macos_version_normalized ]] && log_super "Status: The --install-macos-major-version-target=${install_macos_major_version_target} option is the same as current ${macos_version_full}. Disabling macOS major upgrade workflow."
		install_macos_major_upgrades="FALSE"
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_upgrades is: ${install_macos_major_upgrades}"
	
	# Manage ${install_rapid_security_responses} parameter.
	install_rapid_security_responses=$(get_pref_managed "InstallRapidSecurityResponses")
	{ [[ -z "${install_rapid_security_responses}" ]] && [[ -n "${install_rapid_security_responses_option}" ]]; } && install_rapid_security_responses="${install_rapid_security_responses_option}" && set_pref "InstallRapidSecurityResponses" "${install_rapid_security_responses_option}"
	[[ -z "${install_rapid_security_responses}" ]] && install_rapid_security_responses=$(get_pref "InstallRapidSecurityResponses")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_rapid_security_responses}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_rapid_security_responses is: ${install_rapid_security_responses}"
	
	# Manage ${install_non_system_updates_without_restarting} parameter.
	install_non_system_updates_without_restarting=$(get_pref_managed "InstallNonSystemUpdatesWithoutRestarting")
	{ [[ -z "${install_non_system_updates_without_restarting}" ]] && [[ -n "${install_non_system_updates_without_restarting_option}" ]]; } && install_non_system_updates_without_restarting="${install_non_system_updates_without_restarting_option}" && set_pref "InstallNonSystemUpdatesWithoutRestarting" "${install_non_system_updates_without_restarting_option}"
	[[ -z "${install_non_system_updates_without_restarting}" ]] && install_non_system_updates_without_restarting=$(get_pref "InstallNonSystemUpdatesWithoutRestarting")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_non_system_updates_without_restarting}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_non_system_updates_without_restarting is: ${install_non_system_updates_without_restarting}"
	
	# Manage ${install_safari_update_without_restarting} parameter.
	install_safari_update_without_restarting=$(get_pref_managed "InstallSafariUpdateWithoutRestarting")
	{ [[ -z "${install_safari_update_without_restarting}" ]] && [[ -n "${install_safari_update_without_restarting_option}" ]]; } && install_safari_update_without_restarting="${install_safari_update_without_restarting_option}" && set_pref "InstallSafariUpdateWithoutRestarting" "${install_safari_update_without_restarting_option}"
	[[ -z "${install_safari_update_without_restarting}" ]] && install_safari_update_without_restarting=$(get_pref "InstallSafariUpdateWithoutRestarting")
	{ [[ "${install_non_system_updates_without_restarting}" == "TRUE" ]] && [[ "${install_safari_update_without_restarting}" == "TRUE" ]]; } && log_super "Parameter Error: You can not use both the --install-non-system-updates-without-restarting and the --install-safari-update-without-restarting options simultaneously. Use the --install-non-system-updates-without-restarting to install all non-system updates including any Safari updates." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_safari_update_without_restarting}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_safari_update_without_restarting is: ${install_safari_update_without_restarting}"
	
	# Manage ${install_prioritize_non_restart_updates} parameter.
	install_prioritize_non_restart_updates=$(get_pref_managed "InstallPrioritizeNonRestartUpdates")
	{ [[ -z "${install_prioritize_non_restart_updates}" ]] && [[ -n "${install_prioritize_non_restart_updates_option}" ]]; } && install_prioritize_non_restart_updates="${install_prioritize_non_restart_updates_option}" && set_pref "InstallPrioritizeNonRestartUpdates" "${install_prioritize_non_restart_updates_option}"
	[[ -z "${install_prioritize_non_restart_updates}" ]] && install_prioritize_non_restart_updates=$(get_pref "InstallPrioritizeNonRestartUpdates")
	{ [[ "${install_prioritize_non_restart_updates}" == "TRUE" ]] && { [[ -z "${install_non_system_updates_without_restarting}" ]] || [[ "${install_non_system_updates_without_restarting}" == "FALSE" ]]; } && { [[ -z "${install_safari_update_without_restarting}" ]] || [[ "${install_safari_update_without_restarting}" == "FALSE" ]]; }; } && log_super "Parameter Error: To use the --install-prioritize-non-restart-updates option, you must also specifiy either the --install-non-system-updates-without-restarting option or the --install-safari-update-without-restarting option." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_prioritize_non_restart_updates}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_prioritize_non_restart_updates is: ${install_prioritize_non_restart_updates}"
	
	# Validate ${install_jamf_policy_triggers_option} and if a valid set the ${install_jamf_policy_triggers} parameter.
	install_jamf_policy_triggers=$(get_pref_managed "InstallJamfPolicyTriggers")
	if [[ -z "${install_jamf_policy_triggers}" ]] && [[ "${install_jamf_policy_triggers_option}" == "X" ]]; then
		delete_pref "InstallJamfPolicyTriggers"
	elif [[ -z "${install_jamf_policy_triggers}" ]] && [[ -n "${install_jamf_policy_triggers_option}" ]]; then
		install_jamf_policy_triggers="${install_jamf_policy_triggers_option}"
		set_pref "InstallJamfPolicyTriggers" "${install_jamf_policy_triggers}"
	fi
	[[ -z "${install_jamf_policy_triggers}" ]] && install_jamf_policy_triggers=$(get_pref "InstallJamfPolicyTriggers")
	{ [[ -n "${install_jamf_policy_triggers}" ]] && [[ ! -f "${JAMF_PRO_BINARY}" ]]; } && log_super "Parameter Error: Unable to use the --install-jamf-policy-triggers option due to missing Jamf binary." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_jamf_policy_triggers}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_jamf_policy_triggers is: ${install_jamf_policy_triggers}"
	
	# Manage ${install_jamf_policy_triggers_without_restarting} parameter.
	install_jamf_policy_triggers_without_restarting=$(get_pref_managed "InstallJamfPolicyTriggersWithoutRestarting")
	...
	[[ -z "${install_jamf_policy_triggers_without_restarting}" ]] && install_jamf_policy_triggers_without_restarting=$(get_pref "InstallJamfPolicyTriggersWithoutRestarting")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_jamf_policy_triggers_without_restarting}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_jamf_policy_triggers_without_restarting is: ${install_jamf_policy_triggers_without_restarting}"
	{ [[ "${install_jamf_policy_triggers_without_restarting}" == "TRUE" ]] && [[ ! -f "${JAMF_PRO_BINARY}" ]]; } && log_super "Parameter Error: Unable to use the --install-jamf-policy-triggers-without-restarting option due to missing Jamf binary." && parameter_error="TRUE"
	
	# Manage ${workflow_install_now} parameter.
	workflow_install_now=$(get_pref_managed "WorkflowInstallNow")
	{ [[ -z "${workflow_install_now}" ]] && [[ -n "${workflow_install_now_option}" ]]; } && workflow_install_now="${workflow_install_now_option}" && set_pref "WorkflowInstallNow" "${workflow_install_now_option}"
	[[ -z "${workflow_install_now}" ]] && workflow_install_now=$(get_pref "WorkflowInstallNow")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${workflow_install_now}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_install_now is: ${workflow_install_now}"
	
	# Manage ${workflow_only_download} parameter.
	workflow_only_download=$(get_pref_managed "WorkflowOnlyDownload")
	{ [[ -z "${workflow_only_download}" ]] && [[ -n "${workflow_only_download_option}" ]]; } && workflow_only_download="${workflow_only_download_option}" && set_pref "WorkflowOnlyDownload" "${workflow_only_download_option}"
	[[ -z "${workflow_only_download}" ]] && workflow_only_download=$(get_pref "WorkflowOnlyDownload")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${workflow_only_download}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_only_download is: ${workflow_only_download}"
	
	# Manage ${workflow_require_active_user} parameter.
	workflow_require_active_user=$(get_pref_managed "WorkflowRequireActiveUser")
	{ [[ -z "${workflow_require_active_user}" ]] && [[ -n "${workflow_require_active_user_option}" ]]; } && workflow_require_active_user="${workflow_require_active_user_option}" && set_pref "WorkflowRequireActiveUser" "${workflow_require_active_user_option}"
	[[ -z "${workflow_require_active_user}" ]] && workflow_require_active_user=$(get_pref "WorkflowRequireActiveUser")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${workflow_require_active_user}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_require_active_user is: ${workflow_require_active_user}"
	
	# Manage ${workflow_restart_without_updates} parameter.
	workflow_restart_without_updates=$(get_pref_managed "WorkflowRestartWithoutUpdates")
	{ [[ -z "${workflow_restart_without_updates}" ]] && [[ -n "${workflow_restart_without_updates_option}" ]]; } && workflow_restart_without_updates="${workflow_restart_without_updates_option}" && set_pref "WorkflowRestartWithoutUpdates" "${workflow_restart_without_updates_option}"
	...
	[[ -z "${workflow_disable_update_check}" ]] && workflow_disable_update_check=$(get_pref "WorkflowDisableUpdateCheck")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${workflow_disable_update_check}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_disable_update_check is: ${workflow_disable_update_check}"
	
	# Manage ${workflow_disable_relaunch} parameter.
	workflow_disable_relaunch=$(get_pref_managed "WorkflowDisableRelaunch")
	{ [[ -z "${workflow_disable_relaunch}" ]] && [[ -n "${workflow_disable_relaunch_option}" ]]; } && workflow_disable_relaunch="${workflow_disable_relaunch_option}" && set_pref "WorkflowDisableRelaunch" "${workflow_disable_relaunch_option}"
	[[ -z "${workflow_disable_relaunch}" ]] && workflow_disable_relaunch=$(get_pref "WorkflowDisableRelaunch")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${workflow_disable_relaunch}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_disable_relaunch is: ${workflow_disable_relaunch}"
	
	# Validate ${deferral_timer_default_option} input and if valid set the ${deferral_timer_minutes} parameter. If there is no ${deferral_timer_minutes} then set it to ${DEFERRAL_TIMER_DEFAULT_MINUTES}.
	deferral_timer_minutes=$(get_pref_managed "DeferralTimerDefault")
	if [[ -z $deferral_timer_minutes ]] && [[ "${deferral_timer_default_option}" == "X" ]]; then
		delete_pref "DeferralTimerDefault"
	elif [[ -z $deferral_timer_minutes ]] && [[ -n "${deferral_timer_default_option}" ]]; then
		deferral_timer_minutes="${deferral_timer_default_option}"
		set_pref "DeferralTimerDefault" "${deferral_timer_minutes}"
	fi
	[[ -z $deferral_timer_minutes ]] && deferral_timer_minutes=$(get_pref "DeferralTimerDefault")
	if [[ -n $deferral_timer_minutes ]] && [[ $deferral_timer_minutes =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $deferral_timer_minutes -lt 2 ]]; then
			log_super "Parameter Warning: Specified --deferral-timer-default=minutes value of ${deferral_timer_minutes} is too low, rounding up to 2 minutes."
			deferral_timer_minutes=2
		elif [[ $deferral_timer_minutes -gt 10080 ]]; then
			log_super "Parameter Warning: Specified --deferral-timer-default=minutes value of ${deferral_timer_minutes} is too high, rounding down to 10080 minutes (1 week)."
			deferral_timer_minutes=10080
		fi
	fi
	{ [[ -n $deferral_timer_minutes ]] && [[ ! $deferral_timer_minutes =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --deferral-timer-default=minutes value must only be a number." && parameter_error="TRUE"
	[[ -z $deferral_timer_minutes ]] && deferral_timer_minutes="${DEFERRAL_TIMER_DEFAULT_MINUTES}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_minutes is: ${deferral_timer_minutes}"
	
	# Validate ${deferral_timer_menu_option} input and if valid set the ${deferral_timer_menu_minutes} parameter.
	deferral_timer_menu_minutes=$(get_pref_managed "DeferralTimerMenu")
	if [[ -z "${deferral_timer_menu_minutes}" ]] && [[ "${deferral_timer_menu_option}" == "X" ]]; then
		delete_pref "DeferralTimerMenu"
	elif [[ -z "${deferral_timer_menu_minutes}" ]] && [[ -n "${deferral_timer_menu_option}" ]]; then
		deferral_timer_menu_minutes="${deferral_timer_menu_option}"
		set_pref "DeferralTimerMenu" "${deferral_timer_menu_minutes}"
	fi
	[[ -z "${deferral_timer_menu_minutes}" ]] && deferral_timer_menu_minutes=$(get_pref "DeferralTimerMenu")
	if [[ -n "${deferral_timer_menu_minutes}" ]] && [[ "${deferral_timer_menu_minutes}" =~ ${REGEX_CSV_WHOLE_NUMBERS} ]]; then
		previous_ifs="${IFS}"
		IFS=','
		local deferral_timer_menu_minutes_array
		read -r -a deferral_timer_menu_minutes_array <<<"${deferral_timer_menu_minutes}"
		for array_index in "${!deferral_timer_menu_minutes_array[@]}"; do
			if [[ ${deferral_timer_menu_minutes_array[array_index]} -lt 2 ]]; then
				log_super "Parameter Warning: Specified --deferral-timer-menu=minutes value of ${deferral_timer_menu_minutes_array[array_index]} minutes is too low, rounding up to 2 minutes."
				deferral_timer_menu_minutes_array[array_index]=2
			elif [[ ${deferral_timer_menu_minutes_array[array_index]} -gt 10080 ]]; then
				log_super "Parameter Warning: Specified --deferral-timer-menu=minutes value of ${deferral_timer_menu_minutes_array[array_index]} minutes is too high, rounding down to 10080 minutes (1 week)."
				deferral_timer_menu_minutes_array[array_index]=10080
			fi
		done
		IFS="${previous_ifs}"
	fi
	{ [[ -n "${deferral_timer_menu_minutes}" ]] && [[ ! "${deferral_timer_menu_minutes}" =~ ${REGEX_CSV_WHOLE_NUMBERS} ]]; } && log_super "Parameter Error: The --deferral-timer-menu=minutes,minutes,etc... value must only contain numbers and commas (no spaces)." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deferral_timer_menu_minutes}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_menu_minutes is: ${deferral_timer_menu_minutes}"
	
	# Validate ${deferral_timer_focus_option} input and if valid set the ${deferral_timer_focus_minutes} parameter. If there is no ${deferral_timer_focus_minutes} then set it to ${deferral_timer_minutes}.
	deferral_timer_focus_minutes=$(get_pref_managed "DeferralTimerFocus")
	if [[ -z $deferral_timer_focus_minutes ]] && [[ "${deferral_timer_focus_option}" == "X" ]]; then
		delete_pref "DeferralTimerFocus"
	elif [[ -z $deferral_timer_focus_minutes ]] && [[ -n "${deferral_timer_focus_option}" ]]; then
		deferral_timer_focus_minutes="${deferral_timer_focus_option}"
		set_pref "DeferralTimerFocus" "${deferral_timer_focus_minutes}"
	fi
	[[ -z $deferral_timer_focus_minutes ]] && deferral_timer_focus_minutes=$(get_pref "DeferralTimerFocus")
	if [[ -n $deferral_timer_focus_minutes ]] && [[ $deferral_timer_focus_minutes =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $deferral_timer_focus_minutes -lt 2 ]]; then
			log_super "Parameter Warning: Specified --deferral-timer-focus=minutes value of ${deferral_timer_focus_minutes} is too low, rounding up to 2 minutes."
			deferral_timer_focus_minutes=2
		elif [[ $deferral_timer_focus_minutes -gt 10080 ]]; then
			log_super "Parameter Warning: Specified --deferral-timer-focus=minutes value of ${deferral_timer_focus_minutes} is too high, rounding down to 10080 minutes (1 week)."
			deferral_timer_focus_minutes=10080
		fi
	fi
	{ [[ -n $deferral_timer_focus_minutes ]] && [[ ! $deferral_timer_focus_minutes =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --deferral-timer-default=minutes value must only be a number." && parameter_error="TRUE"
	[[ -z $deferral_timer_focus_minutes ]] && deferral_timer_focus_minutes=$deferral_timer_minutes
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_focus_minutes is: ${deferral_timer_focus_minutes}"
	
	# Validate ${deferral_timer_error_option} input and if valid set the ${deferral_timer_error_minutes} parameter. If there is no ${deferral_timer_error_minutes} then set it to ${deferral_timer_minutes}.
	deferral_timer_error_minutes=$(get_pref_managed "DeferralTimerError")
	if [[ -z $deferral_timer_error_minutes ]] && [[ "${deferral_timer_error_option}" == "X" ]]; then
		delete_pref "DeferralTimerError"
	elif [[ -z $deferral_timer_error_minutes ]] && [[ -n "${deferral_timer_error_option}" ]]; then
		deferral_timer_error_minutes="${deferral_timer_error_option}"
		set_pref "DeferralTimerError" "${deferral_timer_error_minutes}"
	fi
	[[ -z $deferral_timer_error_minutes ]] && deferral_timer_error_minutes=$(get_pref "DeferralTimerError")
	if [[ -n $deferral_timer_error_minutes ]] && [[ $deferral_timer_error_minutes =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $deferral_timer_error_minutes -lt 2 ]]; then
			log_super "Parameter Warning: Specified --deferral-timer-error=minutes value of ${deferral_timer_error_minutes} is too low, rounding up to 2 minutes."
			deferral_timer_error_minutes=2
		elif [[ $deferral_timer_error_minutes -gt 10080 ]]; then
			log_super "Parameter Warning: Specified --deferral-timer-error=minutes value of ${deferral_timer_error_minutes} is too high, rounding down to 10080 minutes (1 week)."
			deferral_timer_error_minutes=10080
		fi
	fi
	{ [[ -n $deferral_timer_error_minutes ]] && [[ ! $deferral_timer_error_minutes =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --deferral-timer-default=minutes value must only be a number." && parameter_error="TRUE"
	[[ -z $deferral_timer_error_minutes ]] && deferral_timer_error_minutes=$deferral_timer_minutes
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_error_minutes is: ${deferral_timer_error_minutes}"
	
	# Validate ${deferral_timer_workflow_relaunch_option} input and if valid set the ${deferral_timer_workflow_relaunch_minutes} parameter. If there is no ${deferral_timer_workflow_relaunch_minutes} then set it to ${DEFERRAL_TIMER_WORKFLOW_RELAUNCH_DEFAULT_MINUTES}.
	deferral_timer_workflow_relaunch_minutes=$(get_pref_managed "DeferralTimerWorkflowRelaunch")
	if [[ -z $deferral_timer_workflow_relaunch_minutes ]] && [[ "${deferral_timer_workflow_relaunch_option}" == "X" ]]; then
		delete_pref "DeferralTimerWorkflowRelaunch"
	elif [[ -z $deferral_timer_workflow_relaunch_minutes ]] && [[ -n "${deferral_timer_workflow_relaunch_option}" ]]; then
		deferral_timer_workflow_relaunch_minutes="${deferral_timer_workflow_relaunch_option}"
		set_pref "DeferralTimerWorkflowRelaunch" "${deferral_timer_workflow_relaunch_minutes}"
	fi
	[[ -z $deferral_timer_workflow_relaunch_minutes ]] && deferral_timer_workflow_relaunch_minutes=$(get_pref "DeferralTimerWorkflowRelaunch")
	if [[ -n $deferral_timer_workflow_relaunch_minutes ]] && [[ $deferral_timer_workflow_relaunch_minutes =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $deferral_timer_workflow_relaunch_minutes -lt 2 ]]; then
			log_super "Parameter Warning: Specified --deferral-timer-workflow-relaunch=minutes value of ${deferral_timer_workflow_relaunch_minutes} is too low, rounding up to 2 minutes."
			deferral_timer_workflow_relaunch_minutes=2
		elif [[ $deferral_timer_workflow_relaunch_minutes -gt 10080 ]]; then
			log_super "Parameter Warning: Specified --deferral-timer-workflow-relaunch=minutes value of ${deferral_timer_workflow_relaunch_minutes} is too high, rounding down to 10080 minutes (1 week)."
			deferral_timer_workflow_relaunch_minutes=10080
		fi
	fi
	{ [[ -n $deferral_timer_workflow_relaunch_minutes ]] && [[ ! $deferral_timer_workflow_relaunch_minutes =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --deferral-timer-default=minutes value must only be a number." && parameter_error="TRUE"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_workflow_relaunch_minutes is: ${deferral_timer_workflow_relaunch_minutes}"
	
	# Validate ${schedule_workflow_active_option} input and if valid set the ${schedule_workflow_active} parameter.
	schedule_workflow_active=$(get_pref_managed "ScheduleWorkflowActive")
	if [[ -z "${schedule_workflow_active}" ]] && [[ "${schedule_workflow_active_option}" == "X" ]]; then
		delete_pref "ScheduleWorkflowActive"
	elif [[ -z "${schedule_workflow_active}" ]] && [[ -n "${schedule_workflow_active_option}" ]]; then
		schedule_workflow_active="${schedule_workflow_active_option}"
		set_pref "ScheduleWorkflowActive" "${schedule_workflow_active}"
	fi
	[[ -z "${schedule_workflow_active}" ]] && schedule_workflow_active=$(get_pref "ScheduleWorkflowActive")
	if [[ -n "${schedule_workflow_active}" ]]; then
		previous_ifs="${IFS}"
		IFS=','
		read -r -a schedule_workflow_active_array <<<"${schedule_workflow_active}"
		for schedule_workflow_active_time_frame in "${schedule_workflow_active_array[@]}"; do
			{ [[ ! "${schedule_workflow_active_time_frame:0:3}" =~ ${REGEX_WEEKDAY} ]]; } && log_super "Parameter Error: Unrecognized --schedule-workflow-active weekday of ${schedule_workflow_active_time_frame:0:3}. The weekdays must be specified as MON|TUE|WED|THU|FRI|SAT|SUN." && parameter_error="TRUE"
			{ [[ ! "${schedule_workflow_active_time_frame:4:11}" =~ ${REGEX_HOURS_MINUTES_RANGE} ]]; } && log_super "Parameter Error: Each --schedule-workflow-active time frame value must be formated in 24-hour time as hh:mm-hh:mm." && parameter_error="TRUE"
			{ [[ ! "${schedule_workflow_active_time_frame}" =~ ${REGEX_SCHEDULE_WORKFLOW_ACITVE_TIME_FRAME} ]]; } && log_super "Parameter Error: Each --schedule-workflow-active weekday and time frame must be formated as DAY:hh:mm-hh:mm." && parameter_error="TRUE"
		done
		IFS="${previous_ifs}"
	fi
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${schedule_workflow_active}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_workflow_active is: ${schedule_workflow_active}"
	
	# Validate ${schedule_deferred_start_file_option} input and if valid set the ${schedule_deferred_start_file} parameter.
	schedule_deferred_start_file=$(get_pref_managed "ScheduleDeferredStartFile")
	if [[ -z "${schedule_deferred_start_file}" ]] && [[ "${schedule_deferred_start_file_option}" == "X" ]]; then
		delete_pref "ScheduleDeferredStartFile"
	elif [[ -z "${schedule_deferred_start_file}" ]] && [[ -n "${schedule_deferred_start_file_option}" ]]; then
		schedule_deferred_start_file="${schedule_deferred_start_file_option}"
		set_pref "ScheduleDeferredStartFile" "${schedule_deferred_start_file}"
	fi
	[[ -z "${schedule_deferred_start_file}" ]] && schedule_deferred_start_file=$(get_pref "ScheduleDeferredStartFile")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${schedule_deferred_start_file}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_deferred_start_file is: ${schedule_deferred_start_file}"
	
	# Validate ${schedule_deferred_start_days_option} input and if valid set the ${schedule_deferred_start_days} parameter.
	schedule_deferred_start_days=$(get_pref_managed "ScheduleDeferredStartDays")
	if [[ -z $schedule_deferred_start_days ]] && [[ "${schedule_deferred_start_days_option}" == "X" ]]; then
		delete_pref "ScheduleDeferredStartDays"
	elif [[ -z $schedule_deferred_start_days ]] && [[ -n "${schedule_deferred_start_days_option}" ]]; then
		schedule_deferred_start_days="${schedule_deferred_start_days_option}"
		set_pref "ScheduleDeferredStartDays" "${schedule_deferred_start_days}"
	fi
	[[ -z $schedule_deferred_start_days ]] && schedule_deferred_start_days=$(get_pref "ScheduleDeferredStartDays")
	if [[ -n $schedule_deferred_start_days ]] && [[ $schedule_deferred_start_days =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $schedule_deferred_start_days -lt 1 ]]; then
			log_super "Parameter Warning: Specified --schedule-deferred-start-days=number value of ${schedule_deferred_start_days} must be greater than zero days to have any affect."
			unset schedule_deferred_start_days
		elif [[ $schedule_deferred_start_days -gt 90 ]]; then
			log_super "Parameter Warning: Specified --schedule-deferred-start-days=number value of ${schedule_deferred_start_days} days is too high, rounding down to 90 days."
			schedule_deferred_start_days=90
		fi
	fi
	{ [[ -n $schedule_deferred_start_days ]] && [[ ! $schedule_deferred_start_days =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --schedule-deferred-start-days=number value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $schedule_deferred_start_days ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_deferred_start_days is: ${schedule_deferred_start_days}"
	
	# Manage ${schedule_zero_date_release} parameter.
	schedule_zero_date_release=$(get_pref_managed "ScheduleZeroDateRelease")
	{ [[ -z "${schedule_zero_date_release}" ]] && [[ -n "${schedule_zero_date_release_option}" ]]; } && schedule_zero_date_release="${schedule_zero_date_release_option}" && set_pref "ScheduleZeroDateRelease" "${schedule_zero_date_release_option}"
	[[ -z "${schedule_zero_date_release}" ]] && schedule_zero_date_release=$(get_pref "ScheduleZeroDateRelease")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${schedule_zero_date_release}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_release is: ${schedule_zero_date_release}"
	
	# Validate ${schedule_zero_date_sofa_custom_url_option} input and if valid set the ${sofa_macos_url} parameter.  If there is no ${sofa_macos_url} then set it to ${SOFA_MACOS_DEFAULT_URL}.
	sofa_macos_url=$(get_pref_managed "ScheduleZeroDateSOFACustomURL")
	if [[ -z "${sofa_macos_url}" ]] && [[ "${schedule_zero_date_sofa_custom_url_option}" == "X" ]]; then
		delete_pref "ScheduleZeroDateSOFACustomURL"
	elif [[ -z "${sofa_macos_url}" ]] && [[ -n "${schedule_zero_date_sofa_custom_url_option}" ]]; then
		sofa_macos_url="${schedule_zero_date_sofa_custom_url_option}"
		set_pref "ScheduleZeroDateSOFACustomURL" "${sofa_macos_url}"
	fi
	[[ -z "${sofa_macos_url}" ]] && sofa_macos_url=$(get_pref "ScheduleZeroDateSOFACustomURL")
	{ [[ -n "${sofa_macos_url}" ]] && [[ ! "${sofa_macos_url}" =~ ${REGEX_HTTPS} ]]; } && log_super "Parameter Error: Invalid ---schedule-zero-date-sofa-custom-url VALUE must start with 'https://': ${sofa_macos_url}" && parameter_error="TRUE"
	{ [[ -n "${sofa_macos_url}" ]] && [[ "${schedule_zero_date_release}" != "TRUE" ]]; } && log_super "Parameter Error: To use the --schedule-zero-date-sofa-custom-url option you must also use the --schedule-zero-date-release option." && parameter_error="TRUE"
	[[ -z "${sofa_macos_url}" ]] && sofa_macos_url="${SOFA_MACOS_DEFAULT_URL}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${sofa_macos_url}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: sofa_macos_url is: ${sofa_macos_url}"
	
	# Validate ${schedule_zero_date_manual_option} input and if valid set the ${schedule_zero_date_manual} parameter.
	schedule_zero_date_manual=$(get_pref_managed "ScheduleZeroDateManual")
	if [[ -z "${schedule_zero_date_manual}" ]] && [[ "${schedule_zero_date_manual_option}" == "X" ]]; then
		delete_pref "ScheduleZeroDateManual"
	elif [[ -z "${schedule_zero_date_manual}" ]] && [[ -n "${schedule_zero_date_manual_option}" ]]; then
		schedule_zero_date_manual="${schedule_zero_date_manual_option}"
		set_pref "ScheduleZeroDateManual" "${schedule_zero_date_manual}"
	fi
	[[ -z "${schedule_zero_date_manual}" ]] && schedule_zero_date_manual=$(get_pref "ScheduleZeroDateManual")
	if [[ -n "${schedule_zero_date_manual}" ]]; then
		extract_date="${schedule_zero_date_manual:0:10}"
		if [[ "${extract_date}" =~ ${REGEX_DATE} ]]; then
			extract_time="${schedule_zero_date_manual:11:5}"
	...
	{ [[ -n "${schedule_zero_date_manual}" ]] && [[ "${schedule_zero_date_release}" == "TRUE" ]]; } && log_super "Parameter Warning: When both the --schedule-zero-date-manual and the --schedule-zero-date-release options are enabled the --schedule-zero-date-manual option takes priority." && unset schedule_zero_date_release
	{ [[ -n "${schedule_zero_date_manual}" ]] && [[ -n $schedule_deferred_start_days ]]; } && log_super "Parameter Warning: When both the --schedule-zero-date-manual and the --schedule-deferred-start-days options are enabled the --schedule-zero-date-manual option takes priority." && unset schedule_deferred_start_days
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${schedule_zero_date_manual}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_manual is: ${schedule_zero_date_manual}"
	
	# Validate ${scheduled_install_days_option} input and if valid set the ${scheduled_install_days} parameter.
	scheduled_install_days=$(get_pref_managed "ScheduledInstallDays")
	if [[ -z "${scheduled_install_days}" ]] && [[ "${scheduled_install_days_option}" == "X" ]]; then
		delete_pref "ScheduledInstallDays"
	elif [[ -z "${scheduled_install_days}" ]] && [[ -n "${scheduled_install_days_option}" ]]; then
	...
	scheduled_install_date=$(get_pref_managed "ScheduledInstallDate")
	if [[ -z "${scheduled_install_date}" ]] && [[ "${scheduled_install_date_option}" == "X" ]]; then
		delete_pref "ScheduledInstallDate"
	elif [[ -z "${scheduled_install_date}" ]] && [[ -n "${scheduled_install_date_option}" ]]; then
		scheduled_install_date="${scheduled_install_date_option}"
		set_pref "ScheduledInstallDate" "${scheduled_install_date}"
	fi
	[[ -z "${scheduled_install_date}" ]] && scheduled_install_date=$(get_pref "ScheduledInstallDate")
	if [[ -n "${scheduled_install_date}" ]]; then
		extract_date="${scheduled_install_date:0:10}"
		if [[ "${extract_date}" =~ ${REGEX_DATE} ]]; then
			extract_time="${scheduled_install_date:11:5}"
			if [[ -n "${extract_time}" ]]; then
				extract_hours="${extract_time:0:2}"
				[[ -z "${extract_hours}" ]] && extract_hours="00"
				extract_minutes="${extract_time:3:2}"
	...
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${scheduled_install_date}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_date is: ${scheduled_install_date}"
	
	# Manage ${scheduled_install_user_choice} parameter.
	scheduled_install_user_choice=$(get_pref_managed "ScheduledInstallUserChoice")
	{ [[ -z "${scheduled_install_user_choice}" ]] && [[ -n "${scheduled_install_user_choice_option}" ]]; } && scheduled_install_user_choice="${scheduled_install_user_choice_option}" && set_pref "ScheduledInstallUserChoice" "${scheduled_install_user_choice_option}"
	[[ -z "${scheduled_install_user_choice}" ]] && scheduled_install_user_choice=$(get_pref "ScheduledInstallUserChoice")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${scheduled_install_user_choice}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_user_choice is: ${scheduled_install_user_choice}"
	
	# Validated that ${scheduled_install_days}, ${scheduled_install_date}, and ${scheduled_install_user_choice} are not simultaneously active.
	...
	[[ "${scheduled_install_user_choice}" == "TRUE" ]] && ((scheduled_install_option_counter++))
	[[ $scheduled_install_option_counter -gt 1 ]] && log_super "Parameter Error: You can only use one of the following scheduled restart options at a time: --scheduled-install-days, --scheduled-install-date, or --scheduled-install-user-choice." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $scheduled_install_option_counter ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_option_counter is: ${scheduled_install_option_counter}"
	
	# Validate ${scheduled_install_reminder_option} input and if valid set the ${scheduled_install_reminder_minutes} parameter.
	scheduled_install_reminder_minutes=$(get_pref_managed "ScheduledInstallReminder")
	if [[ -z "${scheduled_install_reminder_minutes}" ]] && [[ "${scheduled_install_reminder_option}" == "X" ]]; then
		delete_pref "ScheduledInstallReminder"
	elif [[ -z "${scheduled_install_reminder_minutes}" ]] && [[ -n "${scheduled_install_reminder_option}" ]]; then
		scheduled_install_reminder_minutes="${scheduled_install_reminder_option}"
		set_pref "ScheduledInstallReminder" "${scheduled_install_reminder_minutes}"
	fi
	[[ -z "${scheduled_install_reminder_minutes}" ]] && scheduled_install_reminder_minutes=$(get_pref "ScheduledInstallReminder")
	if [[ -n "${scheduled_install_reminder_minutes}" ]] && [[ "${scheduled_install_reminder_minutes}" =~ ${REGEX_CSV_WHOLE_NUMBERS} ]]; then
		previous_ifs="${IFS}"
		IFS=','
		local scheduled_install_reminder_minutes_array
		read -r -a scheduled_install_reminder_minutes_array <<<"${scheduled_install_reminder_minutes}"
		for array_index in "${!scheduled_install_reminder_minutes_array[@]}"; do
			if [[ ${scheduled_install_reminder_minutes_array[array_index]} -lt 2 ]]; then
				log_super "Parameter Warning: Specified --scheduled-install-reminder=minutes value of ${scheduled_install_reminder_minutes_array[array_index]} minutes is too low, rounding up to 2 minutes."
				scheduled_install_reminder_minutes_array[array_index]=2
			elif [[ ${scheduled_install_reminder_minutes_array[array_index]} -gt 10080 ]]; then
				log_super "Parameter Warning: Specified --scheduled-install-reminder=minutes value of ${scheduled_install_reminder_minutes_array[array_index]} minutes is too high, rounding down to 10080 minutes (1 week)."
				scheduled_install_reminder_minutes_array[array_index]=10080
			fi
		done
		IFS="${previous_ifs}"
	fi
	{ [[ -n "${scheduled_install_reminder_minutes}" ]] && [[ ! "${scheduled_install_reminder_minutes}" =~ ${REGEX_CSV_WHOLE_NUMBERS} ]]; } && log_super "Parameter Error: The --scheduled-install-reminder=minutes,minutes,etc... value must only contain numbers and commas (no spaces)." && parameter_error="TRUE"
	{ [[ -n "${scheduled_install_reminder_minutes}" ]] && [[ $scheduled_install_option_counter -eq 0 ]]; } && log_super "Parameter Error: The --scheduled-install-reminder=minutes option requires that you also set one of the following options: --scheduled-install-days, --scheduled-install-date, or --scheduled-install-user-choice." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${scheduled_install_reminder_minutes}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_reminder_minutes is: ${scheduled_install_reminder_minutes}"
	
	# Validate ${deadline_count_focus_option} input and if valid set the ${deadline_count_focus} parameter.
	deadline_count_focus=$(get_pref_managed "DeadlineCountFocus")
	if [[ -z "${deadline_count_focus}" ]] && [[ "${deadline_count_focus_option}" == "X" ]]; then
		delete_pref "DeadlineCountFocus"
	elif [[ -z "${deadline_count_focus}" ]] && [[ -n "${deadline_count_focus_option}" ]]; then
		deadline_count_focus="${deadline_count_focus_option}"
		set_pref "DeadlineCountFocus" "${deadline_count_focus}"
	fi
	[[ -z "${deadline_count_focus}" ]] && deadline_count_focus=$(get_pref "DeadlineCountFocus")
	{ [[ -n "${deadline_count_focus}" ]] && [[ ! "${deadline_count_focus}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --deadline-count-focus=number value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_count_focus}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_count_focus is: ${deadline_count_focus}"
	
	# Validate ${deadline_count_soft_option} input and if valid set the ${deadline_count_soft} parameter.
	deadline_count_soft=$(get_pref_managed "DeadlineCountSoft")
	if [[ -z "${deadline_count_soft}" ]] && [[ "${deadline_count_soft_option}" == "X" ]]; then
		delete_pref "DeadlineCountSoft"
	elif [[ -z "${deadline_count_soft}" ]] && [[ -n "${deadline_count_soft_option}" ]]; then
		deadline_count_soft="${deadline_count_soft_option}"
		set_pref "DeadlineCountSoft" "${deadline_count_soft}"
	fi
	[[ -z "${deadline_count_soft}" ]] && deadline_count_soft=$(get_pref "DeadlineCountSoft")
	{ [[ -n "${deadline_count_soft}" ]] && [[ ! "${deadline_count_soft}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --deadline-count-soft=number value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_count_soft}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_count_soft is: ${deadline_count_soft}"
	
	# Validate ${deadline_count_hard_option} input and if valid set the ${deadline_count_hard} parameter.
	deadline_count_hard=$(get_pref_managed "DeadlineCountHard")
	if [[ -z "${deadline_count_hard}" ]] && [[ "${deadline_count_hard_option}" == "X" ]]; then
		delete_pref "DeadlineCountHard"
	elif [[ -z "${deadline_count_hard}" ]] && [[ -n "${deadline_count_hard_option}" ]]; then
		deadline_count_hard="${deadline_count_hard_option}"
		set_pref "DeadlineCountHard" "${deadline_count_hard}"
	fi
	[[ -z "${deadline_count_hard}" ]] && deadline_count_hard=$(get_pref "DeadlineCountHard")
	{ [[ -n "${deadline_count_hard}" ]] && [[ ! "${deadline_count_hard}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --deadline-count-hard=number value must only be a number." && parameter_error="TRUE"
	{ [[ -n "${deadline_count_soft}" ]] && [[ -n "${deadline_count_hard}" ]]; } && log_super "Parameter Error: You can not use both the --deadline-count-soft and --deadline-count-hard options at the same time. You must pick one deadline count behavior." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_count_hard}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_count_hard is: ${deadline_count_hard}"
	
	# Validate ${deadline_days_focus_option} input and if valid set the ${deadline_days_focus} parameter.
	deadline_days_focus=$(get_pref_managed "DeadlineDaysFocus")
	if [[ -z "${deadline_days_focus}" ]] && [[ "${deadline_days_focus_option}" == "X" ]]; then
		delete_pref "DeadlineDaysFocus"
	elif [[ -z "${deadline_days_focus}" ]] && [[ -n "${deadline_days_focus_option}" ]]; then
		deadline_days_focus="${deadline_days_focus_option}"
		set_pref "DeadlineDaysFocus" "${deadline_days_focus}"
	fi
	[[ -z "${deadline_days_focus}" ]] && deadline_days_focus=$(get_pref "DeadlineDaysFocus")
	{ [[ -n "${deadline_days_focus}" ]] && [[ ! "${deadline_days_focus}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --deadline-days-focus=number value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_days_focus}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_focus is: ${deadline_days_focus}"
	
	# Validate ${deadline_days_soft_option} input and if valid set the ${deadline_days_soft} parameter.
	deadline_days_soft=$(get_pref_managed "DeadlineDaysSoft")
	if [[ -z "${deadline_days_soft}" ]] && [[ "${deadline_days_soft_option}" == "X" ]]; then
		delete_pref "DeadlineDaysSoft"
	elif [[ -z "${deadline_days_soft}" ]] && [[ -n "${deadline_days_soft_option}" ]]; then
		deadline_days_soft="${deadline_days_soft_option}"
		set_pref "DeadlineDaysSoft" "${deadline_days_soft}"
	fi
	[[ -z "${deadline_days_soft}" ]] && deadline_days_soft=$(get_pref "DeadlineDaysSoft")
	{ [[ -n "${deadline_days_soft}" ]] && [[ ! "${deadline_days_soft}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --deadline-days-soft=number value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_days_soft}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_soft is: ${deadline_days_soft}"
	
	# Validate ${deadline_days_hard_option} input and if valid set the ${deadline_days_hard} parameter.
	deadline_days_hard=$(get_pref_managed "DeadlineDaysHard")
	if [[ -z "${deadline_days_hard}" ]] && [[ "${deadline_days_hard_option}" == "X" ]]; then
		delete_pref "DeadlineDaysHard"
	elif [[ -z "${deadline_days_hard}" ]] && [[ -n "${deadline_days_hard_option}" ]]; then
		deadline_days_hard="${deadline_days_hard_option}"
		set_pref "DeadlineDaysHard" "${deadline_days_hard}"
	fi
	[[ -z "${deadline_days_hard}" ]] && deadline_days_hard=$(get_pref "DeadlineDaysHard")
	{ [[ -n "${deadline_days_hard}" ]] && [[ ! "${deadline_days_hard}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --deadline-days-hard=number value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_days_hard}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_hard is: ${deadline_days_hard}"
	
	# Validate ${deadline_days_focus}, ${deadline_days_soft}, ${deadline_days_hard}, and ${schedule_deferred_start_days} parameters in relation to each other.
	{ [[ -n "${deadline_days_hard}" ]] && [[ -n "${deadline_days_soft}" ]] && [[ $deadline_days_hard -le $deadline_days_soft ]]; } && log_super "Parameter Error: The --deadline-days-hard=number value of ${deadline_days_hard} day(s) must be more than the --deadline-days-soft=number value of ${deadline_days_soft} day(s)." && parameter_error="TRUE"
	{ [[ -n "${deadline_days_hard}" ]] && [[ -n "${deadline_days_focus}" ]] && [[ $deadline_days_hard -le $deadline_days_focus ]]; } && log_super "Parameter Error: The --deadline-days-hard=number value of ${deadline_days_hard} day(s) must be more than the --deadline-days-focus=number value of ${deadline_days_focus} day(s)." && parameter_error="TRUE"
	{ [[ -n "${deadline_days_soft}" ]] && [[ -n "${deadline_days_focus}" ]] && [[ $deadline_days_soft -le $deadline_days_focus ]]; } && log_super "Parameter Error: The --deadline-days-soft=number value of ${deadline_days_soft} day(s) must be more than the --deadline-days-focus=number value of ${deadline_days_focus} day(s)." && parameter_error="TRUE"
	{ [[ -n "${deadline_days_focus}" ]] && [[ -n $schedule_deferred_start_days ]] && [[ $deadline_days_focus -le $schedule_deferred_start_days ]]; } && log_super "Parameter Error: The --deadline-days-focus=number value of ${deadline_days_focus} day(s) must be more than the --schedule-deferred-start-days=number value of ${schedule_deferred_start_days} day(s)." && parameter_error="TRUE"
	{ [[ -n "${deadline_days_soft}" ]] && [[ -n $schedule_deferred_start_days ]] && [[ $deadline_days_soft -le $schedule_deferred_start_days ]]; } && log_super "Parameter Error: The --deadline-days-soft=number value of ${deadline_days_soft} day(s) must be more than the --schedule-deferred-start-days=number value of ${schedule_deferred_start_days} day(s)." && parameter_error="TRUE"
	{ [[ -n "${deadline_days_hard}" ]] && [[ -n $schedule_deferred_start_days ]] && [[ $deadline_days_hard -le $schedule_deferred_start_days ]]; } && log_super "Parameter Error: The --deadline-days-hard=number value of ${deadline_days_hard} day(s) must be more than the --schedule-deferred-start-days=number value of ${schedule_deferred_start_days} day(s)." && parameter_error="TRUE"
	
	
	# Validate ${deadline_date_focus_option} input and if valid set the ${deadline_date_focus} and ${deadline_date_focus_epoch} parameters.
	deadline_date_focus=$(get_pref_managed "DeadlineDateFocus")
	if [[ -z "${deadline_date_focus}" ]] && [[ "${deadline_date_focus_option}" == "X" ]]; then
		delete_pref "DeadlineDateFocus"
	elif [[ -z "${deadline_date_focus}" ]] && [[ -n "${deadline_date_focus_option}" ]]; then
		deadline_date_focus="${deadline_date_focus_option}"
		set_pref "DeadlineDateFocus" "${deadline_date_focus}"
	fi
	[[ -z "${deadline_date_focus}" ]] && deadline_date_focus=$(get_pref "DeadlineDateFocus")
	if [[ -n "${deadline_date_focus}" ]]; then
		extract_date="${deadline_date_focus:0:10}"
		if [[ "${extract_date}" =~ ${REGEX_DATE} ]]; then
			extract_time="${deadline_date_focus:11:5}"
			if [[ -n "${extract_time}" ]]; then
				extract_hours="${extract_time:0:2}"
				[[ -z "${extract_hours}" ]] && extract_hours="00"
				extract_minutes="${extract_time:3:2}"
	...
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_date_focus}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_focus is: ${deadline_date_focus}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_date_focus_epoch}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_focus_epoch is: ${deadline_date_focus_epoch}"
	
	# Validate ${deadline_date_soft_option} input and if valid set the ${deadline_date_soft} and ${deadline_date_soft_epoch} parameters.
	deadline_date_soft=$(get_pref_managed "DeadlineDateSoft")
	if [[ -z "${deadline_date_soft}" ]] && [[ "${deadline_date_soft_option}" == "X" ]]; then
		delete_pref "DeadlineDateSoft"
	elif [[ -z "${deadline_date_soft}" ]] && [[ -n "${deadline_date_soft_option}" ]]; then
		deadline_date_soft="${deadline_date_soft_option}"
		set_pref "DeadlineDateSoft" "${deadline_date_soft}"
	fi
	[[ -z "${deadline_date_soft}" ]] && deadline_date_soft=$(get_pref "DeadlineDateSoft")
	if [[ -n "${deadline_date_soft}" ]]; then
		extract_date="${deadline_date_soft:0:10}"
		if [[ "${extract_date}" =~ ${REGEX_DATE} ]]; then
			extract_time="${deadline_date_soft:11:5}"
			if [[ -n "${extract_time}" ]]; then
				extract_hours="${extract_time:0:2}"
				[[ -z "${extract_hours}" ]] && extract_hours="00"
				extract_minutes="${extract_time:3:2}"
	...
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_date_soft}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_soft is: ${deadline_date_soft}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_date_soft_epoch}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_soft_epoch is: ${deadline_date_soft_epoch}"
	
	# Validate ${deadline_date_hard_option} input and if valid set the ${deadline_date_hard} and ${deadline_date_hard_epoch} parameters.
	deadline_date_hard=$(get_pref_managed "DeadlineDateHard")
	if [[ -z "${deadline_date_hard}" ]] && [[ "${deadline_date_hard_option}" == "X" ]]; then
		delete_pref "DeadlineDateHard"
	elif [[ -z "${deadline_date_hard}" ]] && [[ -n "${deadline_date_hard_option}" ]]; then
		deadline_date_hard="${deadline_date_hard_option}"
		set_pref "DeadlineDateHard" "${deadline_date_hard}"
	fi
	[[ -z "${deadline_date_hard}" ]] && deadline_date_hard=$(get_pref "DeadlineDateHard")
	if [[ -n "${deadline_date_hard}" ]]; then
		extract_date="${deadline_date_hard:0:10}"
		if [[ "${extract_date}" =~ ${REGEX_DATE} ]]; then
			extract_time="${deadline_date_hard:11:5}"
			if [[ -n "${extract_time}" ]]; then
				extract_hours="${extract_time:0:2}"
				[[ -z "${extract_hours}" ]] && extract_hours="00"
				extract_minutes="${extract_time:3:2}"
	...
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_date_hard}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_hard is: ${deadline_date_hard}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${deadline_date_hard_epoch}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_hard_epoch is: ${deadline_date_hard_epoch}"
	
	# Validate ${deadline_date_focus_epoch}, ${deadline_date_soft_epoch}, and ${deadline_date_hard_epoch} in relation to each other.
	{ [[ -n "${deadline_date_hard_epoch}" ]] && [[ -n "${deadline_date_soft_epoch}" ]] && [[ $deadline_date_hard_epoch -le $deadline_date_soft_epoch ]]; } && log_super "Parameter Error: The --deadline-date-hard value of ${deadline_date_hard} must be later than the --deadline-date-soft value of ${deadline_date_soft}." && parameter_error="TRUE"
	{ [[ -n "${deadline_date_hard_epoch}" ]] && [[ -n "${deadline_date_focus_epoch}" ]] && [[ $deadline_date_hard_epoch -le $deadline_date_focus_epoch ]]; } && log_super "Parameter Error: The --deadline-date-hard value of ${deadline_date_hard} must be later than --deadline-date-focus value of ${deadline_date_focus}." && parameter_error="TRUE"
	{ [[ -n "${deadline_date_soft_epoch}" ]] && [[ -n "${deadline_date_focus_epoch}" ]] && [[ $deadline_date_soft_epoch -le $deadline_date_focus_epoch ]]; } && log_super "Parameter Error: The --deadline-date-soft value of ${deadline_date_soft} must be later than than --deadline-date-focus value of ${deadline_date_focus}." && parameter_error="TRUE"
	...
	# Validate ${dialog_timeout_default_option} input and if valid set the ${dialog_timeout_default_seconds} parameter.
	dialog_timeout_default_seconds=$(get_pref_managed "DialogTimeoutDefault")
	if [[ -z $dialog_timeout_default_seconds ]] && [[ "${dialog_timeout_default_option}" == "X" ]]; then
		delete_pref "DialogTimeoutDefault"
	elif [[ -z $dialog_timeout_default_seconds ]] && [[ -n "${dialog_timeout_default_option}" ]]; then
		dialog_timeout_default_seconds="${dialog_timeout_default_option}"
		set_pref "DialogTimeoutDefault" "${dialog_timeout_default_seconds}"
	fi
	[[ -z $dialog_timeout_default_seconds ]] && dialog_timeout_default_seconds=$(get_pref "DialogTimeoutDefault")
	if [[ -n $dialog_timeout_default_seconds ]] && [[ $dialog_timeout_default_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $dialog_timeout_default_seconds -lt 60 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-default=seconds value of ${dialog_timeout_default_seconds} is too low, rounding up to 60 seconds."
			dialog_timeout_default_seconds=60
		elif [[ $dialog_timeout_default_seconds -gt 86400 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-default=seconds value of ${dialog_timeout_default_seconds} is too high, rounding down to 86400 seconds (1 day)."
			dialog_timeout_default_seconds=86400
		fi
	fi
	{ [[ -n $dialog_timeout_default_seconds ]] && [[ ! $dialog_timeout_default_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --dialog-timeout-default=seconds value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $dialog_timeout_default_seconds ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_timeout_default_seconds is: ${dialog_timeout_default_seconds}"
	
	# Validate ${dialog_timeout_user_auth_option} input and if valid set the ${dialog_timeout_user_auth_seconds} parameter.
	dialog_timeout_user_auth_seconds=$(get_pref_managed "DialogTimeoutUserAuth")
	if [[ -z $dialog_timeout_user_auth_seconds ]] && [[ "${dialog_timeout_user_auth_option}" == "X" ]]; then
		delete_pref "DialogTimeoutUserAuth"
	elif [[ -z $dialog_timeout_user_auth_seconds ]] && [[ -n "${dialog_timeout_user_auth_option}" ]]; then
		dialog_timeout_user_auth_seconds="${dialog_timeout_user_auth_option}"
		set_pref "DialogTimeoutUserAuth" "${dialog_timeout_user_auth_seconds}"
	fi
	[[ -z $dialog_timeout_user_auth_seconds ]] && dialog_timeout_user_auth_seconds=$(get_pref "DialogTimeoutUserAuth")
	if [[ -n $dialog_timeout_user_auth_seconds ]] && [[ $dialog_timeout_user_auth_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $dialog_timeout_user_auth_seconds -lt 60 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-user-auth=seconds value of ${dialog_timeout_user_auth_seconds} is too low, rounding up to 60 seconds."
			dialog_timeout_user_auth_seconds=60
		elif [[ $dialog_timeout_user_auth_seconds -gt 86400 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-user-auth=seconds value of ${dialog_timeout_user_auth_seconds} is too high, rounding down to 86400 seconds (1 day)."
			dialog_timeout_user_auth_seconds=86400
		fi
	fi
	{ [[ -n $dialog_timeout_user_auth_seconds ]] && [[ ! $dialog_timeout_user_auth_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --dialog-timeout-user-auth=seconds value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $dialog_timeout_user_auth_seconds ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_timeout_user_auth_seconds is: ${dialog_timeout_user_auth_seconds}"
	
	# Validate ${dialog_timeout_user_choice_option} input and if valid set the ${dialog_timeout_user_choice_seconds} parameter.
	dialog_timeout_user_choice_seconds=$(get_pref_managed "DialogTimeoutUserChoice")
	if [[ -z $dialog_timeout_user_choice_seconds ]] && [[ "${dialog_timeout_user_choice_option}" == "X" ]]; then
		delete_pref "DialogTimeoutUserChoice"
	elif [[ -z $dialog_timeout_user_choice_seconds ]] && [[ -n "${dialog_timeout_user_choice_option}" ]]; then
		dialog_timeout_user_choice_seconds="${dialog_timeout_user_choice_option}"
		set_pref "DialogTimeoutUserChoice" "${dialog_timeout_user_choice_seconds}"
	fi
	[[ -z $dialog_timeout_user_choice_seconds ]] && dialog_timeout_user_choice_seconds=$(get_pref "DialogTimeoutUserChoice")
	if [[ -n $dialog_timeout_user_choice_seconds ]] && [[ $dialog_timeout_user_choice_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $dialog_timeout_user_choice_seconds -lt 60 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-user-choice=seconds value of ${dialog_timeout_user_choice_seconds} is too low, rounding up to 60 seconds."
			dialog_timeout_user_choice_seconds=60
		elif [[ $dialog_timeout_user_choice_seconds -gt 86400 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-user-choice=seconds value of ${dialog_timeout_user_choice_seconds} is too high, rounding down to 86400 seconds (1 day)."
			dialog_timeout_user_choice_seconds=86400
		fi
	fi
	{ [[ -n $dialog_timeout_user_choice_seconds ]] && [[ ! $dialog_timeout_user_choice_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --dialog-timeout-user-choice=seconds value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $dialog_timeout_user_choice_seconds ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_timeout_user_choice_seconds is: ${dialog_timeout_user_choice_seconds}"
	
	# Validate ${dialog_timeout_user_schedule_option} input and if valid set the ${dialog_timeout_user_schedule_seconds} parameter.
	dialog_timeout_user_schedule_seconds=$(get_pref_managed "DialogTimeoutUserSchedule")
	if [[ -z $dialog_timeout_user_schedule_seconds ]] && [[ "${dialog_timeout_user_schedule_option}" == "X" ]]; then
		delete_pref "DialogTimeoutUserSchedule"
	elif [[ -z $dialog_timeout_user_schedule_seconds ]] && [[ -n "${dialog_timeout_user_schedule_option}" ]]; then
		dialog_timeout_user_schedule_seconds="${dialog_timeout_user_schedule_option}"
		set_pref "DialogTimeoutUserSchedule" "${dialog_timeout_user_schedule_seconds}"
	fi
	[[ -z $dialog_timeout_user_schedule_seconds ]] && dialog_timeout_user_schedule_seconds=$(get_pref "DialogTimeoutUserSchedule")
	if [[ -n $dialog_timeout_user_schedule_seconds ]] && [[ $dialog_timeout_user_schedule_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $dialog_timeout_user_schedule_seconds -lt 60 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-user-schedule=seconds value of ${dialog_timeout_user_schedule_seconds} is too low, rounding up to 60 seconds."
			dialog_timeout_user_schedule_seconds=60
		elif [[ $dialog_timeout_user_schedule_seconds -gt 86400 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-user-schedule=seconds value of ${dialog_timeout_user_schedule_seconds} is too high, rounding down to 86400 seconds (1 day)."
			dialog_timeout_user_schedule_seconds=86400
		fi
	fi
	{ [[ -n $dialog_timeout_user_schedule_seconds ]] && [[ ! $dialog_timeout_user_schedule_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --dialog-timeout-user-schedule=seconds value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $dialog_timeout_user_schedule_seconds ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_timeout_user_schedule_seconds is: ${dialog_timeout_user_schedule_seconds}"
	
	# Validate ${dialog_timeout_soft_deadline_option} input and if valid set the ${dialog_timeout_soft_deadline_seconds} parameter.
	dialog_timeout_soft_deadline_seconds=$(get_pref_managed "DialogTimeoutSoftDeadline")
	if [[ -z $dialog_timeout_soft_deadline_seconds ]] && [[ "${dialog_timeout_soft_deadline_option}" == "X" ]]; then
		delete_pref "DialogTimeoutSoftDeadline"
	elif [[ -z $dialog_timeout_soft_deadline_seconds ]] && [[ -n "${dialog_timeout_soft_deadline_option}" ]]; then
		dialog_timeout_soft_deadline_seconds="${dialog_timeout_soft_deadline_option}"
		set_pref "DialogTimeoutSoftDeadline" "${dialog_timeout_soft_deadline_seconds}"
	fi
	[[ -z $dialog_timeout_soft_deadline_seconds ]] && dialog_timeout_soft_deadline_seconds=$(get_pref "DialogTimeoutSoftDeadline")
	if [[ -n $dialog_timeout_soft_deadline_seconds ]] && [[ $dialog_timeout_soft_deadline_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $dialog_timeout_soft_deadline_seconds -lt 60 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-soft-deadline=seconds value of ${dialog_timeout_soft_deadline_seconds} is too low, rounding up to 60 seconds."
			dialog_timeout_soft_deadline_seconds=60
		elif [[ $dialog_timeout_soft_deadline_seconds -gt 86400 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-soft-deadline=seconds value of ${dialog_timeout_soft_deadline_seconds} is too high, rounding down to 86400 seconds (1 day)."
			dialog_timeout_soft_deadline_seconds=86400
		fi
	fi
	{ [[ -n $dialog_timeout_soft_deadline_seconds ]] && [[ ! $dialog_timeout_soft_deadline_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --dialog-timeout-soft-deadline=seconds value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $dialog_timeout_soft_deadline_seconds ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_timeout_soft_deadline_seconds is: ${dialog_timeout_soft_deadline_seconds}"
	
	# Validate ${dialog_timeout_insufficient_storage_option} input and if valid set the ${dialog_timeout_insufficient_storage_seconds} parameter.
	dialog_timeout_insufficient_storage_seconds=$(get_pref_managed "DialogTimeoutInsufficientStorage")
	if [[ -z $dialog_timeout_insufficient_storage_seconds ]] && [[ "${dialog_timeout_insufficient_storage_option}" == "X" ]]; then
		delete_pref "DialogTimeoutInsufficientStorage"
	elif [[ -z $dialog_timeout_insufficient_storage_seconds ]] && [[ -n "${dialog_timeout_insufficient_storage_option}" ]]; then
	...
	[[ -z $dialog_timeout_insufficient_storage_seconds ]] && dialog_timeout_insufficient_storage_seconds=$(get_pref "DialogTimeoutInsufficientStorage")
	if [[ -n $dialog_timeout_insufficient_storage_seconds ]] && [[ $dialog_timeout_insufficient_storage_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $dialog_timeout_insufficient_storage_seconds -lt 60 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-insufficient-storage=seconds value of ${dialog_timeout_insufficient_storage_seconds} is too low, rounding up to 60 seconds."
			dialog_timeout_insufficient_storage_seconds=60
		elif [[ $dialog_timeout_insufficient_storage_seconds -gt 86400 ]]; then
			log_super "Parameter Warning: Specified --dialog-timeout-insufficient-storage=seconds value of ${dialog_timeout_insufficient_storage_seconds} is too high, rounding down to 86400 seconds (1 day)."
			dialog_timeout_insufficient_storage_seconds=86400
		fi
	fi
	{ [[ -n $dialog_timeout_insufficient_storage_seconds ]] && [[ ! $dialog_timeout_insufficient_storage_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --dialog-timeout-insufficient-storage=seconds value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $dialog_timeout_insufficient_storage_seconds ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_timeout_insufficient_storage_seconds is: ${dialog_timeout_insufficient_storage_seconds}"
	
	# Validate ${dialog_timeout_power_required_option} input and if valid set the ${dialog_timeout_power_required_seconds} parameter.
	dialog_timeout_power_required_seconds=$(get_pref_managed "DialogTimeoutPowerRequired")
	if [[ -z $dialog_timeout_power_required_seconds ]] && [[ "${dialog_timeout_power_required_option}" == "X" ]]; then
		delete_pref "DialogTimeoutPowerRequired"
	elif [[ -z $dialog_timeout_power_required_seconds ]] && [[ -n "${dialog_timeout_power_required_option}" ]]; then
		dialog_timeout_power_required_seconds="${dialog_timeout_power_required_option}"
		set_pref "DialogTimeoutPowerRequired" "${dialog_timeout_power_required_seconds}"
	fi
	[[ -z $dialog_timeout_power_required_seconds ]] && dialog_timeout_power_required_seconds=$(get_pref "DialogTimeoutPowerRequired")
	if [[ -n $dialog_timeout_power_required_seconds ]] && [[ $dialog_timeout_power_required_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $dialog_timeout_power_required_seconds -lt 60 ]]; then
			log_super "Parameter Warning: Specified -dialog-timeout-power-required=seconds value of ${dialog_timeout_power_required_seconds} is too low, rounding up to 60 seconds."
			dialog_timeout_power_required_seconds=60
		elif [[ $dialog_timeout_power_required_seconds -gt 86400 ]]; then
			log_super "Parameter Warning: Specified -dialog-timeout-power-required=seconds value of ${dialog_timeout_power_required_seconds} is too high, rounding down to 86400 seconds (1 day)."
			dialog_timeout_power_required_seconds=86400
		fi
	fi
	{ [[ -n $dialog_timeout_power_required_seconds ]] && [[ ! $dialog_timeout_power_required_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The -dialog-timeout-power-required=seconds value must only be a number." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n $dialog_timeout_power_required_seconds ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_timeout_power_required_seconds is: ${dialog_timeout_power_required_seconds}"
	
	# Validate ${display_unmovable_option} input and if valid set the ${display_unmovable} parameter.
	display_unmovable=$(get_pref_managed "DisplayUnmovable")
	if [[ -z "${display_unmovable}" ]] && [[ "${display_unmovable_option}" == "X" ]]; then
		delete_pref "DisplayUnmovable"
	elif [[ -z "${display_unmovable}" ]] && [[ -n "${display_unmovable_option}" ]]; then
		display_unmovable="${display_unmovable_option}"
		set_pref "DisplayUnmovable" "${display_unmovable}"
	fi
	[[ -z "${display_unmovable}" ]] && display_unmovable=$(get_pref "DisplayUnmovable")
	if [[ -n "${display_unmovable}" ]]; then
		previous_ifs="${IFS}"
		IFS=','
		local display_unmovable_array
		read -r -a display_unmovable_array <<<"${display_unmovable}"
		for option_type in "${display_unmovable_array[@]}"; do
			{ [[ ! "${option_type}" =~ ${REGEX_WORKFLOW_OPTIONS} ]]; } && log_super "Parameter Error: Unrecognized --display-unmovable type of ${option_type}. You can only specify the following types separated by commas (no spaces): ALWAYS,DIALOG,DEADLINE,SCHEDULED,INSTALLNOW,ERROR" && parameter_error="TRUE"
		done
		IFS="${previous_ifs}"
		[[ $(echo "${display_unmovable}" | grep -c 'ALWAYS') -gt 0 ]] && display_unmovable_status="TRUE"
	fi
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_unmovable}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_unmovable is: ${display_unmovable}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_unmovable_status}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_unmovable_status is: ${display_unmovable_status}"
	
	# Validate ${display_hide_background_option} input and if valid set the ${display_hide_background} parameter.
	display_hide_background=$(get_pref_managed "DisplayHideBackground")
	if [[ -z "${display_hide_background}" ]] && [[ "${display_hide_background_option}" == "X" ]]; then
		delete_pref "DisplayHideBackground"
	elif [[ -z "${display_hide_background}" ]] && [[ -n "${display_hide_background_option}" ]]; then
		display_hide_background="${display_hide_background_option}"
		set_pref "DisplayHideBackground" "${display_hide_background}"
	fi
	[[ -z "${display_hide_background}" ]] && display_hide_background=$(get_pref "DisplayHideBackground")
	if [[ -n "${display_hide_background}" ]]; then
		previous_ifs="${IFS}"
		IFS=','
		local display_hide_background_array
		read -r -a display_hide_background_array <<<"${display_hide_background}"
		for option_type in "${display_hide_background_array[@]}"; do
			{ [[ ! "${option_type}" =~ ${REGEX_WORKFLOW_OPTIONS} ]]; } && log_super "Parameter Error: Unrecognized --display-hide-background type of ${option_type}. You can only specify the following types separated by commas (no spaces): ALWAYS,DIALOG,DEADLINE,SCHEDULED,INSTALLNOW,ERROR" && parameter_error="TRUE"
		done
		IFS="${previous_ifs}"
		[[ $(echo "${display_hide_background}" | grep -c 'ALWAYS') -gt 0 ]] && display_hide_background_status="TRUE"
	fi
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_hide_background}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_hide_background is: ${display_hide_background}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_hide_background_status}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_hide_background_status is: ${display_hide_background_status}"
	
	# Validate ${display_silently_option} input and if valid set the ${display_silently} parameter.
	display_silently=$(get_pref_managed "DisplaySilently")
	if [[ -z "${display_silently}" ]] && [[ "${display_silently_option}" == "X" ]]; then
		delete_pref "DisplaySilently"
	elif [[ -z "${display_silently}" ]] && [[ -n "${display_silently_option}" ]]; then
		display_silently="${display_silently_option}"
		set_pref "DisplaySilently" "${display_silently}"
	fi
	[[ -z "${display_silently}" ]] && display_silently=$(get_pref "DisplaySilently")
	if [[ -n "${display_silently}" ]]; then
		previous_ifs="${IFS}"
		IFS=','
		local display_silently_array
		read -r -a display_silently_array <<<"${display_silently}"
		for option_type in "${display_silently_array[@]}"; do
			{ [[ ! "${option_type}" =~ ${REGEX_WORKFLOW_OPTIONS} ]]; } && log_super "Parameter Error: Unrecognized --display-silently type of ${option_type}. You can only specify the following types separated by commas (no spaces): ALWAYS,DIALOG,DEADLINE,SCHEDULED,INSTALLNOW,ERROR" && parameter_error="TRUE"
		done
		IFS="${previous_ifs}"
		[[ $(echo "${display_silently}" | grep -c 'ALWAYS') -gt 0 ]] && display_silently_status="TRUE"
	fi
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_silently}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_silently is: ${display_silently}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_silently_status}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_silently_status is: ${display_silently_status}"
	
	# Validate ${display_notifications_centered_option} input and if valid set the ${display_notifications_centered} parameter.
	display_notifications_centered=$(get_pref_managed "DisplayNotificationsCentered")
	if [[ -z "${display_notifications_centered}" ]] && [[ "${display_notifications_centered_option}" == "X" ]]; then
		delete_pref "DisplayNotificationsCentered"
	elif [[ -z "${display_notifications_centered}" ]] && [[ -n "${display_notifications_centered_option}" ]]; then
		display_notifications_centered="${display_notifications_centered_option}"
		set_pref "DisplayNotificationsCentered" "${display_notifications_centered}"
	fi
	[[ -z "${display_notifications_centered}" ]] && display_notifications_centered=$(get_pref "DisplayNotificationsCentered")
	if [[ -n "${display_notifications_centered}" ]]; then
		previous_ifs="${IFS}"
		IFS=','
		local display_notifications_centered_array
		read -r -a display_notifications_centered_array <<<"${display_notifications_centered}"
		for option_type in "${display_notifications_centered_array[@]}"; do
			{ [[ ! "${option_type}" =~ ${REGEX_WORKFLOW_OPTIONS} ]]; } && log_super "Parameter Error: Unrecognized --display-hide-progress-bar type of ${option_type}. You can only specify the following types separated by commas (no spaces): ALWAYS,DIALOG,DEADLINE,SCHEDULED,INSTALLNOW,ERROR" && parameter_error="TRUE"
		done
		IFS="${previous_ifs}"
		[[ $(echo "${display_notifications_centered}" | grep -c 'ALWAYS') -gt 0 ]] && display_notifications_centered_status="TRUE"
	fi
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_notifications_centered}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_notifications_centered is: ${display_notifications_centered}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_notifications_centered_status}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_notifications_centered_status is: ${display_notifications_centered_status}"
	...
	if [[ -n "${display_notifications_centered}" ]]; then
		...
		read -r -a display_notifications_centered_array <<<"${display_notifications_centered}"
		for option_type in "${display_notifications_centered_array[@]}"; do
			{ [[ ! "${option_type}" =~ ${REGEX_WORKFLOW_OPTIONS} ]]; } && log_super "Parameter Error: Unrecognized --display-notifications-centered type of ${option_type}. You can only specify the following types separated by commas (no spaces): ALWAYS,DIALOG,DEADLINE,SCHEDULED,INSTALLNOW,ERROR" && parameter_error="TRUE"
		done
		IFS="${previous_ifs}"
		[[ $(echo "${display_notifications_centered}" | grep -c 'ALWAYS') -gt 0 ]] && display_notifications_centered_status="TRUE"
	fi
	...
	# Validate ${display_icon_size_option} input and if valid set the ${display_icon_size} parameter. If there is no ${display_icon_size} then set it to ${DISPLAY_ICON_DEFAULT_SIZE}.
	display_icon_size=$(get_pref_managed "DisplayIconSize")
	if [[ -z $display_icon_size ]] && [[ "${display_icon_size_option}" == "X" ]]; then
		delete_pref "DisplayIconSize"
	elif [[ -z $display_icon_size ]] && [[ -n "${display_icon_size_option}" ]]; then
		display_icon_size="${display_icon_size_option}"
		set_pref "DisplayIconSize" "${display_icon_size}"
	fi
	[[ -z $display_icon_size ]] && display_icon_size=$(get_pref "DisplayIconSize")
	if [[ -n $display_icon_size ]] && [[ $display_icon_size =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $display_icon_size -lt 32 ]]; then
		...
		elif [[ $display_icon_size -gt 150 ]]; then
	...
	{ [[ -n $display_icon_size ]] && [[ ! $display_icon_size =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --display-icon-size=pixels value must only be a number." && parameter_error="TRUE"
	[[ -z $display_icon_size ]] && display_icon_size="${DISPLAY_ICON_DEFAULT_SIZE}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_icon_size is: ${display_icon_size}"
	
	# Validate ${display_icon_file_option} input and if valid set the ${display_icon_file_path} parameter.  If there is no ${display_icon_file_path} then set it to ${DISPLAY_ICON_DEFAULT_FILE}.
	display_icon_file_path=$(get_pref_managed "DisplayIconFile")
	if [[ -z "${display_icon_file_path}" ]] && [[ "${display_icon_file_option}" == "X" ]]; then
		delete_pref "DisplayIconFile"
		delete_pref "DisplayIconLightFileCachedOrigin"
		delete_pref "DisplayIconDarkFileCachedOrigin"
	elif [[ -z "${display_icon_file_path}" ]] && [[ -n "${display_icon_file_option}" ]]; then
		display_icon_file_path="${display_icon_file_option}"
		set_pref "DisplayIconFile" "${display_icon_file_path}"
	fi
	[[ -z "${display_icon_file_path}" ]] && display_icon_file_path=$(get_pref "DisplayIconFile")
	[[ -z "${display_icon_file_path}" ]] && display_icon_file_path="${DISPLAY_ICON_DEFAULT_FILE}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_icon_file_path is: ${display_icon_file_path}"
	
	# Validate ${display_icon_light_file_option} input and if valid set the ${display_icon_light_file_path} parameter.  If there is no ${display_icon_light_file_path} then set it to ${display_icon_file_path}.
	display_icon_light_file_path=$(get_pref_managed "DisplayIconLightFile")
	if [[ -z "${display_icon_light_file_path}" ]] && [[ "${display_icon_light_file_option}" == "X" ]]; then
		delete_pref "DisplayIconLightFile"
		delete_pref "DisplayIconLightFileCachedOrigin"
	elif [[ -z "${display_icon_light_file_path}" ]] && [[ -n "${display_icon_light_file_option}" ]]; then
		display_icon_light_file_path="${display_icon_light_file_option}"
		set_pref "DisplayIconLightFile" "${display_icon_light_file_path}"
	fi
	[[ -z "${display_icon_light_file_path}" ]] && display_icon_light_file_path=$(get_pref "DisplayIconLightFile")
	[[ -z "${display_icon_light_file_path}" ]] && display_icon_light_file_path="${display_icon_file_path}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_icon_light_file_path is: ${display_icon_light_file_path}"
	
	# Validate ${display_icon_dark_file_option} input and if valid set the ${display_icon_dark_file_path} parameter.  If there is no ${display_icon_dark_file_path} then set it to ${display_icon_file_path}.
	display_icon_dark_file_path=$(get_pref_managed "DisplayIconDarkFile")
	if [[ -z "${display_icon_dark_file_path}" ]] && [[ "${display_icon_dark_file_option}" == "X" ]]; then
		delete_pref "DisplayIconDarkFile"
		delete_pref "DisplayIconDarkFileCachedOrigin"
	elif [[ -z "${display_icon_dark_file_path}" ]] && [[ -n "${display_icon_dark_file_option}" ]]; then
		display_icon_dark_file_path="${display_icon_dark_file_option}"
		set_pref "DisplayIconDarkFile" "${display_icon_dark_file_path}"
	fi
	[[ -z "${display_icon_dark_file_path}" ]] && display_icon_dark_file_path=$(get_pref "DisplayIconDarkFile")
	[[ -z "${display_icon_dark_file_path}" ]] && display_icon_dark_file_path="${display_icon_file_path}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_icon_dark_file_path is: ${display_icon_dark_file_path}"
	
	# Validate ${display_accessory_type_option} input and if valid set the ${display_accessory_type} parameter. Validating the actual display accessory files themselves are handled later during the workflow.
	display_accessory_type=$(get_pref_managed "DisplayAccessoryType")
	if [[ -z "${display_accessory_type}" ]] && [[ "${display_accessory_type_option}" == "X" ]]; then
		delete_pref "DisplayAccessoryType"
	elif [[ -z "${display_accessory_type}" ]] && [[ -n "${display_accessory_type_option}" ]]; then
		display_accessory_type="${display_accessory_type_option}"
		set_pref "DisplayAccessoryType" "${display_accessory_type}"
	fi
	[[ -z "${display_accessory_type}" ]] && display_accessory_type=$(get_pref "DisplayAccessoryType")
	{ [[ -n "${display_accessory_type}" ]] && [[ ! "${display_accessory_type}" =~ ^TEXTBOX$|^HTMLBOX$|^HTML$|^IMAGE$|^VIDEO$|^VIDEOAUTO$ ]]; } && log_super "Parameter Error: Unrecognized --display-accessory-type value of ${display_accessory_type}. You must specify one of the following; TEXTBOX, HTMLBOX, HTML, IMAGE, VIDEO, or VIDEOAUTO." && parameter_error="TRUE"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_accessory_type}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_type is: ${display_accessory_type}"
	
	# Validate ${display_accessory_default_file_option} input and if valid set the ${display_accessory_default} parameter. Validating the actual display accessory files themselves are handled later during the workflow.
	display_accessory_default=$(get_pref_managed "DisplayAccessoryDefaultFile")
	if [[ -z "${display_accessory_default}" ]] && [[ "${display_accessory_default_file_option}" == "X" ]]; then
		delete_pref "DisplayAccessoryDefaultFile"
	elif [[ -z "${display_accessory_default}" ]] && [[ -n "${display_accessory_default_file_option}" ]]; then
		display_accessory_default="${display_accessory_default_file_option}"
		set_pref "DisplayAccessoryDefaultFile" "${display_accessory_default}"
	fi
	[[ -z "${display_accessory_default}" ]] && display_accessory_default=$(get_pref "DisplayAccessoryDefaultFile")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_accessory_default}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_default is: ${display_accessory_default}"
	
	# Validate ${display_accessory_macos_minor_update_file_option} input and if valid set the ${display_accessory_macos_minor_update} parameter. Validating the actual display accessory files themselves are handled later during the workflow.
	display_accessory_macos_minor_update=$(get_pref_managed "DisplayAccessoryMacOSMinorUpdateFile")
	if [[ -z "${display_accessory_macos_minor_update}" ]] && [[ "${display_accessory_macos_minor_update_file_option}" == "X" ]]; then
		delete_pref "DisplayAccessoryMacOSMinorUpdateFile"
	elif [[ -z "${display_accessory_macos_minor_update}" ]] && [[ -n "${display_accessory_macos_minor_update_file_option}" ]]; then
		display_accessory_macos_minor_update="${display_accessory_macos_minor_update_file_option}"
		set_pref "DisplayAccessoryMacOSMinorUpdateFile" "${display_accessory_macos_minor_update}"
	fi
	[[ -z "${display_accessory_macos_minor_update}" ]] && display_accessory_macos_minor_update=$(get_pref "DisplayAccessoryMacOSMinorUpdateFile")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_accessory_macos_minor_update}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_macos_minor_update is: ${display_accessory_macos_minor_update}"
	
	# Validate ${display_accessory_macos_major_upgrade_file_option} input and if valid set the ${display_accessory_macos_major_upgrade} parameter. Validating the actual display accessory files themselves are handled later during the workflow.
	display_accessory_macos_major_upgrade=$(get_pref_managed "DisplayAccessoryMacOSMajorUpgradeFile")
	if [[ -z "${display_accessory_macos_major_upgrade}" ]] && [[ "${display_accessory_macos_major_upgrade_file_option}" == "X" ]]; then
		delete_pref "DisplayAccessoryMacOSMajorUpgradeFile"
	elif [[ -z "${display_accessory_macos_major_upgrade}" ]] && [[ -n "${display_accessory_macos_major_upgrade_file_option}" ]]; then
		display_accessory_macos_major_upgrade="${display_accessory_macos_major_upgrade_file_option}"
		set_pref "DisplayAccessoryMacOSMajorUpgradeFile" "${display_accessory_macos_major_upgrade}"
	fi
	[[ -z "${display_accessory_macos_major_upgrade}" ]] && display_accessory_macos_major_upgrade=$(get_pref "DisplayAccessoryMacOSMajorUpgradeFile")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_accessory_macos_major_upgrade}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_macos_major_upgrade is: ${display_accessory_macos_major_upgrade}"
	
	# Validate ${display_accessory_non_system_updates_file_option} input and if valid set the ${display_accessory_non_system_updates} parameter. Validating the actual display accessory files themselves are handled later during the workflow.
	display_accessory_non_system_updates=$(get_pref_managed "DisplayAccessoryNonSystemUpdatesFile")
	if [[ -z "${display_accessory_non_system_updates}" ]] && [[ "${display_accessory_non_system_updates_file_option}" == "X" ]]; then
		delete_pref "DisplayAccessoryNonSystemUpdatesFile"
	elif [[ -z "${display_accessory_non_system_updates}" ]] && [[ -n "${display_accessory_non_system_updates_file_option}" ]]; then
		display_accessory_non_system_updates="${display_accessory_non_system_updates_file_option}"
		set_pref "DisplayAccessoryNonSystemUpdatesFile" "${display_accessory_non_system_updates}"
	fi
	[[ -z "${display_accessory_non_system_updates}" ]] && display_accessory_non_system_updates=$(get_pref "DisplayAccessoryNonSystemUpdatesFile")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_accessory_non_system_updates}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_non_system_updates is: ${display_accessory_non_system_updates}"
	
	# Validate ${display_accessory_safari_update_file_option} input and if valid set the ${display_accessory_safari_update} parameter. Validating the actual display accessory files themselves are handled later during the workflow.
	display_accessory_safari_update=$(get_pref_managed "DisplayAccessorySafariUpdateFile")
	if [[ -z "${display_accessory_safari_update}" ]] && [[ "${display_accessory_safari_update_file_option}" == "X" ]]; then
		delete_pref "DisplayAccessorySafariUpdateFile"
	elif [[ -z "${display_accessory_safari_update}" ]] && [[ -n "${display_accessory_safari_update_file_option}" ]]; then
		display_accessory_safari_update="${display_accessory_safari_update_file_option}"
		set_pref "DisplayAccessorySafariUpdateFile" "${display_accessory_safari_update}"
	fi
	[[ -z "${display_accessory_safari_update}" ]] && display_accessory_safari_update=$(get_pref "DisplayAccessorySafariUpdateFile")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_accessory_safari_update}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_safari_update is: ${display_accessory_safari_update}"
	
	# Validate ${display_accessory_jamf_policy_triggers_file_option} input and if valid set the ${display_accessory_jamf_policy_triggers} parameter. Validating the actual display accessory files themselves are handled later during the workflow.
	display_accessory_jamf_policy_triggers=$(get_pref_managed "DisplayAccessoryJamfPolicyTriggersFile")
	if [[ -z "${display_accessory_jamf_policy_triggers}" ]] && [[ "${display_accessory_jamf_policy_triggers_file_option}" == "X" ]]; then
		delete_pref "DisplayAccessoryJamfPolicyTriggersFile"
	elif [[ -z "${display_accessory_jamf_policy_triggers}" ]] && [[ -n "${display_accessory_jamf_policy_triggers_file_option}" ]]; then
	...
	[[ -z "${display_accessory_jamf_policy_triggers}" ]] && display_accessory_jamf_policy_triggers=$(get_pref "DisplayAccessoryJamfPolicyTriggersFile")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_accessory_jamf_policy_triggers}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_jamf_policy_triggers is: ${display_accessory_jamf_policy_triggers}"
	
	# Validate ${display_accessory_restart_without_updates_file_option} input and if valid set the ${display_accessory_restart_without_updates} parameter. Validating the actual display accessory files themselves are handled later during the workflow.
	display_accessory_restart_without_updates=$(get_pref_managed "DisplayAccessoryRestartWithoutUpdatesFile")
	if [[ -z "${display_accessory_restart_without_updates}" ]] && [[ "${display_accessory_restart_without_updates_file_option}" == "X" ]]; then
		delete_pref "DisplayAccessoryRestartWithoutUpdatesFile"
	elif [[ -z "${display_accessory_restart_without_updates}" ]] && [[ -n "${display_accessory_restart_without_updates_file_option}" ]]; then
		display_accessory_restart_without_updates="${display_accessory_restart_without_updates_file_option}"
		set_pref "DisplayAccessoryRestartWithoutUpdatesFile" "${display_accessory_restart_without_updates}"
	fi
	[[ -z "${display_accessory_restart_without_updates}" ]] && display_accessory_restart_without_updates=$(get_pref "DisplayAccessoryRestartWithoutUpdatesFile")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_accessory_restart_without_updates}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_restart_without_updates is: ${display_accessory_restart_without_updates}"
	
	# Validate ${display_accessory_type} and the various ${display_accessory_*_file} parameters in relation to each other.
	{ [[ -n "${display_accessory_type}" ]] && [[ -z "${display_accessory_default}" ]] && [[ -z "${display_accessory_macos_minor_update}" ]] && [[ -z "${display_accessory_macos_major_upgrade}" ]] && [[ -z "${display_accessory_non_system_updates}" ]] && [[ -z "${display_accessory_safari_update}" ]] && [[ -z "${display_accessory_jamf_policy_triggers}" ]] && [[ -z "${display_accessory_restart_without_updates}" ]]; } && log_super "Parameter Error: To use the --display-accessory-type option you must also specify one of the display accessory file options." && parameter_error="TRUE"
	{ [[ -z "${display_accessory_type}" ]] && { [[ -n "${display_accessory_default}" ]] || [[ -n "${display_accessory_macos_minor_update}" ]] || [[ -n "${display_accessory_macos_major_upgrade}" ]] || [[ -n "${display_accessory_non_system_updates}" ]] || [[ -n "${display_accessory_safari_update}" ]] || [[ -n "${display_accessory_jamf_policy_triggers}" ]] || [[ -n "${display_accessory_restart_without_updates}" ]]; }; } && log_super "Parameter Error: To use any of the display accessory file options you must also specify the --display-accessory-type option." && parameter_error="TRUE"
	
	# Validate ${display_help_button_string_option} input and if valid set the ${display_help_button_string} parameter.
	display_help_button_string=$(get_pref_managed "DisplayHelpButtonString")
	if [[ -z "${display_help_button_string}" ]] && [[ "${display_help_button_string_option}" == "X" ]]; then
		delete_pref "DisplayHelpButtonString"
	elif [[ -z "${display_help_button_string}" ]] && [[ -n "${display_help_button_string_option}" ]]; then
		display_help_button_string="${display_help_button_string_option}"
		set_pref "DisplayHelpButtonString" "${display_help_button_string}"
	fi
	[[ -z "${display_help_button_string}" ]] && display_help_button_string=$(get_pref "DisplayHelpButtonString")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_help_button_string}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_help_button_string is: ${display_help_button_string}"
	
	# Validate ${display_warning_button_string_option} input and if valid set the ${display_warning_button_string} parameter.
	display_warning_button_string=$(get_pref_managed "DisplayWarningButtonString")
	if [[ -z "${display_warning_button_string}" ]] && [[ "${display_warning_button_string_option}" == "X" ]]; then
		delete_pref "DisplayWarningButtonString"
	elif [[ -z "${display_warning_button_string}" ]] && [[ -n "${display_warning_button_string_option}" ]]; then
		display_warning_button_string="${display_warning_button_string_option}"
		set_pref "DisplayWarningButtonString" "${display_warning_button_string}"
	fi
	[[ -z "${display_warning_button_string}" ]] && display_warning_button_string=$(get_pref "DisplayWarningButtonString")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_warning_button_string}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_warning_button_string is: ${display_warning_button_string}"
	...
	if [[ "${mac_cpu_architecture}" == "arm64" ]]; then
		# Manage ${auth_ask_user_to_save_password} parameter.
		auth_ask_user_to_save_password=$(get_pref_managed "AuthAskUserToSavePassword")
		{ [[ -z "${auth_ask_user_to_save_password}" ]] && [[ -n "${auth_ask_user_to_save_password_option}" ]] && [[ "${auth_delete_all_option}" != "TRUE" ]]; } && auth_ask_user_to_save_password="${auth_ask_user_to_save_password_option}" && set_pref_main "AuthAskUserToSavePassword" "${auth_ask_user_to_save_password_option}"
		{ [[ -z "${auth_ask_user_to_save_password}" ]] && [[ "${auth_delete_all_option}" != "TRUE" ]]; }  && auth_ask_user_to_save_password=$(get_pref "AuthAskUserToSavePassword")
		{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${auth_ask_user_to_save_password}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_ask_user_to_save_password is: ${auth_ask_user_to_save_password}"
		
		# Identify and log authentication option conflicts.
		{ [[ -n "${auth_local_account_option}" ]] && [[ -z "${auth_local_password_option}" ]]; } && log_super "Auth Error: The --auth-local-account option requires that you also use the --auth-local-password option." && parameter_error="TRUE"
		{ [[ -z "${auth_local_account_option}" ]] && [[ -n "${auth_local_password_option}" ]]; } && log_super "Auth Error: The --auth-local-password option requires that you also set the --auth-local-account option." && parameter_error="TRUE"
		{ [[ -n "${auth_service_add_via_admin_account_option}" ]] && [[ -z "${auth_service_add_via_admin_password_option}" ]]; } && log_super "Auth Error: The --auth-service_add_via_admin-account option requires that you also use the --auth-service_add_via_admin-password option." && parameter_error="TRUE"
		{ [[ -z "${auth_service_add_via_admin_account_option}" ]] && [[ -n "${auth_service_add_via_admin_password_option}" ]]; } && log_super "Auth Error: The --auth-service_add_via_admin-password option requires that you also set the --auth-service_add_via_admin-account option." && parameter_error="TRUE"
		{ [[ -n "${auth_service_account_option}" ]] && { [[ -z "${auth_service_add_via_admin_account_option}" ]] || [[ -z "${auth_service_add_via_admin_password_option}" ]]; }; } && log_super "Auth Error: The --auth-service-account option requires that you also set the ---auth-service-add-via-admin-account and --auth-service-add-via-admin-password options." && parameter_error="TRUE"
		{ [[ -n "${auth_service_password_option}" ]] && { [[ -z "${auth_service_add_via_admin_account_option}" ]] || [[ -z "${auth_service_add_via_admin_password_option}" ]]; }; } && log_super "Auth Error: The --auth-service-password option requires that you also set the ---auth-service-add-via-admin-account and --auth-service-add-via-admin-password options." && parameter_error="TRUE"
		{ [[ -n "${auth_jamf_client_option}" ]] && [[ -z "${auth_jamf_secret_option}" ]]; } && log_super "Auth Error: The --auth-jamf-client option requires that you also set the --auth-jamf-secret option." && parameter_error="TRUE"
		{ [[ -z "${auth_jamf_client_option}" ]] && [[ -n "${auth_jamf_secret_option}" ]]; } && log_super "Auth Error: The --auth-jamf-secret option requires that you also set the --auth-jamf-client." && parameter_error="TRUE"
		{ [[ -n "${auth_jamf_client_option}" ]] && [[ $jamf_version_normalized -lt 104900 ]]; } && log_super "Auth Error: The --auth-jamf-client option requires Jamf Pro version 10.49 or later, the currently installed version of Jamf Pro ${jamf_version_number_full} does not support this option." && parameter_error="TRUE"
		{ [[ -n "${auth_jamf_account_option}" ]] && [[ -z "${auth_jamf_password_option}" ]]; } && log_super "Auth Error: The --auth-jamf-account option requires that you also set the --auth-jamf-password option." && parameter_error="TRUE"
		{ [[ -z "${auth_jamf_account_option}" ]] && [[ -n "${auth_jamf_password_option}" ]]; } && log_super "Auth Error: The --auth-jamf-password option requires that you also set the --auth-jamf-account." && parameter_error="TRUE"
		{ [[ -n "${auth_jamf_account_option}" ]] && [[ $jamf_version_normalized -ge 104900 ]]; } && log_super "Parameter Warning: The --auth-jamf-account option is not recommended for Jamf Pro version 10.49 or later. The recommended implementation is the more secure --auth-jamf-client option."
		
		# Validate ${auth_jamf_custom_url_option} input and if valid set the ${auth_jamf_custom_url} parameter.
		auth_jamf_custom_url=$(get_pref_managed "AuthJamfCustomURL")
		if [[ -z "${auth_jamf_custom_url}" ]] && [[ "${auth_jamf_custom_url_option}" == "X" ]]; then
			delete_pref_main "AuthJamfCustomURL"
		elif [[ -z "${auth_jamf_custom_url}" ]] && [[ -n "${auth_jamf_custom_url_option}" ]]; then
			auth_jamf_custom_url="${auth_jamf_custom_url_option}"
			set_pref_main "AuthJamfCustomURL" "${auth_jamf_custom_url}"
		fi
		[[ -z "${auth_jamf_custom_url}" ]] && auth_jamf_custom_url=$(get_pref "AuthJamfCustomURL")
		if [[ -n "${auth_jamf_custom_url}" ]] && [[ "${auth_jamf_custom_url}" =~ ${REGEX_HTTPS} ]] && [[ ! "${auth_jamf_custom_url}" =~ .*\/$ ]]; then
			auth_jamf_custom_url="${auth_jamf_custom_url}/"
			log_super "Parameter Warning: Adding a trailing slash to the --auth-jamf-custom-url value: ${auth_jamf_custom_url}"
		fi
		{ [[ -n "${auth_jamf_custom_url}" ]] && [[ ! "${auth_jamf_custom_url}" =~ ${REGEX_HTTPS} ]]; } && log_super "Parameter Error: Invalid --auth-jamf-custom-url=URL VALUE must start with 'https://': ${auth_jamf_custom_url}" && parameter_error="TRUE"
		{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${auth_jamf_custom_url}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_custom_url is: ${auth_jamf_custom_url}"
		
		# Manage ${auth_credential_failover_to_user} parameter.
		auth_credential_failover_to_user=$(get_pref_managed "AuthCredentialFailoverToUser")
		{ [[ -z "${auth_credential_failover_to_user}" ]] && [[ -n "${auth_credential_failover_to_user_option}" ]]; } && auth_credential_failover_to_user="${auth_credential_failover_to_user_option}" && set_pref_main "AuthCredentialFailoverToUser" "${auth_credential_failover_to_user_option}"
		[[ -z "${auth_credential_failover_to_user}" ]] && auth_credential_failover_to_user=$(get_pref "AuthCredentialFailoverToUser")
		{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${auth_credential_failover_to_user}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_credential_failover_to_user is: ${auth_credential_failover_to_user}"
		
		# Validate ${auth_mdm_failover_to_user_option} input and if valid set the ${auth_mdm_failover_to_user} parameter.
		auth_mdm_failover_to_user=$(get_pref_managed "AuthMDMFailoverToUser")
		if [[ -z "${auth_mdm_failover_to_user}" ]] && [[ "${auth_mdm_failover_to_user_option}" == "X" ]]; then
			delete_pref_main "AuthMDMFailoverToUser"
		elif [[ -z "${auth_mdm_failover_to_user}" ]] && [[ -n "${auth_mdm_failover_to_user_option}" ]]; then
			auth_mdm_failover_to_user="${auth_mdm_failover_to_user_option}"
			set_pref_main "AuthMDMFailoverToUser" "${auth_mdm_failover_to_user}"
		fi
		[[ -z "${auth_mdm_failover_to_user}" ]] && auth_mdm_failover_to_user=$(get_pref "AuthMDMFailoverToUser")
		if [[ -n "${auth_mdm_failover_to_user}" ]]; then
			previous_ifs="${IFS}"
			IFS=','
			local auth_mdm_failover_to_user_array
			read -r -a auth_mdm_failover_to_user_array <<<"${auth_mdm_failover_to_user}"
			for option_type in "${auth_mdm_failover_to_user_array[@]}"; do
				{ [[ ! "${option_type}" =~ ${REGEX_WORKFLOW_OPTIONS} ]]; } && log_super "Parameter Error: Unrecognized --auth-mdm-failover-to-user type of ${option_type}. You can only specify the following types separated by commas (no spaces): ALWAYS,DIALOG,DEADLINE,SCHEDULED,INSTALLNOW,ERROR" && parameter_error="TRUE"
			done
			IFS="${previous_ifs}"
			[[ $(echo "${auth_mdm_failover_to_user}" | grep -c 'ALWAYS') -gt 0 ]] && auth_mdm_failover_to_user_status="TRUE"
		fi
		{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${auth_mdm_failover_to_user}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_mdm_failover_to_user is: ${auth_mdm_failover_to_user}"
		{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${auth_mdm_failover_to_user_status}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_mdm_failover_to_user_status is: ${auth_mdm_failover_to_user_status}"
	fi
	
	# Manage ${test_mode} parameter.
	test_mode=$(get_pref_managed "TestMode")
	{ [[ -z "${test_mode}" ]] && [[ -n "${test_mode_option}" ]]; } && test_mode="${test_mode_option}" && set_pref "TestMode" "${test_mode_option}"
	[[ -z "${test_mode}" ]] && test_mode=$(get_pref "TestMode")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: test_mode is: ${test_mode}"
	
	# Validate ${test_mode_timeout_option} input and if valid set the ${test_mode_timeout_seconds} parameter. If there is no ${test_mode_timeout_seconds} then set it to ${TEST_MODE_DEFAULT_TIMEOUT}.
	test_mode_timeout_seconds=$(get_pref_managed "TestModeTimeout")
	if [[ -z $test_mode_timeout_seconds ]] && [[ "${test_mode_timeout_option}" == "X" ]]; then
		delete_pref "TestModeTimeout"
	elif [[ -z $test_mode_timeout_seconds ]] && [[ -n "${test_mode_timeout_option}" ]]; then
		test_mode_timeout_seconds="${test_mode_timeout_option}"
		set_pref "TestModeTimeout" "${test_mode_timeout_seconds}"
	fi
	[[ -z $test_mode_timeout_seconds ]] && test_mode_timeout_seconds=$(get_pref "TestModeTimeout")
	if [[ -n $test_mode_timeout_seconds ]] && [[ $test_mode_timeout_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		if [[ $test_mode_timeout_seconds -lt 10 ]]; then
			log_super "Parameter Warning: Specified --test-mode-timeout=seconds value of ${test_mode_timeout_seconds} seconds is too low, rounding up to 10 seconds."
			test_mode_timeout_seconds=10
		elif [[ $test_mode_timeout_seconds -gt 120 ]]; then
			log_super "Parameter Warning: Specified --test-mode-timeout=seconds value of ${test_mode_timeout_seconds} seconds is too high, rounding down to 120 seconds (2 minutes)."
			test_mode_timeout_seconds=120
		fi
	fi
	{ [[ -n $test_mode_timeout_seconds ]] && [[ ! $test_mode_timeout_seconds =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --test-mode-timeout=seconds value must only be a number." && parameter_error="TRUE"
	[[ -z $test_mode_timeout_seconds ]] && test_mode_timeout_seconds="${TEST_MODE_DEFAULT_TIMEOUT}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: test_mode_timeout_seconds is: ${test_mode_timeout_seconds}"
	...
	if [[ -z "${storage_required_update_gb}" ]] && [[ "${test_storage_update_option}" == "X" ]]; then
		delete_pref "TestStorageUpdate"
	elif [[ -z "${storage_required_update_gb}" ]] && [[ -n "${test_storage_update_option}" ]]; then
		storage_required_update_gb="${test_storage_update_option}"
		set_pref "TestStorageUpdate" "${storage_required_update_gb}"
	fi
	...
	if [[ -z "${storage_required_upgrade_gb}" ]] && [[ "${test_storage_upgrade_option}" == "X" ]]; then
		delete_pref "TestStorageUpgrade"
	elif [[ -z "${storage_required_upgrade_gb}" ]] && [[ -n "${test_storage_upgrade_option}" ]]; then
		storage_required_upgrade_gb="${test_storage_upgrade_option}"
		set_pref "TestStorageUpgrade" "${storage_required_upgrade_gb}"
	fi
	...
	if [[ -z "${power_required_battery_percent}" ]] && [[ "${test_battery_level_option}" == "X" ]]; then
		delete_pref "TestBatteryLevel"
	elif [[ -z "${power_required_battery_percent}" ]] && [[ -n "${test_battery_level_option}" ]]; then
		power_required_battery_percent="${test_battery_level_option}"
		set_pref "TestBatteryLevel" "${power_required_battery_percent}"
	fi
	[[ -z "${power_required_battery_percent}" ]] && power_required_battery_percent=$(get_pref "TestBatteryLevel")
	[[ -n "${power_required_battery_percent}" ]]  && log_super "Parameter Warning: Specifying the --test-battery-level option should only be used for testing purposes."
	{ [[ -n "${power_required_battery_percent}" ]] && [[ ! "${power_required_battery_percent}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; } && log_super "Parameter Error: The --test-battery-level=percentage value must only be a number." && parameter_error="TRUE"
	{ [[ -z "${power_required_battery_percent}" ]] && [[ "${mac_cpu_architecture}" == "arm64" ]]; } && power_required_battery_percent="${POWER_REQUIRED_BATTERY_APPLE_SILICON_PERCENT}"
	{ [[ -z "${power_required_battery_percent}" ]] && [[ "${mac_cpu_architecture}" != "arm64" ]]; } && power_required_battery_percent="${POWER_REQUIRED_BATTERY_INTEL_PERCENT}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: power_required_battery_percent is: ${power_required_battery_percent}"
}
...
manage_authentication_options() {
	...
	# Collect any previously saved credential states (including legacy methods) from ${SUPER_MAIN_PLIST}.
	...
	if [[ "${auth_ask_user_to_save_password}" == "TRUE" ]]; then
		if [[ "${current_user_account_name}" != "FALSE" ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_user_password_keychain: ${auth_user_password_keychain}"
	...
	auth_local_account_saved=$(get_pref "AuthLocalAccount")
	auth_service_account_saved=$(get_pref "AuthServiceAccount")
	auth_jamf_client_saved=$(get_pref "AuthJamfClient")
	auth_jamf_account_saved=$(get_pref "AuthJamfAccount")
	local auth_legacy_local_account_saved
	auth_legacy_local_account_saved=$(get_pref "LocalAccount")
	local auth_legacy_super_account_saved
	auth_legacy_super_account_saved=$(get_pref "SuperAccount")
	local auth_legacy_jamf_account_saved
	auth_legacy_jamf_account_saved=$(get_pref "JamfAccount")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_ask_user_to_save_password: ${auth_ask_user_to_save_password}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_user_account_saved: ${auth_user_account_saved}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_account_saved: ${auth_service_account_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client_saved: ${auth_jamf_client_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account_saved: ${auth_jamf_account_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_legacy_local_account_saved: ${auth_legacy_local_account_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_legacy_super_account_saved: ${auth_legacy_super_account_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_legacy_jamf_account_saved: ${auth_legacy_jamf_account_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_account_option: ${auth_local_account_option}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_add_via_admin_account_option: ${auth_service_add_via_admin_account_option}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client_option: ${auth_jamf_client_option}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account_option: ${auth_jamf_account_option}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_delete_all_option: ${auth_delete_all_option}"
	...
	if [[ "${auth_user_account_saved}" == "TRUE" ]] && { [[ "${auth_ask_user_to_save_password}" == "FALSE" ]] || [[ -n "${auth_local_account_option}" ]] || [[ -n "${auth_service_add_via_admin_account_option}" ]] || [[ -n "${auth_jamf_client_option}" ]] || [[ -n "${auth_jamf_account_option}" ]] || [[ "${auth_delete_all_option}" == "TRUE" ]]; }; then
		log_super_audit "Status: Deleting saved credentials for the --auth-ask-user-to-save-password option."
		...
		delete_pref_main "AuthAskUserToSavePassword"
		auth_ask_user_to_save_password="FALSE"
		auth_user_account_saved="FALSE"
	fi
	if [[ "${auth_ask_user_to_save_password}" == "TRUE" ]] || [[ -n "${auth_local_account_option}" ]] || [[ -n "${auth_service_add_via_admin_account_option}" ]] || [[ -n "${auth_jamf_client_option}" ]] || [[ -n "${auth_jamf_account_option}" ]] || [[ "${auth_delete_all_option}" == "TRUE" ]]; then
		if [[ "${auth_local_account_saved}" == "TRUE" ]]; then
			log_super_audit "Status: Deleting saved credentials for the --auth-local-account option."
			...
			delete_pref_main "AuthLocalAccount"
			auth_local_account_saved="FALSE"
		fi
		if [[ "${auth_service_account_saved}" == "TRUE" ]]; then
			log_super_audit "Status: Deleting the super service account and saved credentials."
			...
			delete_pref_main "AuthServiceAccount"
			auth_service_account_saved="FALSE"
		fi
		if [[ "${auth_jamf_client_saved}" == "TRUE" ]]; then
			log_super_audit "Status: Deleting saved credentials for the --auth-jamf-client option."
			...
			delete_pref_main "AuthJamfClient"
			auth_jamf_client_saved="FALSE"
		fi
		if [[ "${auth_jamf_account_saved}" == "TRUE" ]]; then
			log_super_audit "Status: Deleting saved credentials for the --auth-jamf-account option."
			...
			delete_pref_main "AuthJamfAccount"
			auth_jamf_account_saved="FALSE"
		fi
		if [[ -n "${auth_legacy_local_account_saved}" ]]; then
			log_super_audit "Status: Deleting saved credentials for legacy local account."
			delete_pref_main "LocalAccount"
		...
		if [[ -n "${auth_legacy_super_account_saved}" ]]; then
			log_super_audit "Status: Deleting local account and saved credentials for legacy super service account."
			sysadminctl -deleteUser "${auth_legacy_super_account_saved}" >/dev/null 2>&1
			delete_pref_main "SuperAccount"
		...
		if [[ -n "${auth_legacy_jamf_account_saved}" ]]; then
			log_super_audit "Status: Deleting saved credentials for legacy Jamf Pro API account."
			delete_pref_main "JamfAccount"
	...
	if [[ -n "${auth_legacy_local_account_saved}" ]]; then
		log_super_audit "Status: Migrating saved legacy local account credentials..."
	...
	if [[ -n "${auth_legacy_super_account_saved}" ]]; then
		log_super_audit "Status: Migrating saved legacy super service account credentials..."
	...
	if [[ -n "${auth_legacy_jamf_account_saved}" ]]; then
		log_super_audit "Status: Migrating saved legacy Jamf Pro API account credentials..."
	...
	if [[ "${auth_ask_user_to_save_password}" == "TRUE" ]]; then
		...
		if [[ "${auth_error_new}" != "TRUE" ]]; then
			...
			if [[ "${auth_local_account_option}" != "${auth_local_account_keychain}" ]]; then
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_account_option is: ${auth_local_account_option}"
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_account_keychain is: ${auth_local_account_keychain}"
			...
			if [[ "${auth_local_password_option}" != "${auth_local_password_keychain}" ]]; then
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_password_option is: ${auth_local_password_option}"
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_password_keychain is: ${auth_local_password_keychain}"
		...
		# If the saved credentials are valid then update ${SUPER_MAIN_PLIST}.
		if [[ "${auth_error_new}" != "TRUE" ]]; then
			[[ "${auth_legacy_local_migrate}" != "TRUE" ]] && log_super_audit "Status: Saved new credentials for the --auth-local-account option."
			[[ "${auth_legacy_local_migrate}" == "TRUE" ]] && log_super_audit "Status: Saved migrated credentials for the --auth-local-account option."
			set_pref_main "AuthLocalAccount" "TRUE"
		...
		if [[ "${auth_error_new}" != "TRUE" ]]; then
			...
			if [[ "${auth_service_account}" != "${auth_service_account_keychain}" ]]; then
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_account is: ${auth_service_account}"
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_account_keychain is: ${auth_service_account_keychain}"
			...
			if [[ "${auth_service_password}" != "${auth_service_password_keychain}" ]]; then
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_password is: ${auth_service_password}"
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_password_keychain is: ${auth_service_password_keychain}"
		...
		if [[ "${auth_error_new}" != "TRUE" ]] && [[ "${auth_legacy_service_migrate}" != "TRUE" ]]; then # If migrating a legacy sevice account, then a new service account doesn't have to be created.
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: sysadminctl_response is:\n${sysadminctl_response}"
		...
		# If the super service account is valid then update ${SUPER_MAIN_PLIST}.
		if [[ "${auth_error_new}" != "TRUE" ]]; then
			[[ "${auth_legacy_service_migrate}" != "TRUE" ]] && log_super_audit "log_super_audit: Created new super service account."
			[[ "${auth_legacy_service_migrate}" == "TRUE" ]] && log_super_audit "log_super_audit: Validated migrated super service account."
			set_pref_main "AuthServiceAccount" "TRUE"
		...
		if [[ "${auth_error_new}" != "TRUE" ]]; then
			...
			if [[ "${auth_jamf_client_option}" != "${auth_jamf_client_keychain}" ]]; then
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client_option is: ${auth_jamf_client_option}"
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client_keychain is: ${auth_jamf_client_keychain}"
			...
			if [[ "${auth_jamf_secret_option}" != "${auth_jamf_secret_keychain}" ]]; then
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_secret_option is: ${auth_jamf_secret_option}"
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_secret_keychain is: ${auth_jamf_secret_keychain}"
		...
		# If the saved credentials are valid then update ${SUPER_MAIN_PLIST}.
		if [[ "${auth_error_new}" != "TRUE" ]]; then
			log_super_audit "Status: Saved new credentials for the --auth-jamf-client option."
			set_pref_main "AuthJamfClient" "TRUE"
		...
		if [[ "${auth_error_new}" != "TRUE" ]]; then
			...
			if [[ "${auth_jamf_account_option}" != "${auth_jamf_account_keychain}" ]]; then
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account_option is: ${auth_jamf_account_option}"
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account_keychain is: ${auth_jamf_account_keychain}"
			...
			if [[ "${auth_jamf_password_option}" != "${auth_jamf_password_keychain}" ]]; then
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_password_option is: ${auth_jamf_password_option}"
				[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_password_keychain is: ${auth_jamf_password_keychain}"
		...
		# If the saved credentials are valid then update ${SUPER_MAIN_PLIST}.
		if [[ "${auth_error_new}" != "TRUE" ]]; then
			[[ "${auth_legacy_jamf_migrate}" != "TRUE" ]] && log_super_audit "Status: Saved new credentials for the --auth-jamf-account option."
			[[ "${auth_legacy_jamf_migrate}" == "TRUE" ]] && log_super_audit "Status: Saved migrated credentials for the --auth-jamf-account option."
			set_pref_main "AuthJamfAccount" "TRUE"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client_saved: ${auth_jamf_client_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account_saved: ${auth_jamf_account_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_error_new: ${auth_error_new}"
	...
	if [[ "${auth_error_new}" == "FALSE" ]]; then
		...
		if [[ "${auth_legacy_service_migrate}" == "TRUE" ]]; then
			log_super_audit "Status: Deleting saved credentials for legacy super service account."
			delete_pref_main "SuperAccount"
			security delete-generic-password -a "${auth_legacy_super_account_saved}" -s "Super Service Account" /Library/Keychains/System.keychain >/dev/null 2>&1
		fi
...
manage_workflow_options() {
	...
	# If the ${workflow_disable_update_check} is enabled then there is no reason to continue this function.
	[[ "${workflow_disable_update_check}" == "TRUE" ]] && return 0
	...
	if [[ "${auth_error_new}" != "TRUE" ]]; then
		if [[ "${mac_cpu_architecture}" == "i386" ]] || [[ "${workflow_only_download_active}" == "TRUE" ]]; then # All Intel workflows and the only download workflow do not need authentication.
			...
			if [[ "${auth_user_account_saved}" == "TRUE" ]] || [[ "${auth_local_account_saved}" == "TRUE" ]] || [[ "${auth_service_account_saved}" == "TRUE" ]]; then
				...
				if [[ $macos_version_normalized -ge 110300 ]]; then
					[[ -n "${auth_mdm_failover_to_user}" ]] && log_super "Status: macOS update/upgrade workflows automatically authenticated via Jamf Pro API with --auth-mdm-failover-to-user=${auth_mdm_failover_to_user}."
					[[ -z "${auth_mdm_failover_to_user}" ]] && log_super "Status: macOS update/upgrade workflows automatically authenticated via Jamf Pro API with no --auth-mdm-failover-to-user options."
		...
		if [[ "${auth_credential_failover_to_user}" == "TRUE" ]]; then
			if [[ "${current_user_account_name}" != "FALSE" ]] && [[ "${current_user_has_secure_token}" == "TRUE" ]] && [[ "${current_user_is_volume_owner}" == "TRUE" ]]; then
		...
		elif [[ "${auth_jamf_account_saved}" == "TRUE" ]] && { [[ "${auth_mdm_failover_to_user_status}" == "TRUE" ]] || [[ $(echo "${auth_mdm_failover_to_user}" | grep -c 'ERROR') -gt 0 ]] || { [[ "${workflow_install_now_active}" == "TRUE" ]] && [[ $(echo "${auth_mdm_failover_to_user}" | grep -c 'INSTALLNOW') -gt 0 ]]; }; }; then
			if [[ "${current_user_account_name}" != "FALSE" ]] && [[ "${current_user_has_secure_token}" == "TRUE" ]] && [[ "${current_user_is_volume_owner}" == "TRUE" ]]; then
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_auth_error is: ${workflow_auth_error}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_macos_auth is: ${workflow_macos_auth}"
...
get_saved_authentication() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_ask_user_to_save_password: ${auth_ask_user_to_save_password}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_user_saved_password_valid: ${auth_user_saved_password_valid}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_user_account_saved: ${auth_user_account_saved}"
	...
	if [[ "${auth_local_account_saved}" == "TRUE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_account_keychain: ${auth_local_account_keychain}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_password_keychain: ${auth_local_password_keychain}"
	...
	if [[ "${auth_service_account_saved}" == "TRUE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_account_keychain: ${auth_service_account_keychain}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_password_keychain: ${auth_service_password_keychain}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_account_saved: ${auth_service_account_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_service_account_valid: ${auth_service_account_valid}"
	...
	if [[ "${auth_jamf_client_saved}" == "TRUE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client_keychain: ${auth_jamf_client_keychain}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_secret_keychain: ${auth_jamf_secret_keychain}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client_saved: ${auth_jamf_client_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client_valid: ${auth_jamf_client_valid}"
	...
	if [[ "${auth_jamf_account_saved}" == "TRUE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account_keychain: ${auth_jamf_account_keychain}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_password_keychain: ${auth_jamf_password_keychain}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account_saved: ${auth_jamf_account_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account_valid: ${auth_jamf_account_valid}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_error_saved: ${auth_error_saved}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_account: ${auth_local_account}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_password: ${auth_local_password}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client: ${auth_jamf_client}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_secret: ${auth_jamf_secret}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account: ${auth_jamf_account}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_password: ${auth_jamf_password}"
}
...
workflow_installation() {
	...
	[[ ! -d "${SUPER_CONFIGS_FOLDER}" ]] && mkdir -p "${SUPER_CONFIGS_FOLDER}"
...
# This script checks a variety of settings to ensure that the super workflow only restarts when necessary.
# Version ${SUPER_VERSION}
# ${SUPER_DATE}
...
if [[ "\$(/usr/libexec/PlistBuddy -c "Print :WorkflowRestartValidate" "${SUPER_MAIN_PLIST}.plist" 2> /dev/null)" == "true" ]]; then
	mac_last_startup_saved_epoch=\$(date -j -f %Y-%m-%d:%H:%M:%S "\$(/usr/libexec/PlistBuddy -c "Print :MacLastStartup" "${SUPER_MAIN_PLIST}.plist" 2> /dev/null)" +%s 2> /dev/null)
...
EOSS
	...
	/bin/cat <<EOLD >"/Library/LaunchDaemons/${SUPER_LAUNCH_DAEMON_LABEL}.plist"
...
EOLD
	...
	chown -R root:wheel "${SUPER_FOLDER}" >/dev/null 2>&1
	chmod -R a+r "${SUPER_FOLDER}" >/dev/null 2>&1
	chmod -R go-w "${SUPER_FOLDER}" >/dev/null 2>&1
	chmod a+x "${SUPER_FOLDER}/super" >/dev/null 2>&1
	chmod a+x "${SUPER_FOLDER}/super-starter" >/dev/null 2>&1
	chown root:wheel "${SUPER_LINK}" >/dev/null 2>&1
	chmod a+rx "${SUPER_LINK}" >/dev/null 2>&1
	chmod go-w "${SUPER_LINK}" >/dev/null 2>&1
	chmod 644 "/Library/LaunchDaemons/${SUPER_LAUNCH_DAEMON_LABEL}.plist" >/dev/null 2>&1
	chown root:wheel "/Library/LaunchDaemons/${SUPER_LAUNCH_DAEMON_LABEL}.plist" >/dev/null 2>&1
	set_pref_main "SuperVersion" "${SUPER_VERSION}" >/dev/null 2>&1
}
...
get_ibm_notifier() {
	...
	download_response=$(curl --user-agent "${SUPER_USER_AGENT}" --location "${IBM_NOTIFIER_DOWNLOAD_URL}" --output "${temp_file}" 2>&1)
	if [[ -f "${temp_file}" ]]; then
		...
		if [[ -d "${IBM_NOTIFIER_APP}" ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: unzip_response is:\n${unzip_response}"
		fi
	else
		log_super "Error: Unable to download IBM Notifier.app from: ${IBM_NOTIFIER_DOWNLOAD_URL}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: download_response is:\n${download_response}"
	fi
...
check_ibm_notifier() {
	...
	if [[ $(echo "${codesign_response}" | grep -c 'valid on disk') -gt 0 ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: codesign_response is:\n${codesign_response}"
	fi
}
...
get_display_icon() {
	...
	if [[ "${get_display_icon_path}" =~ ${REGEX_HTML_URL} ]]; then
		...
		download_response=$(curl --user-agent "${SUPER_USER_AGENT}" --location "${get_display_icon_path}" --output "${get_display_icon_temp_file}" 2>&1)
		...
		download_response=$(cp "${get_display_icon_path}" "${get_display_icon_temp_file}" 2>&1)
		if [[ ! -f "${get_display_icon_temp_file}" ]]; then
	...
	if [[ "${get_display_icon_error}" == "FALSE" ]]; then
		...
		if [[ ! -f "${get_display_icon_temp_file}.png" ]] || [[ $(echo "${sips_response}" | grep -c 'Warning') -gt 0 ]] || [[ $(echo "${sips_response}" | grep -c 'Error') -gt 0 ]]; then
			...
			get_display_icon_error="TRUE"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: sips_response is:\n${sips_response}"
		fi
	fi
	rm -Rf "${get_display_icon_temp_file}" >/dev/null 2>&1
	umask "${previous_umask}"
	unset get_display_icon_path
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: get_display_icon_error is: ${get_display_icon_error}"
}
...
get_mist_cli() {
	...
	local download_response
	download_response=$(curl --user-agent "${SUPER_USER_AGENT}" --location "${MIST_CLI_DOWNLOAD_URL}" --output "${temp_file}" 2>&1)
	if [[ -f "${temp_file}" ]]; then
		...
		if ! { [[ $(echo "${install_response}" | grep -c 'The software was successfully installed.') -gt 0 ]] || [[ $(echo "${install_response}" | grep -c 'The install was successful.') -gt 0 ]]; }; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_response is:\n${install_response}"
		fi
	else
...
manage_helpers() {
	...
	if [[ -n "${install_macos_minor_version_target}" ]] || [[ "${install_macos_major_upgrades}" == "TRUE" ]];then
	...
	# If user is only making alternate configuration changes then this function should exit.
	{ [[ "${config_edit_main_option}" == "TRUE" ]] || [[ -n "${config_edit_option}" ]] || [[ -n "${config_delete_option}" ]] || [[ "${config_delete_all_option}" == "TRUE" ]]; } && return 0
	
	# Set the ${display_icon_light} using either the ${DISPLAY_ICON_DEFAULT_FILE} or the ${display_icon_light_file_path} option.
	local display_icon_light_file_cached_origin
	display_icon_light_file_cached_origin=$(get_pref "DisplayIconLightFileCachedOrigin")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_icon_light_file_cached_origin is: ${display_icon_light_file_cached_origin}"
	if [[ "${display_icon_light_file_path}" == "${DISPLAY_ICON_DEFAULT_FILE}" ]]; then # Using the ${DISPLAY_ICON_DEFAULT_FILE}.
		display_icon_light="${DISPLAY_ICON_DEFAULT_FILE}"
	elif [[ "${display_icon_light_file_path}" == "${display_icon_light_file_cached_origin}" ]] && [[ -f "${DISPLAY_ICON_LIGHT_FILE_CACHE}" ]]; then # Cached copy of the ${display_icon_light_file_path} is intact.
		display_icon_light="${DISPLAY_ICON_LIGHT_FILE_CACHE}"
	else # Need to acquire a new copy of the ${display_icon_light_file_path}.
		[[ -f "${DISPLAY_ICON_LIGHT_FILE_CACHE}" ]] && rm -f "${DISPLAY_ICON_LIGHT_FILE_CACHE}" 2>/dev/null
		delete_pref_main "DisplayIconLightFileCachedOrigin"
		...
		if [[ "${get_display_icon_error}" == "FALSE" ]]; then
			...
			set_pref_main "DisplayIconLightFileCachedOrigin" "${display_icon_light_file_path}"
		else
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_icon_light is: ${display_icon_light}"
	
	# Set the ${display_icon_dark} using either the ${DISPLAY_ICON_DEFAULT_FILE} or the ${display_icon_dark_file_path} option.
	local display_icon_dark_file_cached_origin
	display_icon_dark_file_cached_origin=$(get_pref "DisplayIconDarkFileCachedOrigin")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_icon_dark_file_cached_origin is: ${display_icon_dark_file_cached_origin}"
	if [[ "${display_icon_dark_file_path}" == "${DISPLAY_ICON_DEFAULT_FILE}" ]]; then # Using the ${DISPLAY_ICON_DEFAULT_FILE}.
		display_icon_dark="${DISPLAY_ICON_DEFAULT_FILE}"
	elif [[ "${display_icon_dark_file_path}" == "${display_icon_dark_file_cached_origin}" ]] && [[ -f "${DISPLAY_ICON_DARK_FILE_CACHE}" ]]; then  # Cached copy of the ${display_icon_dark_file_path} is intact.
		display_icon_dark="${DISPLAY_ICON_DARK_FILE_CACHE}"
	else # Need to acquire a new copy of the ${display_icon_dark_file_path}.
		[[ -f "${DISPLAY_ICON_DARK_FILE_CACHE}" ]] && rm -f "${DISPLAY_ICON_DARK_FILE_CACHE}" 2>/dev/null
		delete_pref_main "DisplayIconDarkFileCachedOrigin"
		...
		if [[ "${get_display_icon_error}" == "FALSE" ]]; then
			...
			rm -Rf "${get_display_icon_temp_file}.png" >/dev/null 2>&1
			unset get_display_icon_temp_file
			set_pref_main "DisplayIconDarkFileCachedOrigin" "${display_icon_dark_file_path}"
		else
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_icon_dark is: ${display_icon_dark}"
}
...
workflow_startup() {
	...
	# Start the ${config_status_option} function. This function will exit the workflow when complete.
	...
	# If using an alternate configuration mode then save a ${super_status_backup} otherwise delete the NextAutoLaunch preference in case the workflow crashes or the system restarts unexpectedly before super exits. A "missing" NextAutoLaunch preference will cause super to automatically restart via the LaunchDaemon.
	if [[ "${config_edit_main_option}" == "TRUE" ]] || [[ -n "${config_edit_option}" ]] || [[ -n "${config_delete_option}" ]] || [[ "${config_delete_all_option}" == "TRUE" ]]; then
		local super_status_backup
		super_status_backup=$(get_pref "SuperStatus")
	else
	...
	# Manage the ${verbose_mode} parameter and if enabled start additional logging.
	[[ "${reset_super_option}" == "TRUE" ]] && delete_pref_main "VerboseMode"
	verbose_mode=$(get_pref_managed "VerboseMode")
	{ [[ -z "${verbose_mode}" ]] && [[ -n "${verbose_mode_option}" ]]; } && verbose_mode="${verbose_mode_option}" && set_pref_main "VerboseMode" "${verbose_mode_option}"
	[[ -z "${verbose_mode}" ]] && verbose_mode=$(get_pref "VerboseMode")
	...
	# Initial system and user checks.
	check_system
	check_current_user
	
	# Workflow for ${open_logs_option}.
	if [[ "${open_logs_option}" == "TRUE" ]]; then
		if [[ "${current_user_account_name}" != "FALSE" ]]; then
			...
			touch "${INSTALLER_WORKFLOW_LOG}" "${MSU_WORKFLOW_LOG}" "${MACOS_INSTALLERS_LIST_LOG}" "${MDMCLIENT_LIST_LOG}" "${MSU_LIST_LOG}" "${SUPER_AUDIT_LOG}" "${SUPER_LOG}"
			...
			sudo -u "${current_user_account_name}" open "${SUPER_AUDIT_LOG}"
	...
	# Configuration management and helper validations, if any of these fail then it's unsafe for the workflow to continue.
	if [[ "${check_system_error}" == "FALSE" ]]; then
		check_jamf_management_framework
		manage_preference_resets
		manage_parameter_options
	fi
	{ [[ "${check_system_error}" == "FALSE" ]] && [[ "${parameter_error}" == "FALSE" ]]; } && manage_helpers
	if [[ "${check_system_error}" == "TRUE" ]] || [[ "${parameter_error}" == "TRUE" ]] || [[ "${helper_error}" == "TRUE" ]]; then
	...
	if [[ $(get_pref "WorkflowRestartValidate") == "TRUE" ]]; then
	...
	elif [[ "${workflow_install_now}" == "TRUE" ]]; then
		log_super "Status: Install now workflow active."
		workflow_install_now_active="TRUE"
		[[ $(echo "${display_unmovable}" | grep -c 'INSTALLNOW') -gt 0 ]] && display_unmovable_status="TRUE"
		[[ $(echo "${display_hide_background}" | grep -c 'INSTALLNOW') -gt 0 ]] && display_hide_background_status="TRUE"
		[[ $(echo "${display_silently}" | grep -c 'INSTALLNOW') -gt 0 ]] && display_silently_status="TRUE"
		[[ $(echo "${display_notifications_centered}" | grep -c 'INSTALLNOW') -gt 0 ]] && display_notifications_centered_status="TRUE"
		[[ $(echo "${display_notifications_centered}" | grep -c 'INSTALLNOW') -gt 0 ]] && display_notifications_centered_status="TRUE"
		if [[ "${current_user_account_name}" != "FALSE" ]]; then
		...
		if [[ $(get_pref "WorkflowScheduledInstall") == "TRUE" ]]; then
			log_super "Warning: Removing previously scheduled installation workflow due to current install now workflow."
			delete_pref_main "WorkflowScheduledInstall"
		fi
	elif [[ $(get_pref "WorkflowScheduledInstall") =~ ${REGEX_DATE_HOURS_MINUTES} ]]; then
		workflow_scheduled_install=$(get_pref "WorkflowScheduledInstall")
	...
	elif [[ "${workflow_only_download}" == "TRUE" ]]; then
		log_super "Status: Only download workflow active."
		workflow_only_download_active="TRUE"
	fi
	{ [[ "${workflow_only_download_active}" != "TRUE" ]] && [[ -n "${install_jamf_policy_triggers}" ]]; } && log_super "Status: Jamf Pro Policy triggers: ${install_jamf_policy_triggers}"
	[[ "${install_jamf_policy_triggers_without_restarting}" == "TRUE" ]] && log_super "Warning: Install Jamf Pro Policy Triggers without restarting is enabled, this computer will run Jamf Pro Policies even if there is no macOS update or upgrade available."
	[[ "${workflow_restart_without_updates}" == "TRUE" ]] && log_super "Warning: Restart without updates workflow is enabled, this computer will restart even if there is no macOS update or upgrade available."
	workflow_reset_super_after_completion_active="FALSE"
	if [[ "${workflow_reset_super_after_completion_option}" == "FALSE" ]]; then
		delete_pref "WorkflowResetSuperAfterCompletion"
		delete_pref_main "WorkflowResetSuperAfterCompletionNow"
	elif [[ "${workflow_reset_super_after_completion_option}" == "TRUE" ]] || [[ $(get_pref "WorkflowResetSuperAfterCompletion") == "TRUE" ]]; then
		...
		set_pref "WorkflowResetSuperAfterCompletion" "TRUE"
	fi
	[[ "${workflow_restart_validate_active}" != "TRUE" ]] && set_pref_main "MacLastStartup" "${mac_last_startup}"
	...
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${super_alternate_plist}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Alternate preference file after startup validation: ${super_alternate_plist}:\n$(defaults read "${super_alternate_plist}" 2>/dev/null)"
	
	# If the macOS Setup Assistant is incomplete, or the ${schedule_deferred_start_file} is not found, or the admin is only making alternate configuration changes then the workflow should automatically defer or exit.
	if [[ ! -f "${APPLE_SETUP_DONE_FILE}" ]] || { [[ -n "${schedule_deferred_start_file}" ]] && [[ ! -f "${schedule_deferred_start_file}" ]]; } || [[ "${config_edit_main_option}" == "TRUE" ]] || [[ -n "${config_edit_option}" ]] || [[ -n "${config_delete_option}" ]] || [[ "${config_delete_all_option}" == "TRUE" ]]; then
		{ [[ "${config_delete_reset_workflow}" == "TRUE" ]] && [[ "${verbose_mode}" == "TRUE" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Calling reset_workflow() function."
		[[ "${config_delete_reset_workflow}" == "TRUE" ]] && reset_workflow
		local next_auto_launch
		if [[ "${workflow_disable_relaunch}" == "TRUE" ]]; then
			/usr/libexec/PlistBuddy -c "Delete :NextAutoLaunch" "${SUPER_MAIN_PLIST}.plist" 2> /dev/null
			/usr/libexec/PlistBuddy -c "Add :NextAutoLaunch string FALSE" "${SUPER_MAIN_PLIST}.plist" 2> /dev/null
			next_auto_launch="FALSE"
		else
			next_auto_launch=$(/usr/libexec/PlistBuddy -c "Print :NextAutoLaunch" "${SUPER_MAIN_PLIST}.plist" 2> /dev/null)
		fi
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: next_auto_launch: ${next_auto_launch}"
		if [[ $(launchctl print "system/${SUPER_LAUNCH_DAEMON_LABEL}" 2>&1 | grep -c "system/${SUPER_LAUNCH_DAEMON_LABEL}") -gt 0 ]] && [[ "${next_auto_launch}" != "FALSE" ]]; then
			if [[ ! -f "${APPLE_SETUP_DONE_FILE}" ]] || { [[ -n "${schedule_deferred_start_file}" ]] && [[ ! -f "${schedule_deferred_start_file}" ]]; } || [[ "${config_delete_reset_workflow}" == "TRUE" ]]; then
				if [[ ! -f "${APPLE_SETUP_DONE_FILE}" ]]; then
					log_super "Exit: macOS Setup Assistant is not done. The super workflow is scheduled to automatically relaunch in ${deferral_timer_minutes} minutes."
					log_status "Pending: macOS Setup Assistant is not done. The super workflow is scheduled to automatically relaunch in ${deferral_timer_minutes} minutes."
				elif [[ -n "${schedule_deferred_start_file}" ]] && [[ ! -f "${schedule_deferred_start_file}" ]]; then
					log_super "Status: The following schedule deferred start file was not found on the system ${schedule_deferred_start_file}."
					log_super "Exit: Schedule deferred start file not found. The super workflow is scheduled to automatically relaunch in ${deferral_timer_minutes} minutes."
					log_status "Pending: Schedule deferred start file not found. The super workflow is scheduled to automatically relaunch in ${deferral_timer_minutes} minutes."
				else
					log_super "Exit: Configuration mode complete. The super workflow is scheduled to automatically relaunch in ${deferral_timer_minutes} minutes."
					log_status "Pending: Configuration mode complete. The super workflow is scheduled to automatically relaunch in ${deferral_timer_minutes} minutes."
				fi
				set_auto_launch_deferral
			else
				if [[ -n "${next_auto_launch}" ]]; then
					log_super "Exit: Configuration mode complete. The super workflow is scheduled to automatically relaunch at ${next_auto_launch}."
					[[ -n "${super_status_backup}" ]] && set_pref_main "SuperStatus" "${super_status_backup}"
					[[ -z "${super_status_backup}" ]] && log_status "Pending: Configuration mode complete. The super workflow is scheduled to automatically relaunch at ${next_auto_launch}."
				else
					log_super "Exit: Configuration mode complete. The super workflow is scheduled to automatically relaunch in ${deferral_timer_minutes} minutes."
					log_status "Pending: Configuration mode complete. The super workflow is scheduled to automatically relaunch in ${deferral_timer_minutes} minutes."
					set_auto_launch_deferral
			...
			if [[ ! -f "${APPLE_SETUP_DONE_FILE}" ]]; then
				log_super "Exit: macOS Setup Assistant is not done. The super workflow is not scheduled to automatically relaunch. You must run super again to start a workflow."
				log_status "Inactive: macOS Setup Assistant is not done. The super workflow is not scheduled to automatically relaunch. You must run super again to start a workflow."
			elif [[ -n "${schedule_deferred_start_file}" ]] && [[ ! -f "${schedule_deferred_start_file}" ]]; then
				log_super "Status: The following schedule deferred start file was not found on the system ${schedule_deferred_start_file}."
				log_super "Exit: Schedule deferred start file not found. The super workflow is not scheduled to automatically relaunch. You must run super again to start a workflow."
				log_status "Inactive: Schedule deferred start file not found. The super workflow is not scheduled to automatically relaunch. You must run super again to start a workflow."
			else
				log_super "Exit: Configuration mode complete. The super workflow is not scheduled to automatically relaunch. You must run super again to start a workflow."
				log_status "Inactive: Configuration mode complete. The super workflow is not scheduled to automatically relaunch. You must run super again to start a workflow."
			fi
		fi
		exit_clean
	fi
...
restart_super() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Main preference file at restart exit: ${SUPER_MAIN_PLIST}:\n$(defaults read "${SUPER_MAIN_PLIST}" 2>/dev/null)"
	log_super "**** S.U.P.E.R.M.A.N. ${SUPER_VERSION} - EXIT AND RESTART WORKFLOW ****"
	rm -f "${SUPER_PID_FILE}" 2>/dev/null
	exit 0
}

# Configure super to automatically launch ${deferral_timer_minutes} from now by setting the NextAutoLaunch attribute in the ${SUPER_MAIN_PLIST}.
set_auto_launch_deferral() {
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_minutes is: ${deferral_timer_minutes}"
	local deferral_timer_epoch
	deferral_timer_epoch=$(($(date +%s) + (deferral_timer_minutes * 60)))
	local next_auto_launch
	next_auto_launch="$(date -j -f %s "${deferral_timer_epoch}" +%Y-%m-%d:%H:%M:00 | xargs)"
	/usr/libexec/PlistBuddy -c "Add :NextAutoLaunch string ${next_auto_launch}" "${SUPER_MAIN_PLIST}.plist" 2> /dev/null
	launchctl bootstrap system "/Library/LaunchDaemons/${SUPER_LAUNCH_DAEMON_LABEL}.plist" >/dev/null 2>&1
...
# Append input to the command line and logs located at ${SUPER_LOG} and ${SUPER_AUDIT_LOG}.
log_super_audit() {
	echo -e "$(date +"%a %b %d %T") $(hostname -s) $(basename "$0")[$$]: $*" | tee -a "${SUPER_LOG}" "${SUPER_AUDIT_LOG}" 
}
...
# Update the SuperStatus key in the ${SUPER_MAIN_PLIST}.
log_status() {
	set_pref_main "SuperStatus" "$(date +"%a %b %d %T"): $*"
}
...
archive_logs() {
	...
	[[ $(ls -l "${SUPER_AUDIT_LOG}" 2>/dev/null | awk '{print int($5/1000)}') -gt $SUPER_LOG_ARCHIVE_SIZE ]] && archive_logs_needed="TRUE"
	...
	if [[ "${archive_logs_needed}" == "TRUE" ]]; then
		...
		[[ -f "${SUPER_AUDIT_LOG}" ]] && mv "${SUPER_AUDIT_LOG}" "${SUPER_LOG_ARCHIVE_FOLDER}/${log_archive_name}/$(basename ${SUPER_AUDIT_LOG})"
...
check_current_user() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_account_name is: ${current_user_account_name}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_id is: ${current_user_id}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_account_name_response is: ${current_user_account_name_response}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_id_response is: ${current_user_id_response}"
	...
	if [[ "${current_user_account_name}" != "FALSE" ]] && [[ "${current_user_id}" != "FALSE" ]] && [[ -d "${SUPER_LOG_FOLDER}" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_guid is: ${current_user_guid}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_real_name is: ${current_user_real_name}"
		...
		if [[ -n "${current_user_id}" ]] && [[ -n "${current_user_guid}" ]] && [[ -n "${current_user_real_name}" ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_appearance_mode_response is: ${current_user_appearance_mode_response}"
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_is_admin is: ${current_user_is_admin}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_has_secure_token is: ${current_user_has_secure_token}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_is_volume_owner is: ${current_user_is_volume_owner}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_appearance_mode is: ${current_user_appearance_mode}"
		else
...
# Collect parameters for detailed system information setting a variety of parameters including ${macos_version_minor}, ${macos_version_patch}, ${macos_version_normalized}, ${macos_version_extra}, ${macos_build}, ${macos_title}, ${macos_version_full}, ${mac_cpu_architecture}, ${mac_model_name}, ${mac_is_portable}, and ${mac_last_startup}.
check_system() {
	check_system_error="FALSE"
	...
	macos_version_normalized=$(sw_vers -productVersion | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') # Expected output: 100502, 140701, 260100
	...
	if [[ "${mac_cpu_architecture}" == "arm64" ]]; then # Mac computers with Apple Silicon.
		[[ "${config_status_option}" != "TRUE" ]] && log_super "Status: Mac computer with Apple silicon running: ${macos_version_full}"
		[[ "${config_status_option}" == "TRUE" ]] && echo "Config Status: Mac computer with Apple silicon running: ${macos_version_full}"
	else # Mac computers with Intel.
		[[ "${config_status_option}" != "TRUE" ]] && log_super "Status: Mac computer with Intel running: ${macos_version_full}"
		[[ "${config_status_option}" == "TRUE" ]] && echo "Config Status: Mac computer with Intel running: ${macos_version_full}"
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_version_normalized is: ${macos_version_normalized}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mac_model_name is: ${mac_model_name}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mac_is_portable is: ${mac_is_portable}"
	...
	if [[ "${kernel_boot_epoch}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		mac_last_startup=$(date -r "${kernel_boot_epoch}" +%Y-%m-%d:%H:%M:%S) # Expected output: 2023-08-25:13:25:45
	else
	...
	if [[ "${mac_last_startup}" =~ ${REGEX_DATE_HOURS_MINUTES_SECONDS} ]]; then
		[[ "${config_status_option}" != "TRUE" ]] && log_super "Status: Last macOS startup was: ${mac_last_startup}"
		[[ "${config_status_option}" == "TRUE" ]] && echo "Config Status: Last macOS startup was: ${mac_last_startup}"
	else
		log_super "Error: Unrecognized last startup date and time: ${mac_last_startup}"
		check_system_error="TRUE"
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: check_system_error is: ${check_system_error}"
}
...
check_msu_settings() {
	...
	if [[ "${msu_automatic_check}" == "TRUE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_download_managed is: ${msu_automatic_download_managed}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_download_local is: ${msu_automatic_download_local}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_download is: ${msu_automatic_download}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_macos_installation_managed is: ${msu_automatic_macos_installation_managed}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_macos_installation_local is: ${msu_automatic_macos_installation_local}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_macos_installation is: ${msu_automatic_macos_installation}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_config_data_managed is: ${msu_automatic_config_data_managed}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_config_data_local is: ${msu_automatic_config_data_local}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_critical_update_managed is: ${msu_automatic_critical_update_managed}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_critical_update_local is: ${msu_automatic_critical_update_local}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_automatic_security_updates is: ${msu_automatic_security_updates}"
...
check_storage_available() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: check_storage_available_error: ${check_storage_available_error}"
...
check_power_required() {
	...
	power_ready="FALSE"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mac_is_portable: ${mac_is_portable}"
	if [[ "${mac_is_portable}" == "TRUE" ]]; then
		[[ $(pmset -g ps | grep -ic 'AC Power') -ne 0 ]] && power_required_charger_connected="TRUE"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: power_required_charger_connected: ${power_required_charger_connected}"
		if [[ "${power_required_charger_connected}" == "TRUE" ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: power_battery_percent: ${power_battery_percent}"
			if [[ -z "${power_battery_percent}" ]] || [[ ! "${power_battery_percent}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
				...
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: power_required_battery_percent: ${power_required_battery_percent}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: check_power_required_error: ${check_power_required_error}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: power_ready: ${power_ready}"
}
...
check_mdm_service() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: profiles_response:\n${profiles_response}"
	if [[ $(echo "${profiles_response}" | grep -c 'MDM server') -gt 0 ]]; then
		...
		local curl_response
		curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --head --location "${mdm_service_url}" | head -n 1)
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdm_enrolled: ${mdm_enrolled}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdm_automatically_enrolled: ${mdm_automatically_enrolled}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdm_service_url: ${mdm_service_url}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_error_mdm: ${auth_error_mdm}"
}
...
check_bootstrap_token_escrow() {
	...
	if [[ $(echo "${profiles_response}" | grep -c 'YES') -eq 2 ]]; then
		if [[ $macos_version_normalized -ge 130300 ]]; then
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_error_bootstrap_token: ${auth_error_bootstrap_token}"
}

# MARK: *** Schedules, Deferrals, & Deadlines ***
################################################################################

# Delete the deadline counters in ${SUPER_MAIN_PLIST} to reset the counters.
...
# Delete any automatic zero dates and associated cached dates in ${SUPER_MAIN_PLIST}.
reset_schedule_zero_date() {
	log_super "Status: Resetting any workflow dates."
	delete_pref_main "ScheduleZeroDateAutomaticRelease"
	delete_pref_main "ScheduleZeroDateAutomaticStart"
	delete_pref_main "DeadlineDaysFocusDate"
	delete_pref_main "DeadlineDaysSoftDate"
	delete_pref_main "DeadlineDaysHardDate"
}

# Delete all workflow items, counters, and zero dates in ${SUPER_MAIN_PLIST} to reset the workflow target.
reset_workflow() {
	log_super "Status: Resetting workflow target."
...
# Evaluate ${schedule_workflow_active} and set ${workflow_time_weekday}, ${schedule_workflow_active_array}, and ${schedule_workflow_active_deferral} accordingly.
...
check_schedule_workflow_active() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_workflow_active is: ${schedule_workflow_active}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_time_weekday is: ${workflow_time_weekday}"
	
	# First check to see if workflow is allowed for current weekday and time.
	local previous_ifs
	previous_ifs="${IFS}"
	IFS=','
	[[ -z "${schedule_workflow_active_array[*]}" ]] && read -r -a schedule_workflow_active_array <<<"${schedule_workflow_active}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_workflow_active_current_end_epoch is: ${schedule_workflow_active_current_end_epoch}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_workflow_active_current_end is: ${schedule_workflow_active_current_end}"
	...
	if [[ "${schedule_workflow_active_deferral}" == "TRUE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_workflow_active_next_start_epoch is: ${schedule_workflow_active_next_start_epoch}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_workflow_active_next_start is: ${schedule_workflow_active_next_start}"
	fi
	IFS="${previous_ifs}"
}

# Adjust ${deferral_timer_minutes_schedule_workflow_active_check}, ${deadline_epoch_schedule_workflow_active_check}, or ${workflow_scheduled_install} to coordinate with the ${schedule_workflow_active}.
...
set_schedule_workflow_active_adjustments() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_time_epoch: ${workflow_time_epoch}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_time_weekday: ${workflow_time_weekday}"
	
	# If there is a ${deferral_timer_minutes_schedule_workflow_active_check} or ${deferral_timer_menu_schedule_workflow_active_check} then adjust it to coordinate with the ${schedule_workflow_active} and set ${deferral_timer_minutes_schedule_workflow_active_adjusted} accordingly.
	if [[ -n "${deferral_timer_minutes_schedule_workflow_active_check}" ]] || [[ -n "${deferral_timer_menu_schedule_workflow_active_check}" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_epoch_temp: ${deferral_timer_epoch_temp}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_days_away: ${deferral_timer_days_away}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_match: ${deferral_timer_match}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_workflow_active_matches_now: ${schedule_workflow_active_matches_now}"
		...
		if [[ "${deferral_timer_match}" == "TRUE" ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_epoch: ${deadline_epoch}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_minutes_schedule_workflow_active_adjusted: ${deferral_timer_minutes_schedule_workflow_active_adjusted}"
		IFS="${previous_ifs}"
		return 0
	fi
	
	# If there is a ${deadline_epoch_schedule_workflow_active_check} then adjust it to coordinate with the ${schedule_workflow_active} and set ${deadline_epoch_schedule_workflow_active_adjusted} accordingly.
	if [[ -n "${deadline_epoch_schedule_workflow_active_check}" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_epoch_temp: ${deadline_epoch_temp}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_away: ${deadline_days_away}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_epoch_schedule_workflow_active_adjusted: ${deadline_epoch_schedule_workflow_active_adjusted}"
		IFS="${previous_ifs}"
		return 0
	fi
	
	# If there is a ${workflow_scheduled_install} via the ${scheduled_install_days} and ${scheduled_install_date} options then compare to the ${schedule_workflow_active} for possible adjustments.
	if [[ -n "${scheduled_install_days}" ]] || [[ -n "${scheduled_install_date}" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_scheduled_install_adjusted: ${workflow_scheduled_install_adjusted}"
	...
	# If there is a ${workflow_scheduled_install} via the ${scheduled_install_user_choice} then compare it to the ${schedule_workflow_active} for possible adjustment.
	if [[ "${scheduled_install_user_choice}" == "TRUE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_user_choice_adjusted: ${scheduled_install_user_choice_adjusted}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_scheduled_install_adjusted: ${workflow_scheduled_install_adjusted}"
	fi
	IFS="${previous_ifs}"
}
...
get_sofa_macos_json() {
	...
	sofa_macos_json_checksum_cache=$(get_pref "SOFAMacOSJsonCacheChecksum")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: sofa_macos_json_checksum_cache is: ${sofa_macos_json_checksum_cache}"
	if [[ -n "${sofa_macos_json_checksum_cache}" ]]; then
		if [[ -e "${SOFA_MACOS_JSON_ETAG_CACHE}" ]] && [[ "${sofa_macos_json_checksum_cache}" == "$(md5 -q "${SOFA_MACOS_JSON_CACHE}" 2>/dev/null)" ]]; then
			...
			delete_pref_main "SOFAMacOSJsonCacheChecksum"
			rm -f "${SOFA_MACOS_JSON_ETAG_CACHE}" 2>/dev/null
			rm -f "${SOFA_MACOS_JSON_CACHE}" 2>/dev/null
		fi
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: sofa_macos_cache is: ${sofa_macos_cache}"
	...
	if [[ "${sofa_macos_cache}" == "TRUE" ]]; then
		...
		curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --write-out "%{http_code}" --max-time 5 --etag-compare "${SOFA_MACOS_JSON_ETAG_CACHE}" --location "${sofa_macos_url}" --etag-save "${SOFA_MACOS_JSON_ETAG_CACHE}" --output "${SOFA_MACOS_JSON_CACHE}" 2>&1)
		...
		curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --write-out "%{http_code}" --max-time 5 --location "${sofa_macos_url}" --etag-save "${SOFA_MACOS_JSON_ETAG_CACHE}" --output "${SOFA_MACOS_JSON_CACHE}" 2>&1)
	fi
	...
	if [[ "${sofa_macos_json}" == "TRUE" ]]; then
		set_pref_main "SOFAMacOSJsonCacheChecksum" "$(md5 -q "${SOFA_MACOS_JSON_CACHE}" 2>/dev/null)"
	else
		delete_pref_main "SOFAMacOSJsonCacheChecksum"
		rm -f "${SOFA_MACOS_JSON_ETAG_CACHE}" 2>/dev/null
	fi
...
# Evaluate ${schedule_zero_date_manual} and ${schedule_zero_date_release} to set ${schedule_zero_date}, ${schedule_zero_date_epoch}, and ${display_string_schedule_zero_date} accordingly.
check_schedule_zero_date() {
	schedule_zero_date="FALSE"
	schedule_deferred_start_days_active="FALSE"
	local schedule_zero_date_automatic_release_previous
	schedule_zero_date_automatic_release_previous=$(get_pref "ScheduleZeroDateAutomaticRelease")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_automatic_release_previous: ${schedule_zero_date_automatic_release_previous}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_automatic_release_failover_previous: ${schedule_zero_date_automatic_release_failover_previous}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_automatic_start_previous: ${schedule_zero_date_automatic_start_previous}"
	...
	if [[ -n "${schedule_zero_date_manual}" ]]; then
		...
		[[ -n "${schedule_zero_date_automatic_release_failover_previous}" ]] && delete_pref_main "ScheduleZeroDateAutomaticReleaseFailover" && unset schedule_zero_date_automatic_release_failover_previous
		[[ -n "${schedule_zero_date_automatic_release_previous}" ]] && delete_pref_main "ScheduleZeroDateAutomaticRelease" && unset schedule_zero_date_automatic_release_previous
	fi
	...
	if [[ "${schedule_zero_date}" == "FALSE" ]]; then
		...
		if { [[ "${workflow_target}" == "Non-system Software Updates" ]] || [[ "${workflow_target}" == "Jamf Pro Policy Triggers Without Restarting" ]] || [[ "${workflow_target}" == "Jamf Pro Policy Triggers With Restart" ]] || [[ "${workflow_target}" == "Restart Without Updates" ]]; } && [[ "${schedule_zero_date_release}" == "TRUE" ]]; then
			log_super "Warning: The --schedule-zero-date-release option can not be used with the current ${workflow_target} workflow target."
			schedule_zero_date_release="FALSE"
		fi
		
		# If the current ${workflow_target} matches the previously set workflow target then collect any previously saved zero dates.
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_release: ${schedule_zero_date_release}"
		if [[ "${workflow_target}" == "${workflow_target_previous}" ]]; then
			if [[ "${schedule_zero_date_release}" == "TRUE" ]] && [[ $schedule_zero_date_automatic_release_failover_previous -ne 1 ]]; then
	...
	# If no ${schedule_zero_date} yet and the ${schedule_zero_date_release} is enabled then download ${SOFA_MACOS_JSON_CACHE} if it hasn't already been cached.
	if [[ "${schedule_zero_date}" == "FALSE" ]] && [[ "${schedule_zero_date_release}" == "TRUE" ]]; then
		sofa_macos_json="FALSE"
		...
		{ [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_installer_minor_update_target}" != "FALSE" ]]; } && schedule_zero_date_version_target="${macos_installer_version}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_version_target: ${schedule_zero_date_version_target}"
	...
	# If no ${schedule_zero_date} yet and the ${schedule_zero_date_release} is enabled and there is a ${schedule_zero_date_version_target} and there appears to be a valid ${SOFA_MACOS_JSON_CACHE}, then attempt to parse and find the release date associated with ${schedule_zero_date_version_target}
	if [[ "${schedule_zero_date}" == "FALSE" ]] && [[ "${schedule_zero_date_version_target}" != "FALSE" ]] && [[ "${schedule_zero_date_release}" == "TRUE" ]] && [[ "${sofa_macos_json}" == "TRUE" ]]; then
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_release_failover: ${schedule_zero_date_release_failover}"
	
	# If there is no ${schedule_zero_date} at this point then set one based on workflow start date/time of now.
	if [[ "${schedule_zero_date}" == "FALSE" ]]; then
		schedule_zero_date=$(date -r "${workflow_time_epoch}" +%Y-%m-%d:%H:%M)
		log_super_audit "Status: Setting new automatic zero date based on the ${workflow_target} workflow start date of ${schedule_zero_date}."
		set_pref_main "ScheduleZeroDateAutomaticStart" "${schedule_zero_date}"
		delete_pref_main "ScheduleZeroDateAutomaticRelease"
		[[ -n "${schedule_zero_date_automatic_release_previous}" ]] && delete_pref_main "ScheduleZeroDateAutomaticRelease" && unset schedule_zero_date_automatic_release_previous
		if [[ "${schedule_zero_date_release_failover}" == "TRUE" ]]; then
			set_pref_main "ScheduleZeroDateAutomaticReleaseFailover" "TRUE"
		else
			[[ -n "${schedule_zero_date_automatic_release_failover_previous}" ]] && delete_pref_main "ScheduleZeroDateAutomaticReleaseFailover" && unset schedule_zero_date_automatic_release_failover_previous
		fi
	fi
	
	# Set the ${schedule_zero_date_epoch} and ${display_string_schedule_zero_date} parameters.
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_epoch: ${schedule_zero_date_epoch}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_string_schedule_zero_date_only_date: ${display_string_schedule_zero_date_only_date}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_string_schedule_zero_date_only_time: ${display_string_schedule_zero_date_only_time}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_string_schedule_zero_date: ${display_string_schedule_zero_date}"
	
	# If required, handle the ${schedule_deferred_start_days} option and ${schedule_deferred_start_days_active} accordingly.
	if [[ -n $schedule_deferred_start_days ]]; then
		schedule_zero_date_deferred_start_date=$(date -r $((schedule_zero_date_epoch + (schedule_deferred_start_days * 86400))) +%Y-%m-%d:%H:%M)
		schedule_zero_date_deferred_start_epoch=$(date -j -f %Y-%m-%d:%H:%M:%S "${schedule_zero_date_deferred_start_date}:00" +%s)
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_deferred_start_epoch: ${schedule_zero_date_deferred_start_epoch}"
		if [[ $schedule_zero_date_deferred_start_epoch -lt $workflow_time_epoch ]]; then
			log_super "Status: Schedule deferred start of ${schedule_zero_date_deferred_start_date} (${schedule_deferred_start_days} day(s) after zero date of ${schedule_zero_date}) has passed."
		else
			local schedule_zero_date_deferred_start_difference
			schedule_zero_date_deferred_start_difference=$((schedule_zero_date_deferred_start_epoch - workflow_time_epoch))
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_zero_date_deferred_start_difference is: ${schedule_zero_date_deferred_start_difference}"
			if [[ $schedule_zero_date_deferred_start_difference -le 180 ]]; then
				log_super "Status: Schedule deferred start of ${schedule_zero_date_deferred_start_date} (${schedule_deferred_start_days} day(s) after zero date of ${schedule_zero_date}) is only ${schedule_zero_date_deferred_start_difference} seconds away, waiting for deferred start to pass..."
				sleep $((schedule_zero_date_deferred_start_difference + 1))
				log_super "Status: Schedule deferred start of ${schedule_zero_date_deferred_start_date} (${schedule_deferred_start_days} day(s) after zero date of ${schedule_zero_date}) has passed."
			else
				log_super "Status: Schedule deferred start of ${schedule_zero_date_deferred_start_date} (${schedule_deferred_start_days} day(s) after zero date of ${schedule_zero_date}) has not passed."
				schedule_deferred_start_days_active="TRUE"
			fi
		fi
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: schedule_deferred_start_days_active: ${schedule_deferred_start_days_active}"
}
...
check_scheduled_install() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_days: ${scheduled_install_days}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_date: ${scheduled_install_date}"
	if [[ -z "${workflow_scheduled_install}" ]] && { [[ -n "${scheduled_install_days}" ]] || [[ -n "${scheduled_install_date}" ]]; }; then
		...
		if [[ -n "${scheduled_install_date}" ]]; then
			...
			scheduled_install_suppress_reminder="TRUE"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_past_deadline: ${scheduled_install_past_deadline}"
		fi
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_scheduled_install: ${workflow_scheduled_install}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_suppress_reminder: ${scheduled_install_suppress_reminder}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_scheduled_install: ${workflow_scheduled_install}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_past_deadline: ${scheduled_install_past_deadline}"
		
		# If needed adjust for the ${schedule_workflow_active} and also write to log.
		if [[ -n "${schedule_workflow_active}" ]] && { [[ -n "${scheduled_install_days}" ]] || [[ $(echo "${scheduled_install_date}" | grep -c ':00:00$') -gt 0 ]]; }; then
			set_schedule_workflow_active_adjustments
			if [[ -n "${workflow_scheduled_install_adjusted}" ]]; then
				...
				if [[ -n "${scheduled_install_days}" ]] && [[ $scheduled_install_days -eq 0 ]]; then
					log_super_audit "Status: Setting new scheduled installation for $(date -j -f %Y-%m-%d "${workflow_scheduled_install:0:10}" +%a | tr '[:lower:]' '[:upper:]') ${workflow_scheduled_install} which is the start of the soonest workflow active time frame after zero day."
				else
					[[ "${schedule_workflow_active_matches_now}" == "TRUE" ]] && log_super_audit "Status: Setting new scheduled installation for $(date -j -f %Y-%m-%d "${workflow_scheduled_install:0:10}" +%a | tr '[:lower:]' '[:upper:]') ${workflow_scheduled_install} which is the start of the current schedule workflow active time frame."
					[[ "${schedule_workflow_active_matches_now}" == "FALSE" ]] && log_super_audit "Status: Setting new scheduled installation for $(date -j -f %Y-%m-%d "${workflow_scheduled_install:0:10}" +%a | tr '[:lower:]' '[:upper:]') ${workflow_scheduled_install} which is the start of the latest workflow active time frame on the original scheduled installation date."
				fi
			else
		...
		elif [[ -n "${schedule_workflow_active}" ]]; then
			...
			[[ -n "${scheduled_install_days}" ]] && log_super_audit "Status: Setting new scheduled installation for ${workflow_scheduled_install} which is ${scheduled_install_days} days after zero date."
		...
		set_pref_main "WorkflowScheduledInstall" "${workflow_scheduled_install}"
	fi
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_scheduled_install_epoch: ${workflow_scheduled_install_epoch}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_scheduled_install_active: ${workflow_scheduled_install_active}"
	...
	if [[ "${workflow_scheduled_install_active}" == "TRUE" ]]; then
		...
		[[ $(echo "${display_unmovable}" | grep -c 'SCHEDULED') -gt 0 ]] && display_unmovable_status="TRUE"
		[[ $(echo "${display_hide_background}" | grep -c 'SCHEDULED') -gt 0 ]] && display_hide_background_status="TRUE"
		[[ $(echo "${display_silently}" | grep -c 'SCHEDULED') -gt 0 ]] && display_silently_status="TRUE"
		[[ $(echo "${display_notifications_centered}" | grep -c 'SCHEDULED') -gt 0 ]] && display_notifications_centered_status="TRUE"
		[[ $(echo "${display_notifications_centered}" | grep -c 'SCHEDULED') -gt 0 ]] && display_notifications_centered_status="TRUE"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_string_scheduled_install_only_date: ${display_string_scheduled_install_only_date}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_string_scheduled_install_only_time: ${display_string_scheduled_install_only_time}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_string_scheduled_install: ${display_string_scheduled_install}"
		...
		if [[ "${schedule_workflow_active_matches_now}" == "TRUE" ]] || [[ $workflow_scheduled_install_epoch -lt $workflow_time_epoch ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_scheduled_install_difference is: ${workflow_scheduled_install_difference}"
			{ [[ "${scheduled_install_user_choice}" == "TRUE" ]] && [[ $workflow_scheduled_install_difference -gt $deferral_timer_minutes ]]; } && scheduled_install_user_choice_active="TRUE"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_user_choice_active: ${scheduled_install_user_choice_active}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_scheduled_install_now: ${workflow_scheduled_install_now}"
	fi
}
...
set_scheduled_install_deferral() {
	...
	if [[ -n "${scheduled_install_reminder_minutes}" ]]; then
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_reminder_minutes is: ${scheduled_install_reminder_minutes}"
		local previous_ifs
		previous_ifs="${IFS}"
		IFS=','
...
check_deadlines_days_date() {
	...
	if [[ -n "${deadline_days_focus}" ]]; then
		...
		set_pref_main "DeadlineDaysFocusDate" "${deadline_days_focus_date}"
		local deadline_days_focus_epoch
		deadline_days_focus_epoch=$(date -j -f %Y-%m-%d:%H:%M:%S "${deadline_days_focus_date}:00:00:00" +%s)
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_focus_epoch: ${deadline_days_focus_epoch}"
		if [[ $deadline_days_focus_epoch -lt $workflow_time_epoch ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_focus_difference is: ${deadline_days_focus_difference}"
	...
	if [[ -n "${deadline_days_soft}" ]]; then
		...
		set_pref_main "DeadlineDaysSoftDate" "${deadline_days_soft_date}"
		local deadline_days_soft_epoch
		deadline_days_soft_epoch=$(date -j -f %Y-%m-%d:%H:%M:%S "${deadline_days_soft_date}:00:00:00" +%s)
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_soft_epoch: ${deadline_days_soft_epoch}"
		if [[ $deadline_days_soft_epoch -lt $workflow_time_epoch ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_soft_difference is: ${deadline_days_soft_difference}"
	...
	if [[ -n "${deadline_days_hard}" ]]; then
		...
		set_pref_main "DeadlineDaysHardDate" "${deadline_days_hard_date}"
		local deadline_days_hard_epoch
		deadline_days_hard_epoch=$(date -j -f %Y-%m-%d:%H:%M:%S "${deadline_days_hard_date}:00:00:00" +%s)
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_hard_epoch: ${deadline_days_hard_epoch}"
		if [[ $deadline_days_hard_epoch -lt $workflow_time_epoch ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_hard_difference is: ${deadline_days_hard_difference}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_status is: ${deadline_days_status}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_days_epoch is: ${deadline_days_epoch}"
	
	# Evaluate date deadlines and set ${deadline_date_status} and ${deadline_date_epoch}.
	if [[ -n "${deadline_date_focus}" ]]; then
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_focus_epoch is: ${deadline_date_focus_epoch}"
		if [[ $deadline_date_focus_epoch -lt $workflow_time_epoch ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_focus_difference is: ${deadline_date_focus_difference}"
	...
	if [[ -n "${deadline_date_soft}" ]]; then
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_soft_epoch is: ${deadline_date_soft_epoch}"
		if [[ $deadline_date_soft_epoch -lt $workflow_time_epoch ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_soft_difference is: ${deadline_date_soft_difference}"
	...
	if [[ -n "${deadline_date_hard}" ]]; then
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_hard_epoch is: ${deadline_date_hard_epoch}"
		if [[ $deadline_date_hard_epoch -lt $workflow_time_epoch ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_hard_difference is: ${deadline_date_hard_difference}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_status is: ${deadline_date_status}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_date_epoch is: ${deadline_date_epoch}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_epoch is: ${deadline_epoch}"
	...
	if [[ -n "${deadline_epoch}" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_string_deadline_only_date is: ${display_string_deadline_only_date}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_string_deadline_only_time is: ${display_string_deadline_only_time}"
	...
	if [[ -n "${deadline_epoch}" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_deadline_minutes is: ${deferral_timer_deadline_minutes}"
		if [[ -n "${deferral_timer_menu_minutes}" ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_menu_reduced is: ${deferral_timer_menu_reduced}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_menu_reduced_array is: ${deferral_timer_menu_reduced_array[*]}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_deadline_active is: ${deferral_timer_deadline_active}"
...
check_user_focus() {
	...
	if [[ -n "${deadline_count_focus}" ]] || [[ -n "${deadline_days_focus}" ]] || [[ -n "${deadline_date_focus}" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: focus_response is: ${focus_response}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_assertions_array is:\n${display_assertions_array[*]}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: user_focus_active is: ${user_focus_active}"
}
...
check_deadlines_count() {
	...
	if [[ "${user_focus_active}" == "TRUE" ]]; then
		if [[ -n "${deadline_count_focus}" ]]; then
			...
			deadline_counter_focus_previous=$(get_pref "DeadlineCounterFocus")
	...
	if [[ "${user_focus_active}" == "FALSE" ]]; then
		if [[ -n "${deadline_count_soft}" ]]; then
			...
			deadline_counter_soft_previous=$(get_pref "DeadlineCounterSoft")
		...
		if [[ -n "${deadline_count_hard}" ]]; then
			...
			deadline_counter_hard_previous=$(get_pref "DeadlineCounterHard")
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deadline_count_status is: ${deadline_count_status}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: user_focus_active is: ${user_focus_active}"
}
...
check_software_update_status_cached() {
	...
	local msu_last_sccessful_check_date
	msu_last_sccessful_check_date=$(defaults read "${MSU_LOCAL_PLIST}" LastSuccessfulDate 2>/dev/null)
	local msu_last_sccessful_check_epoch
	[[ -n "${msu_last_sccessful_check_date}" ]] && msu_last_sccessful_check_epoch=$(date -j -u -f "%Y-%m-%d %H:%M:%S %z" "${msu_last_sccessful_check_date}" +%s)
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_last_sccessful_check_date is: ${msu_last_sccessful_check_date}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_last_sccessful_check_epoch is: ${msu_last_sccessful_check_epoch}"
	local super_last_sccessful_check_date
	super_last_sccessful_check_date=$(get_pref "LastSuccessfulCheckDate")
	local super_last_sccessful_check_epoch
	[[ -n "${super_last_sccessful_check_date}" ]] && super_last_sccessful_check_epoch=$(date -j -u -f "%Y-%m-%d:%H:%M:%S" "${super_last_sccessful_check_date}" +%s)
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: super_last_sccessful_check_date is: ${super_last_sccessful_check_date}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: super_last_sccessful_check_epoch is: ${super_last_sccessful_check_epoch}"
	if [[ $msu_last_sccessful_check_epoch -lt $((workflow_time_epoch - (SOFTWARE_STATUS_CACHE_AGE_MINUTES * 60))) ]] && [[ $super_last_sccessful_check_epoch -lt $((workflow_time_epoch - (SOFTWARE_STATUS_CACHE_AGE_MINUTES * 60))) ]]; then
		log_super "Status: Last macOS software status check was more than $((SOFTWARE_STATUS_CACHE_AGE_MINUTES / 60)) hours ago, full software status check required."
	...
	deferral_restrictions_checksum=$(get_pref "DeferralRestrictionsChecksum")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_restrictions_checksum is: ${deferral_restrictions_checksum}"
	if { [[ $deferral_restrictions_checksum -eq 0 ]] && [[ ! -f "${APPLICATION_ACCESS_MANAGED_PLIST}.plist" ]]; } || [[ "${deferral_restrictions_checksum}" == "$(md5 -q "${APPLICATION_ACCESS_MANAGED_PLIST}.plist" 2>/dev/null)" ]]; then
		local deferral_restrictions_cache
		deferral_restrictions_cache=$(get_pref "DeferralRestrictionsCache")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_restrictions_cache is: ${deferral_restrictions_cache}"
	...
	macos_beta_program_cache=$(get_pref "MacOSBetaProgramCache")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_beta_program_cache is: ${macos_beta_program_cache}"
	if [[ -n "${macos_beta_program_cache}" ]]; then
		if [[ "${macos_beta_program_cache}" == "TRUE" ]]; then
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_beta_program is: ${macos_beta_program}"
	
	# Check the mdmclient list cache.
	local mdmclient_list_checksum_cache
	mdmclient_list_checksum_cache=$(get_pref "MDMClientListChecksum")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_list_checksum_cache is: ${mdmclient_list_checksum_cache}"
	if [[ "${mdmclient_list_checksum_cache}" == "$(md5 -q "${MDMCLIENT_LIST_LOG}" 2>/dev/null)" ]]; then
		local mdmclient_available_cache
		mdmclient_available_cache=$(get_pref "MDMClientAvailableCache")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_available_cache is: ${mdmclient_available_cache}"
		if [[ -n "${mdmclient_available_cache}" ]]; then
			if [[ "${mdmclient_available_cache}" == "TRUE" ]]; then
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_list is:\n${mdmclient_list}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_available is: ${mdmclient_available}"
	
	# Check the macOS installers list cache.
	local macos_installers_list_checksum_cache
	macos_installers_list_checksum_cache=$(get_pref "MacOSInstallersListChecksum")
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installers_list is:\n${macos_installers_list}"
	
	# Check the macOS software update list cache.
	local msu_list_checksum_cache
	msu_list_checksum_cache=$(get_pref "MSUListChecksum")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_list_checksum_cache is: ${msu_list_checksum_cache}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_list is:\n${msu_list}"
}
...
reset_software_update_status() {
	...
	deferral_restrictions_macOS_minor_updates="FALSE"
	deferral_restrictions_macOS_major_upgrades="FALSE"
	deferral_restrictions_non_system_updates="FALSE"
	macos_beta_program="FALSE"
	mdmclient_available="FALSE"
	mdmclient_list="FALSE"
	macos_installers_list="FALSE"
	...
	delete_pref_main "DeferralRestrictionsChecksum"
	delete_pref_main "DeferralRestrictionsCache"
	delete_pref_main "MacOSBetaProgramCache"
	delete_pref_main "MDMClientListChecksum"
	delete_pref_main "MDMClientAvailableCache"
	delete_pref_main "MacOSInstallerDownloaded"
	delete_pref_main "MacOSMSULabelDownloaded"
	rm -f "${MDMCLIENT_LIST_LOG}" 2>/dev/null
	delete_pref_main "MacOSInstallersListChecksum"
	rm -f "${MACOS_INSTALLERS_LIST_LOG}" 2>/dev/null
	delete_pref_main "MSUListChecksum" 
...
check_deferral_restrictions() {
	...
	if [[ -f "${APPLICATION_ACCESS_MANAGED_PLIST}.plist" ]]; then
		set_pref_main "DeferralRestrictionsChecksum" "$(md5 -q "${APPLICATION_ACCESS_MANAGED_PLIST}.plist" 2>/dev/null)"
		local deferral_restrictions_response
		deferral_restrictions_response=$(defaults read "${APPLICATION_ACCESS_MANAGED_PLIST}" 2>&1 | grep -E 'enforcedSoftware|forceDelayed' | tr -d ';')
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_restrictions_response is:\n${deferral_restrictions_response}"
	if [[ -n "${deferral_restrictions_response}" ]]; then
		set_pref_main "DeferralRestrictionsCache" "TRUE"
		if [[ $(echo "${deferral_restrictions_response}" | awk '/forceDelayedMajorSoftwareUpdates/ {print $3}') -gt 0 ]]; then
			deferral_restrictions_macOS_major_upgrades=30
			[[ $(echo "${deferral_restrictions_response}" | awk '/enforcedSoftwareUpdateMajorOSDeferredInstallDelay/ {print $3}') -gt 0 ]] && deferral_restrictions_macOS_major_upgrades=$(echo "${deferral_restrictions_response}" | awk '/enforcedSoftwareUpdateMajorOSDeferredInstallDelay/ {print $3}')
			log_super "Status: Restrictions configuration profile is deferring macOS major upgrades for ${deferral_restrictions_macOS_major_upgrades} days."
		fi
		if [[ $(echo "${deferral_restrictions_response}" | awk '/forceDelayedSoftwareUpdates/ {print $3}') -gt 0 ]]; then
			deferral_restrictions_macOS_minor_updates=30
			[[ $(echo "${deferral_restrictions_response}" | awk '/enforcedSoftwareUpdateDelay/ {print $3}') -gt 0 ]] && deferral_restrictions_macOS_minor_updates=$(echo "${deferral_restrictions_response}" | awk '/enforcedSoftwareUpdateDelay/ {print $3}')
			[[ $(echo "${deferral_restrictions_response}" | awk '/enforcedSoftwareUpdateMinorOSDeferredInstallDelay/ {print $3}') -gt 0 ]] && deferral_restrictions_macOS_minor_updates=$(echo "${deferral_restrictions_response}" | awk '/enforcedSoftwareUpdateMinorOSDeferredInstallDelay/ {print $3}')
			log_super "Status: Restrictions configuration profile is deferring macOS minor updates for ${deferral_restrictions_macOS_minor_updates} days."
		fi
		if [[ $(echo "${deferral_restrictions_response}" | awk '/forceDelayedAppSoftwareUpdates/ {print $3}') -gt 0 ]]; then
			deferral_restrictions_non_system_updates=30
			[[ $(echo "${deferral_restrictions_response}" | awk '/enforcedSoftwareUpdateDelay/ {print $3}') -gt 0 ]] && deferral_restrictions_non_system_updates=$(echo "${deferral_restrictions_response}" | awk '/enforcedSoftwareUpdateDelay/ {print $3}')
			[[ $(echo "${deferral_restrictions_response}" | awk '/enforcedSoftwareUpdateNonOSDeferredInstallDelay/ {print $3}') -gt 0 ]] && deferral_restrictions_non_system_updates=$(echo "${deferral_restrictions_response}" | awk '/enforcedSoftwareUpdateNonOSDeferredInstallDelay/ {print $3}')
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_restrictions_macOS_minor_updates is: ${deferral_restrictions_macOS_minor_updates}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_restrictions_non_system_updates is: ${deferral_restrictions_non_system_updates}"
}

# Check for macOS beta program enrollment and set ${macos_beta_program} accordingly.
check_macos_beta_program() {
	macos_beta_program="FALSE"
	if [[ $macos_version_normalized -ge 130400 ]]; then
		local mdmclient_response
		mdmclient_response=$(/usr/libexec/mdmclient QueryDeviceInformation 2>/dev/null | grep 'IsDefaultCatalog')
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_response is:\n${mdmclient_response}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: seedutil_response is:\n${seedutil_response}"
...
get_mdmclient_list() {
	...
	if [[ "${get_mdmclient_list_error}" == "FALSE" ]] && [[ "${get_mdmclient_list_timeout}" == "FALSE" ]]; then
		mdmclient_list=$(sed -e '1,/^Available updates/d' -e '/^)$/d' < "${MDMCLIENT_LIST_LOG}")
		set_pref_main "MDMClientListChecksum" "$(md5 -q "${MDMCLIENT_LIST_LOG}" 2>/dev/null)"
		if [[ $(echo "${mdmclient_list}" | grep -c 'HumanReadableName') -gt 0 ]]; then
			mdmclient_available="TRUE"
			set_pref_main "MDMClientAvailableCache" "TRUE"
		else
			set_pref_main "MDMClientAvailableCache" "FALSE"
		fi
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: get_mdmclient_list_error is: ${get_mdmclient_list_error}"
...
get_macos_installers_list() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: get_macos_installers_list_error is: ${get_macos_installers_list_error}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: get_macos_installers_list_timeout is: ${get_macos_installers_list_timeout}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installers_list is:\n${macos_installers_list}"
}
...
get_msu_list() {
	...
	if [[ "${get_msu_list_error}" == "FALSE" ]] && [[ "${get_msu_list_timeout}" == "FALSE" ]]; then
		msu_list=$(<"${MSU_LIST_LOG}")
		set_pref_main "MSUListChecksum" "$(md5 -q "${MSU_LIST_LOG}" 2>/dev/null)"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: get_msu_list_error is: ${get_msu_list_error}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: get_msu_list_timeout is: ${get_msu_list_timeout}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: msu_list is:\n${msu_list}"
}
...
# This function sets ${macos_installer_major_upgrade_target}, ${macos_installer_minor_update_target}, ${macos_msu_major_upgrade_target}, ${macos_msu_minor_update_target}, ${non_system_msu_targets}, and ${non_system_msu_safari_target}.
# If there is a ${macos_installer_major_upgrade_target} or ${macos_installer_minor_update_target}, then this also sets ${macos_installer_title}, ${macos_installer_version}, ${macos_installer_build}, and ${macos_installer_size}.
...
# If there is ${non_system_msu_targets} then this also sets ${non_system_msu_safari_target}, ${non_system_msu_labels_array[]} and ${non_system_msu_titles_array[]}.
workflow_check_software_status() {
	...
	macos_installer_major_upgrade_target="FALSE"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: check_software_status_required is: ${check_software_status_required}"
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ "${check_software_status_required}" == "TRUE" ]]; then
		...
		if [[ "${get_mdmclient_list_error}" == "TRUE" ]] || [[ "${get_mdmclient_list_timeout}" == "TRUE" ]]; then
			...
			sleep 10
			get_mdmclient_list
		elif [[ $macos_version_normalized -lt 130300 ]] && [[ "${mdmclient_available}" == "FALSE" ]]; then
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_installers_array is:\n${mdmclient_macos_installers_array[*]}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_msu_array is:\n${mdmclient_macos_msu_array[*]}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_non_system_array is:\n${mdmclient_non_system_array[*]}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_installers_deferred_array is:\n${mdmclient_macos_installers_deferred_array[*]}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_msu_deferred_array is:\n${mdmclient_macos_msu_deferred_array[*]}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_non_system_deferred_array is:\n${mdmclient_non_system_deferred_array[*]}"
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ ${#mdmclient_macos_msu_array[@]} -gt 0 ]]; then
		...
		for mdmclient_macos_msu_item in "${mdmclient_macos_msu_array[@]}"; do
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_msu_item is: ${mdmclient_macos_msu_item}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_msu_major_upgrade_array is:\n${mdmclient_macos_msu_major_upgrade_array[*]}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_msu_minor_update_array is:\n${mdmclient_macos_msu_minor_update_array[*]}"
	fi
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ ${#mdmclient_macos_msu_deferred_array[@]} -gt 0 ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_msu_major_upgrade_deferred_array is:\n${mdmclient_macos_msu_major_upgrade_deferred_array[*]}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_msu_minor_update_deferred_array is:\n${mdmclient_macos_msu_minor_update_deferred_array[*]}"
	fi
	
	# If (no errors) there are deferred items then evaluate and report to super.log. Further, if deferral restrictions are not present then erroneously deferred items are moved back to the appropriate available items array.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && { [[ ${#mdmclient_macos_installers_deferred_array[@]} -gt 0 ]] || [[ ${#mdmclient_macos_msu_major_upgrade_deferred_array[@]} -gt 0 ]] || [[ ${#mdmclient_macos_msu_minor_update_deferred_array[@]} -gt 0 ]] || [[ ${#mdmclient_non_system_deferred_array[@]} -gt 0 ]]; }; then
		...
		{ [[ "${deferral_restrictions_macOS_major_upgrades}" != "FALSE" ]] || [[ "${deferral_restrictions_macOS_minor_updates}" != "FALSE" ]] || [[ "${deferral_restrictions_non_system_updates}" != "FALSE" ]]; } && log_super "Status: Some updates may be deferred due to a deferral restrictions configuration profile."
		{ [[ "${deferral_restrictions_macOS_major_upgrades}" == "FALSE" ]] || [[ "${deferral_restrictions_macOS_minor_updates}" == "FALSE" ]] || [[ "${deferral_restrictions_non_system_updates}" == "FALSE" ]]; } && log_super "Warning: Some updates are inaccurately reporting as deferred even though deferral restrictions configuration is not enabled. As such, these updates will still be considered for installation."
		if [[ ${#mdmclient_macos_installers_deferred_array[@]} -gt 0 ]]; then
			for array_index in "${!mdmclient_macos_installers_deferred_array[@]}"; do
				if [[ "${deferral_restrictions_macOS_major_upgrades}" == "FALSE" ]] || [[ $(date -j -f "%Y-%m-%d %H:%M:%S %z" "$(echo "${mdmclient_macos_installers_deferred_array[array_index]}" | awk -F ',' '{print $1}' | sed -e 's/DeferredUntil://g')" +%s) -le $workflow_time_epoch ]]; then
				...
				elif [[ "${deferral_restrictions_macOS_major_upgrades}" != "FALSE" ]]; then
					log_super "Deferral Restrictions: macOS major upgrade installer $((array_index + 1)) of ${#mdmclient_macos_installers_deferred_array[@]} is: ${mdmclient_macos_installers_deferred_array[array_index]}"
				fi
			done
			[[ "${deferral_restrictions_macOS_major_upgrades}" == "FALSE" ]] && mdmclient_macos_installers_array=($(sort -t ':' -k4 -V -r  <<<"${mdmclient_macos_installers_array[*]}"))
		fi
		...
		if [[ ${#mdmclient_macos_msu_major_upgrade_deferred_array[@]} -gt 0 ]]; then
			for array_index in "${!mdmclient_macos_msu_major_upgrade_deferred_array[@]}"; do
				if [[ "${deferral_restrictions_macOS_major_upgrades}" == "FALSE" ]] || [[ $(date -j -f "%Y-%m-%d %H:%M:%S %z" "$(echo "${mdmclient_macos_msu_major_upgrade_deferred_array[array_index]}" | awk -F ',' '{print $1}' | sed -e 's/DeferredUntil://g')" +%s) -le $workflow_time_epoch ]]; then
					...
					log_super "Deferral Restrictions: macOS major upgrade $((array_index + 1)) of ${#mdmclient_macos_msu_major_upgrade_deferred_array[@]} is: ${mdmclient_macos_msu_major_upgrade_deferred_array[array_index]}"
				fi
			done
			[[ "${deferral_restrictions_macOS_major_upgrades}" == "FALSE" ]] && mdmclient_macos_msu_major_upgrade_array=($(sort -t ':' -k3 -V -r  <<<"${mdmclient_macos_msu_major_upgrade_array[*]}"))
		fi
		...
		if [[ ${#mdmclient_macos_msu_minor_update_deferred_array[@]} -gt 0 ]]; then
			for array_index in "${!mdmclient_macos_msu_minor_update_deferred_array[@]}"; do
				if [[ "${deferral_restrictions_macOS_minor_updates}" == "FALSE" ]] || [[ $(date -j -f "%Y-%m-%d %H:%M:%S %z" "$(echo "${mdmclient_macos_msu_minor_update_deferred_array[array_index]}" | awk -F ',' '{print $1}' | sed -e 's/DeferredUntil://g')" +%s) -le $workflow_time_epoch ]]; then
				...
				elif [[ "${deferral_restrictions_macOS_minor_updates}" != "FALSE" ]]; then
					log_super "Deferral Restrictions: macOS minor update $((array_index + 1)) of ${#mdmclient_macos_msu_minor_update_deferred_array[@]} is: ${mdmclient_macos_msu_minor_update_deferred_array[array_index]}"
				fi
			done
			[[ "${deferral_restrictions_macOS_minor_updates}" == "FALSE" ]] && mdmclient_macos_msu_minor_update_array=($(sort -t ':' -k3 -V -r  <<<"${mdmclient_macos_msu_minor_update_array[*]}"))
		fi
		
		if [[ ${#mdmclient_non_system_deferred_array[@]} -gt 0 ]]; then
			for array_index in "${!mdmclient_non_system_deferred_array[@]}"; do
				if [[ "${deferral_restrictions_non_system_updates}" == "FALSE" ]] || [[ $(date -j -f "%Y-%m-%d %H:%M:%S %z" "$(echo "${mdmclient_non_system_deferred_array[array_index]}" | awk -F ',' '{print $1}' | sed -e 's/DeferredUntil://g')" +%s) -le $workflow_time_epoch ]]; then
				...
				elif [[ "${deferral_restrictions_non_system_updates}" != "FALSE" ]]; then
					log_super "Deferral Restrictions: Non-system update $((array_index + 1)) of ${#mdmclient_non_system_deferred_array[@]} is: ${mdmclient_non_system_deferred_array[array_index]}"
				fi
			done
		fi
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_installers_array is:\n${mdmclient_macos_installers_array[*]}"
	...
	# If (no errors) then evaluate versions in ${mdmclient_macos_msu_major_upgrade_array[]}, ${mdmclient_macos_installers_array[]}, and ${mdmclient_macos_msu_minor_update_array[]} for target selection and logging.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && { [[ ${#mdmclient_macos_msu_major_upgrade_array[@]} -gt 0 ]] || [[ ${#mdmclient_macos_installers_array[@]} -gt 0 ]] || [[ ${#mdmclient_macos_msu_minor_update_array[@]} -gt 0 ]]; }; then
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_upgrades is: ${install_macos_major_upgrades}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_version_target is: ${install_macos_major_version_target}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_version_normalized_target is: ${install_macos_major_version_normalized_target}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_minor_updates is: ${install_macos_minor_updates}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_minor_version_target is: ${install_macos_minor_version_target}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_minor_version_normalized_target is: ${install_macos_minor_version_normalized_target}"
		
		# If ${install_macos_major_upgrades} then attempt to resolve an "over-the air" macOS major upgrade target given ${install_macos_major_version_target} and ${mdmclient_macos_msu_major_upgrade_array[0]}.
		if [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ ${#mdmclient_macos_msu_major_upgrade_array[@]} -gt 0 ]]; then
			[[ -z "${install_macos_major_version_target}" ]] && mdmclient_macos_msu_major_upgrade_target="${mdmclient_macos_msu_major_upgrade_array[0]}"
			if [[ -n "${install_macos_major_version_target}" ]]; then
				if [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 0 ]] && [[ $(echo "${mdmclient_macos_msu_major_upgrade_array[0]}" | awk -F ',' '{print $3}' | sed -e 's/.*://g' -e 's/\..*//g') -eq ${install_macos_major_version_target} ]]; then
					mdmclient_macos_msu_major_upgrade_target="${mdmclient_macos_msu_major_upgrade_array[0]}"
				elif [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -gt 0 ]] && [[ $(echo "${mdmclient_macos_msu_major_upgrade_array[0]}" | awk -F ',' '{print $3}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -eq $install_macos_major_version_normalized_target ]]; then
					mdmclient_macos_msu_major_upgrade_target="${mdmclient_macos_msu_major_upgrade_array[0]}"
				else
					log_super "Warning: Unable to target the latest available \"over-the-air\" macOS major upgrade given the specified --install-macos-major-version-target option of ${install_macos_major_version_target}."
				fi
			fi
			[[ "${deferral_restrictions_macOS_major_upgrades}" == "FALSE" ]] && [[ "${mdmclient_macos_msu_major_upgrade_target}" != "FALSE" ]] && macos_major_upgrade_latest="TRUE"
		fi
		...
		# If there is no ${mdmclient_macos_msu_major_upgrade_target} then attempt to resolve an installer-based macOS major upgrade target given ${install_macos_major_version_target} and ${mdmclient_macos_installers_array[]}.
		if [[ "${mdmclient_macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ ${#mdmclient_macos_installers_array[@]} -gt 0 ]]; then
			if [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ -z "${install_macos_major_version_target}" ]]; then
				mdmclient_macos_installer_target="${mdmclient_macos_installers_array[0]}"
				[[ "${deferral_restrictions_macOS_major_upgrades}" == "FALSE" ]] && macos_major_upgrade_latest="TRUE"
			fi
			if [[ "${mdmclient_macos_installer_target}" == "FALSE" ]]; then
				...
				for mdmclient_macos_installer_item in "${mdmclient_macos_installers_array[@]}"; do
					[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_installer_item is: ${mdmclient_macos_installer_item}"
					if [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ -n "${install_macos_major_version_target}" ]] && [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 0 ]] && [[ $(echo "${mdmclient_macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' -e 's/\..*//g') -eq $install_macos_major_version_target ]]; then
						mdmclient_macos_installer_target="${mdmclient_macos_installer_item}"
						break
					elif [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ -n "${install_macos_major_version_target}" ]] && [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -gt 0 ]] && [[ $(echo "${mdmclient_macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -eq $install_macos_major_version_normalized_target ]]; then
						mdmclient_macos_installer_target="${mdmclient_macos_installer_item}"
						break
					fi
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_msu_major_upgrade_target is: ${mdmclient_macos_msu_major_upgrade_target}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_installer_target is: ${mdmclient_macos_installer_target}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_major_upgrade_latest is: ${macos_major_upgrade_latest}"
		
		# If there is no ${mdmclient_macos_msu_major_upgrade_target} or ${mdmclient_macos_installer_target} then attempt to resolve an "over-the-air" macOS minor update target given ${install_macos_minor_version_target} and ${mdmclient_macos_msu_minor_update_array[0]}.
		if [[ "${install_macos_minor_updates}" == "TRUE" ]] && [[ "${mdmclient_macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${mdmclient_macos_installer_target}" == "FALSE" ]] && [[ ${#mdmclient_macos_msu_minor_update_array[@]} -gt 0 ]]; then
			[[ -z "${install_macos_minor_version_target}" ]] && mdmclient_macos_msu_minor_update_target="${mdmclient_macos_msu_minor_update_array[0]}"
			if [[ -n "${install_macos_minor_version_target}" ]]; then
				if [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -eq 0 ]] && [[ $(echo "${mdmclient_macos_msu_minor_update_array[0]}" | awk -F ',' '{print $3}' | sed -e 's/.*://g' -e 's/\..*//g') -eq ${install_macos_minor_version_target} ]]; then
					mdmclient_macos_msu_minor_update_target="${mdmclient_macos_msu_minor_update_array[0]}"
				elif [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -gt 0 ]] && [[ $(echo "${mdmclient_macos_msu_minor_update_array[0]}" | awk -F ',' '{print $3}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -eq $install_macos_minor_version_normalized_target ]]; then
					mdmclient_macos_msu_minor_update_target="${mdmclient_macos_msu_minor_update_array[0]}"
				else
					log_super "Warning: Unable to target the latest available \"over-the-air\" macOS minor update given the specified --install-macos-minor-version-target option of ${install_macos_minor_version_target}."
				fi
			fi
			[[ "${deferral_restrictions_macOS_minor_updates}" == "FALSE" ]] && [[ "${mdmclient_macos_msu_minor_update_target}" != "FALSE" ]] && macos_minor_update_latest="TRUE"
		fi
		{ [[ "${mdmclient_macos_msu_major_upgrade_target}" != "FALSE" ]] || [[ "${mdmclient_macos_installer_target}" != "FALSE" ]] || [[ "${mdmclient_macos_msu_minor_update_target}" != "FALSE" ]]; } && macos_target_mdm_compatible="TRUE"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_msu_minor_update_target is: ${mdmclient_macos_msu_minor_update_target}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_target_mdm_compatible is: ${macos_target_mdm_compatible}"
	fi
	
	# Handle issues then targeting non-mdm macOS versions.
	if [[ "${macos_target_mdm_compatible}" == "FALSE" ]] && [[ "${workflow_macos_auth}" == "JAMF" ]]; then
	...
	# If (no errors) there is the possibility of targeting a macOS installer then collect the ${macos_installers_list} and create the ${macos_installers_array[]}.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ "${mdmclient_macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${mdmclient_macos_msu_minor_update_target}" == "FALSE" ]] && { [[ "${mdmclient_macos_installer_target}" != "FALSE" ]] || [[ -n "${install_macos_major_version_target}" ]] || [[ -n "${install_macos_minor_version_target}" ]]; }; then
		...
		# Make sure we have a ${macos_installers_list} an if so create the ${macos_installers_array[]}.
		if [[ "${macos_installers_list}" == "FALSE" ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installers_sanitized_list is:\n${macos_installers_sanitized_list}"
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installers_array is:\n${macos_installers_array[*]}"
			local macos_installer_major_upgrades_array
			macos_installer_major_upgrades_array=()
			local macos_installer_minor_updates_array
			macos_installer_minor_updates_array=()
			for macos_installer_item in "${macos_installers_array[@]}"; do
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_item is: ${macos_installer_item}"
				[[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' -e 's/\..*//g') -gt $macos_version_major ]] && macos_installer_major_upgrades_array+=("${macos_installer_item}")
				[[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' -e 's/\..*//g') -eq $macos_version_major ]] && macos_installer_minor_updates_array+=("${macos_installer_item}")
			done
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_major_upgrades_array is:\n${macos_installer_major_upgrades_array[*]}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_minor_updates_array is:\n${macos_installer_minor_updates_array[*]}"
		fi
	fi
	
	# If (no errors) then evaluate versions in ${macos_installer_major_upgrades_array[]} and ${macos_installer_minor_updates_array[]} for targeting or logging.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && { [[ ${#macos_installer_major_upgrades_array[@]} -gt 0 ]] || [[ ${#macos_installer_minor_updates_array[@]} -gt 0 ]]; }; then
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_upgrades is: ${install_macos_major_upgrades}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_version_target is: ${install_macos_major_version_target}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_version_normalized_target is: ${install_macos_major_version_normalized_target}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_minor_version_target is: ${install_macos_minor_version_target}"
		...
		# Evaluate versions in ${macos_installer_major_upgrades_array[]} and set ${macos_installer_major_upgrade_target} or log disallowed installers.
		if [[ -n "${install_macos_major_version_target}" ]] && [[ ${#macos_installer_major_upgrades_array[@]} -gt 0 ]]; then
			local macos_installer_major_upgrades_disallowed_array
			macos_installer_major_upgrades_disallowed_array=()
			local mdmclient_macos_installer_version_normalized_target
			[[ "${mdmclient_macos_installer_target}" != "FALSE" ]] && mdmclient_macos_installer_version_normalized_target=$(echo "${install_macos_minor_version_target}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_installer_version_normalized_target is: ${mdmclient_macos_installer_version_normalized_target}"
			for macos_installer_item in "${macos_installer_major_upgrades_array[@]}"; do
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_item is: ${macos_installer_item}"
				if [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ "${mdmclient_macos_installer_target}" != "FALSE" ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $3}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -eq $mdmclient_macos_installer_version_normalized_target ]]; then
					macos_installer_major_upgrade_target="${macos_installer_item}"
					break
				elif [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 0 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' -e 's/\..*//g') -eq ${install_macos_major_version_target} ]]; then
					macos_installer_major_upgrade_target="${macos_installer_item}"
					break
				elif [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 1 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d", $1, $2}') -eq ${install_macos_major_version_normalized_target:0:4} ]]; then
					macos_installer_major_upgrade_target="${macos_installer_item}"
					break
				elif [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 2 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -eq $install_macos_major_version_normalized_target ]]; then
					macos_installer_major_upgrade_target="${macos_installer_item}"
					break
				fi
				[[ "${macos_installer_major_upgrade_target}" == "FALSE" ]] && macos_installer_major_upgrades_disallowed_array+=("${macos_installer_item}")
			done
			if [[ "${mdmclient_macos_installer_target}" != "FALSE" ]] && [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]]; then
				log_super "Error: The mist-cli listing did not contain a macOS installer that matches targeted version: ${mdmclient_macos_installer_target}"
				workflow_check_software_status_error="TRUE"
			fi
			{ [[ "${mdmclient_macos_installer_target}" == "FALSE" ]] && [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]]; } && log_super "Warning: Unable to target any available macOS major upgrade installer given the specified --install-macos-major-version-target option of ${install_macos_major_version_target}."
		fi
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_major_upgrade_target is: ${macos_installer_major_upgrade_target}"
		
		# If there is no ${macos_installer_major_upgrade_target} then evaluate versions in ${macos_installer_minor_updates_array[]} and set ${macos_installer_minor_update_target} or log disallowed installers.
		if [[ -n "${install_macos_minor_version_target}" ]] && [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]] && [[ ${#macos_installer_minor_updates_array[@]} -gt 0 ]]; then
			local macos_installer_minor_updates_disallowed_array
			macos_installer_minor_updates_disallowed_array=()
			for macos_installer_item in "${macos_installer_minor_updates_array[@]}"; do
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_item is: ${macos_installer_item}"
				if [[ "${install_macos_minor_updates}" == "TRUE" ]] && [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -eq 0 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' -e 's/\..*//g') -eq ${install_macos_minor_version_target} ]]; then
					macos_installer_minor_update_target="${macos_installer_item}"
					break
				elif [[ "${install_macos_minor_updates}" == "TRUE" ]] && [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -eq 1 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d", $1, $2}') -eq ${install_macos_minor_version_normalized_target:0:4} ]]; then
					macos_installer_minor_update_target="${macos_installer_item}"
					break
				elif [[ "${install_macos_minor_updates}" == "TRUE" ]] && [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -eq 2 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -eq $install_macos_minor_version_normalized_target ]]; then
					macos_installer_minor_update_target="${macos_installer_item}"
					break
				fi
				[[ "${macos_installer_minor_update_target}" == "FALSE" ]] && macos_installer_minor_updates_disallowed_array+=("${macos_installer_item}")
			done
			[[ "${macos_installer_minor_update_target}" == "FALSE" ]] && log_super "Warning: Unable to target any available macOS minor update installer given the specified --install-macos-minor-version-target option of ${install_macos_minor_version_target}."
		fi
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_minor_update_target is: ${macos_installer_minor_update_target}"
	fi
	
	# If (no errors) there are macOS updates or upgrades but they aren't possible or allowed then report to super.log.
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_installers_disallowed_array is:\n${mdmclient_macos_installers_disallowed_array[*]}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_major_upgrades_disallowed_array is:\n${macos_installer_major_upgrades_disallowed_array[*]}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_minor_updates_disallowed_array is:\n${macos_installer_minor_updates_disallowed_array[*]}"
	if [[ "${mdmclient_macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ ${#mdmclient_macos_msu_major_upgrade_array[@]} -gt 0 ]]; then
		log_super "Disallowed: \"over-the-air\" macOS major upgrade is available but not allowed: $(echo "${mdmclient_macos_msu_major_upgrade_array[0]}" | sed -e 's/Title://' -e 's/,Build:/-/' -e 's/,.*//')"
		[[ -n "${install_macos_major_version_target}" ]] && [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] && log_super "Warning: The --install-macos-major-version-target=${install_macos_major_version_target} option is forcing the workflow to target an older macOS installer (as opposed to a more efficient \"over-the-air\" macOS major upgrade)."
	fi
	if [[ ${#mdmclient_macos_installers_disallowed_array[@]} -gt 0 ]] || [[ ${#macos_installer_major_upgrades_disallowed_array[@]} -gt 0 ]]; then
		if [[ ${#mdmclient_macos_installers_disallowed_array[@]} -ge ${#macos_installer_major_upgrades_disallowed_array[@]} ]]; then
			for macos_installer_item in "${mdmclient_macos_installers_disallowed_array[@]}"; do
				[[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -lt $(echo "${mdmclient_macos_msu_major_upgrade_array[0]}" | awk -F ',' '{print $3}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') ]] && log_super "Disallowed: macOS major upgrade installer is available but not allowed: $(echo "${macos_installer_item}" | sed -e 's/Title://' -e 's/,Version:/ /' -e 's/,Build:/-/' -e 's/,.*//')"
			done
		else
			for macos_installer_item in "${macos_installer_major_upgrades_disallowed_array[@]}"; do
				[[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -lt $(echo "${mdmclient_macos_msu_major_upgrade_array[0]}" | awk -F ',' '{print $3}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') ]] && log_super "Disallowed: macOS major upgrade installer is available but not allowed: $(echo "${macos_installer_item}" | sed -e 's/Title://' -e 's/,Version:/ /' -e 's/,Build:/-/' -e 's/,.*//')"
			done
		fi
	fi
	if [[ "${mdmclient_macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${mdmclient_macos_installer_target}" == "FALSE" ]] && [[ "${mdmclient_macos_msu_minor_update_target}" == "FALSE" ]] && [[ ${#mdmclient_macos_msu_minor_update_array[@]} -gt 0 ]]; then
		log_super "Disallowed: \"over-the-air\" macOS minor update is available but not allowed: $(echo "${mdmclient_macos_msu_minor_update_array[0]}" | sed -e 's/Title://' -e 's/,Build:/-/' -e 's/,.*//')"
		[[ "${macos_installer_minor_update_target}" != "FALSE" ]] && log_super "Warning: The --install-macos-minor-version-target=${install_macos_minor_version_target} option is forcing the workflow to target an older macOS installer (as opposed to a more efficient \"over-the-air\" macOS minor update)."
	fi
	if [[ ${#macos_installer_minor_updates_disallowed_array[@]} -gt 0 ]]; then
		for macos_installer_item in "${macos_installer_minor_updates_disallowed_array[@]}"; do
			{ [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -gt $macos_version_normalized ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -lt $(echo "${mdmclient_macos_msu_minor_update_array[0]}" | awk -F ',' '{print $3}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') ]]; } && log_super "Disallowed: macOS minor update installer is available but not allowed: $(echo "${macos_installer_item}" | sed -e 's/Title://' -e 's/,Version:/ /' -e 's/,Build:/-/' -e 's/,.*//')"
		done
	fi
	
	# If (no errors) check for older targeted macOS version betas plus Jamf API beta limitation.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ "${macos_beta_program}" == "TRUE" ]] && [[ "${workflow_macos_auth}" == "JAMF" ]] && { [[ -n "${install_macos_major_version_target}" ]] || [[ -n "${install_macos_minor_version_target}" ]]; }; then
	...
	# If there is a ${macos_installer_major_upgrade_target} or ${macos_installer_minor_update_target} then set the remaining individual parameters.
	if [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_installer_minor_update_target}" != "FALSE" ]]; then
		local macos_installer_target
		[[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] && macos_installer_target="${macos_installer_major_upgrade_target}"
		[[ "${macos_installer_minor_update_target}" != "FALSE" ]] && macos_installer_target="${macos_installer_minor_update_target}"
		...
		[[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] && log_super "Target: macOS major upgrade installer ${macos_installer_title} ${macos_installer_version}-${macos_installer_build} which is a ${macos_installer_size}GB download."
		[[ "${macos_installer_minor_update_target}" != "FALSE" ]] && log_super "Target: macOS minor update installer ${macos_installer_title} ${macos_installer_version}-${macos_installer_build} which is a ${macos_installer_size}GB download."
		
		# No longer necessary to continue this function if the target will be the macOS installer.
		if [[ "${install_prioritize_non_restart_updates}" != "TRUE" ]] || { [[ "${install_prioritize_non_restart_updates}" == "TRUE" ]] && [[ ${#mdmclient_non_system_array[@]} -eq 0 ]]; }; then
			IFS="${previous_ifs}"
			set_pref_main "LastSuccessfulCheckDate" "$(date +%Y-%m-%d:%H:%M:%S)"
			return 0
		fi
	...
	# If (no errors) there is not MSU target then it's no longer necessary to continue this function.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ "${mdmclient_macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${mdmclient_macos_msu_minor_update_target}" == "FALSE" ]]; then
		set_pref_main "LastSuccessfulCheckDate" "$(date +%Y-%m-%d:%H:%M:%S)"
		return 0
	fi
	...
	# If a ${macos_msu_major_upgrade_target} is selected then this function will also set ${macos_msu_label}, ${macos_msu_title}, ${macos_msu_version}, ${macos_msu_build}, and ${macos_msu_size }.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ "${mdmclient_macos_msu_major_upgrade_target}" != "FALSE" ]] && [[ ${#macos_msu_array[@]} -gt 0 ]]; then
		...
		for macos_msu_item in "${macos_msu_array[@]}"; do
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_item is: ${macos_msu_item}"
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_major_upgrade_target is: ${macos_msu_major_upgrade_target}"
		...
		if [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]]; then
			...
			# No longer necessary to continue this function if the target will be the macOS major upgrade.
			if [[ "${install_prioritize_non_restart_updates}" != "TRUE" ]] || { [[ "${install_prioritize_non_restart_updates}" == "TRUE" ]] && [[ ${#mdmclient_non_system_array[@]} -eq 0 ]]; }; then
				IFS="${previous_ifs}"
				set_pref_main "LastSuccessfulCheckDate" "$(date +%Y-%m-%d:%H:%M:%S)"
				return 0
	...
	# If a ${macos_msu_minor_update_target} is selected then this function will also set ${macos_msu_label}, ${macos_msu_title}, ${macos_msu_version}, ${macos_msu_build}, and ${macos_msu_size }.
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_minor_update_target is: ${macos_msu_minor_update_target}"
		...
		if [[ "${macos_msu_minor_update_target}" != "FALSE" ]]; then
			...
			# No longer necessary to continue this function if the target will be the macOS minor update.
			if [[ "${install_prioritize_non_restart_updates}" != "TRUE" ]] || { [[ "${install_prioritize_non_restart_updates}" == "TRUE" ]] && [[ ${#mdmclient_non_system_array[@]} -eq 0 ]]; }; then
				IFS="${previous_ifs}"
				set_pref_main "LastSuccessfulCheckDate" "$(date +%Y-%m-%d:%H:%M:%S)"
				return 0
	...
	# If (no errors) then evaluate the ${non_system_msu_array[]} and set ${non_system_msu_targets} and ${non_system_msu_safari_target}.
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ ${#mdmclient_non_system_array[@]} -gt 0 ]] && [[ ${#non_system_msu_array[@]} -gt 0 ]]; then
		...
		if [[ ${#non_system_msu_array[@]} -eq $(echo "${mdmclient_non_system_array[*]}" | awk -F ',' '{print $1}' | uniq | wc -l | xargs) ]];then
			...
			for array_index in "${!non_system_msu_array[@]}"; do
				...
				[[ $(echo "${non_system_msu_array[array_index]}" | grep -c 'Safari') -ge 1 ]] && non_system_msu_safari_target=$(echo "${non_system_msu_array[array_index]}" | awk -F ',' '{print $4}' | sed -e 's/Version://')
				...
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: non_system_msu_labels_array[${array_index}] is: ${non_system_msu_labels_array[array_index]}"
			done
		else
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: non_system_msu_targets is: ${non_system_msu_targets}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: non_system_msu_safari_target is: ${non_system_msu_safari_target}"
...
# This function checks for a downloaded and prepared softwareupdate macOS update/upgrade cache and sets ${macos_msu_downloaded_version}, ${macos_msu_downloaded_version_extra} and ${macos_msu_downloaded_build} accordingly.
check_macos_msu_download() {
	macos_msu_downloaded_version="FALSE"
	macos_msu_downloaded_version_extra="FALSE"
	macos_msu_downloaded_build="FALSE"
	local update_asset_attributes
	update_asset_attributes=$(defaults read /System/Volumes/Update/Update update-asset-attributes 2>&1)
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: update_asset_attributes is:\n${update_asset_attributes}"
	if [[ ! $(echo "${update_asset_attributes}" | grep -c 'does not exist') -gt 0 ]]; then
		macos_msu_downloaded_version=$(echo "${update_asset_attributes}" | grep -w 'OSVersion' | awk -F '"' '{print $2;}')
		if [[ $macos_version_major -ge 13 ]]; then
			macos_msu_downloaded_version_extra=$(echo "${update_asset_attributes}" | grep -w 'ProductVersionExtra' | awk -F '"' '{print $2;}')
			[[ -n "${macos_msu_downloaded_version_extra}" ]] && macos_msu_downloaded_version="${macos_msu_downloaded_version} ${macos_msu_downloaded_version_extra}"
		fi
		macos_msu_downloaded_build=$(echo "${update_asset_attributes}" | grep -w 'Build' | awk '{print substr($3, 1, length($3)-1)}')
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_downloaded_version is: ${macos_msu_downloaded_version}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_downloaded_version_extra is: ${macos_msu_downloaded_version_extra}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_downloaded_build is: ${macos_msu_downloaded_build}"
}

# Returns the macOS marketing name, version, and build that will be installed given a macOS installer application path ${1}.
get_macos_installer_info() {
	[[ -d "/Volumes/Shared Support" ]] && diskutil unmount force "/Volumes/Shared Support" >/dev/null 2>&1
	if [[ -f "${1}/Contents/SharedSupport/SharedSupport.dmg" ]]; then
		local hidiutil_response
		hidiutil_response=$(hdiutil attach -quiet -noverify -nobrowse "${1}/Contents/SharedSupport/SharedSupport.dmg" 2>&1)
		sleep 1
		if [[ -d "/Volumes/Shared Support" ]]; then
			if [[ -f "/Volumes/Shared Support/com_apple_MobileAsset_MacSoftwareUpdate/com_apple_MobileAsset_MacSoftwareUpdate.xml" ]]; then
				local macos_installer_version_downloaded
				macos_installer_version_downloaded=$(/usr/libexec/PlistBuddy -c "Print :Assets:0:OSVersion" "/Volumes/Shared Support/com_apple_MobileAsset_MacSoftwareUpdate/com_apple_MobileAsset_MacSoftwareUpdate.xml")
				...
				if [[ -n "${macos_installer_version_downloaded}" ]] && [[ -n "${macos_installer_build_downloaded}" ]]; then
					[[ -d "/Volumes/Shared Support" ]] && diskutil unmount force "/Volumes/Shared Support" >/dev/null 2>&1
					echo "$(basename "${1}" | sed -e 's/Install //g' -e 's/\..*//g') ${macos_installer_version_downloaded}-${macos_installer_build_downloaded}" && return 0
				else
					log_super "Error: Unable to resolve information for macOS installer located at: ${1}"
					[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_version_downloaded is: ${macos_installer_version_downloaded}"
					[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_build_downloaded is: ${macos_installer_build_downloaded}"
				fi
			else
				log_super "Error: Unable to locate macOS installer com_apple_MobileAsset_MacSoftwareUpdate.xml file for validation."
			fi
		else
			log_super "Error: Unable to mount macOS installer SharedSupport.dmg for validation."
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: hidiutil_response is:\n${hidiutil_response}"
		fi
	else
		log_super "Error: Unable to locate macOS installer at: ${1}"
	fi
	return 1
}
...
check_macos_installer_download() {
	...
	if [[ "$(get_macos_installer_info "/Applications/Install ${macos_installer_title}.app")" != "${macos_installer_title} ${macos_installer_version}-${macos_installer_build}" ]]; then
			log_super "Warning: Downloaded macOS installer does not match the target macOS installer."
			check_macos_installer_error="TRUE"
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: check_macos_installer_error is: ${check_macos_installer_error}"
	...
	if [[ "${check_macos_installer_error}" == "FALSE" ]]; then
		...
		if [[ $(echo "${startosinstall_response}" | grep -c 'Usage: startosinstall') -eq 0 ]]; then
			log_super "Warning: Downloaded macOS installer failed Gatekeeper validation."
			check_macos_installer_error="TRUE"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: startosinstall_response is:\n${startosinstall_response}"
		fi
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: check_macos_installer_error is: ${check_macos_installer_error}"
}

# Delete any unneeded or incorrect version macOS installers.
delete_unneeded_macos_installers() {
	local previous_ifs
	previous_ifs="${IFS}"
	IFS=$'\n'
	local macos_installer_local_paths_array
	macos_installer_local_paths_array=($(mdfind kind:app -name "Install macOS" 2>/dev/null))
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_local_paths_array is:\n${macos_installer_local_paths_array[*]}"
	if [[ -n "${macos_installer_local_paths_array[*]}" ]]; then
		for macos_installer_path in "${macos_installer_local_paths_array[@]}"; do
			if [[ $(echo "${macos_installer_path}" | grep -c '/Volumes/') -gt 0 ]] || { [[ $(echo "${macos_installer_path}" | grep -c '/Users/') -gt 0 ]] && [[ $(echo "${macos_installer_path}" | grep -c '/Users/.*/Applications/') -eq 0 ]] && [[ $(echo "${macos_installer_path}" | grep -c '/Users/.*/Desktop/') -eq 0 ]] && [[ $(echo "${macos_installer_path}" | grep -c '/Users/.*/Downloads/') -eq 0 ]]; }; then
				...
				if [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]] && [[ "${macos_installer_minor_update_target}" == "FALSE" ]]; then
					if [[ "${test_mode}" == "TRUE" ]]; then # Test mode.
				...
				elif [[ "${macos_installer_path}" != "/Applications/Install ${macos_installer_title}.app" ]] || [[ "$(get_macos_installer_info "${macos_installer_path}")" != "${macos_installer_title} ${macos_installer_version}-${macos_installer_build}" ]]; then
					if [[ "${test_mode}" == "TRUE" ]]; then # Test mode.
						log_super "Test Mode: Found incorrect version target macOS installer at: ${macos_installer_path}"
					else # Normal workflow.
...
# This function determines which macOS updates, upgrades, or installers have been downloaded and validates any previously downloaded macOS updates, upgrades, or installers and sets ${macos_installer_download_required}, ${macos_msu_download_required}, and ${macos_msu_automatic_download_deferral} accordingly.
workflow_check_download_status() {
	...
	if [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_installer_minor_update_target}" != "FALSE" ]]; then
		...
		macos_installer_download=$(get_pref "MacOSInstallerDownloaded")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_download is: ${macos_installer_download}"
		if [[ -d "/Applications/Install ${macos_installer_title}.app" ]]; then
			...
			set_pref_main "MacOSInstallerDownloaded" "${macos_installer_title} ${macos_installer_version}-${macos_installer_build}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_download_required is: ${macos_installer_download_required}"
	
	# If a softwareupdate is targeted then evaluate if there is a cached macOS update/upgrade.
	if [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_msu_minor_update_target}" != "FALSE" ]]; then
		...
		check_macos_msu_download
		macos_msu_label_downloaded=$(get_pref "MacOSMSULabelDownloaded")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_label_downloaded is: ${macos_msu_label_downloaded}"
		[[ $(get_pref "WorkflowDownloadMacOSAuthRequired") == "TRUE" ]] && workflow_download_macos_auth_required="TRUE"
		...
		# If an MSU macOS version is cached then determine if it's valid.
		if [[ "${macos_msu_downloaded_version}" != "FALSE" ]]; then
			{ [[ "${macos_msu_downloaded_version}" == "${macos_msu_version}" ]] && [[ -z "${macos_msu_label_downloaded}" ]]; } && macos_msu_label_downloaded="${macos_msu_label}" && set_pref_main "MacOSMSULabelDownloaded" "${macos_msu_label}"
			[[ "${macos_msu_downloaded_version}" != "${macos_msu_version}" ]] && log_super "Warning: Previously downloaded macOS update/upgrade version ${macos_msu_downloaded_version} doesn't match workflow target version ${macos_msu_version}." && macos_msu_downloaded_error="TRUE"
		else # No msu download is prepared.
			macos_msu_downloaded_error="TRUE"
			[[ -n "${macos_msu_label_downloaded}" ]] && log_super "Warning: Previously downloaded macOS update/upgrade is no longer valid."
		fi
		if [[ "${macos_msu_downloaded_error}" == "TRUE" ]]; then
			macos_msu_download_required="TRUE"
			unset macos_msu_label_downloaded
			delete_pref_main "MacOSMSULabelDownloaded"
		fi
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_download_required is: ${macos_msu_download_required}"
...
# Download macOS update or upgrade via softwareupdate command, and also save responses to ${SUPER_LOG}, ${MSU_WORKFLOW_LOG}, and ${SUPER_MAIN_PLIST}.
download_macos_msu() {
	...
	if [[ "${download_macos_msu_start_error}" == "FALSE" ]] && [[ "${download_macos_msu_start_timeout}" == "FALSE" ]] && [[ "${download_macos_msu_timeout_error}" == "FALSE" ]]; then	
		...
		if [[ "${macos_msu_title}" == "${macos_msu_title_downloaded}" ]]; then
			...
			check_macos_msu_download
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_version is: ${macos_msu_version}"
			[[ "${macos_msu_downloaded_version}" == "${macos_msu_version}" ]] && download_macos_msu_validation_error="FALSE"
		fi
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: download_macos_msu_title_error is: ${download_macos_msu_title_error}"
	...
	if [[ "${download_macos_msu_start_error}" == "FALSE" ]] || [[ "${download_macos_msu_start_timeout}" == "FALSE" ]] || [[ "${download_macos_msu_timeout_error}" == "FALSE" ]] || [[ "${download_macos_msu_title_error}" == "FALSE" ]] || [[ "${download_macos_msu_validation_error}" == "FALSE" ]]; then
		...
		if [[ "${download_macos_msu_start_error}" == "CONNECT" ]]; then
			...
			[[ "${mac_cpu_architecture}" == "arm64" ]] && set_pref_main "WorkflowDownloadMacOSAuthRequired" "TRUE"
			...
			if [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]]; then
				log_msu "Error: Downloaded macOS major upgrade version of ${macos_msu_downloaded_version} doesn't match expected version ${macos_msu_version}."
				log_super "Error: Downloaded macOS major upgrade version of ${macos_msu_downloaded_version} doesn't match expected version ${macos_msu_version}."
			else # "${macos_msu_minor_update_target}" != "FALSE"
				log_msu "Error: Downloaded macOS minor update version of ${macos_msu_downloaded_version} doesn't match expected version ${macos_msu_version}."
				log_super "Error: Downloaded macOS mintor update version of ${macos_msu_downloaded_version} doesn't match expected version ${macos_msu_version}."
			fi
		fi
...
download_macos_installer() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: download_macos_installer_timeout_seconds is: ${download_macos_installer_timeout_seconds}"
	while read -t "${download_macos_installer_timeout_seconds}" -r log_line; do
		...
		if [[ $(echo "${log_line}" | grep -c 'No macOS Installer found') -gt 0 ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: download_macos_installer_complete_percent is: ${download_macos_installer_complete_percent}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: download_macos_installer_start_error is: ${download_macos_installer_start_error}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: download_macos_installer_start_timeout is: ${download_macos_installer_start_timeout}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: download_macos_installer_timeout_error is: ${download_macos_installer_timeout_error}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: download_macos_installer_app_error is: ${download_macos_installer_app_error}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: download_macos_installer_error is: ${download_macos_installer_error}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_download_required is: ${macos_installer_download_required}"
}
...
workflow_download_macos() {
	...
	# If downloads are required then check for possible automatic download deferral and available storage.
	if [[ "${macos_installer_download_required}" == "TRUE" ]] || [[ "${macos_msu_download_required}" == "TRUE" ]]; then
		# If a macOS automatic download should be possible then automatically defer.
		if [[ "${macos_msu_automatic_download_deferral}" == "TRUE" ]]; then
			if [[ "${workflow_install_now_active}" != "TRUE" ]] && [[ "${deadline_date_status}" == "FALSE" ]] && [[ "${deadline_days_status}" == "FALSE" ]] && [[ "${deadline_count_status}" == "FALSE" ]]; then
				log_super "Warning: Automatic macOS minor update downloads are enabled and should be completed by the system, trying again in ${deferral_timer_minutes} minutes."
				log_status "Pending: Automatic macOS minor update downloads are enabled and should be completed by the system, trying again in ${deferral_timer_minutes} minutes."
				set_auto_launch_deferral
			else
				[[ "${workflow_install_now_active}" == "TRUE" ]] && log_super "Warning: Ignoring automatic macOS minor update download deferral due to current install now workflow."
				{ [[ "${deadline_date_status}" != "FALSE" ]] || [[ "${deadline_days_status}" != "FALSE" ]] || [[ "${deadline_count_status}" != "FALSE" ]]; } && log_super "Warning: Ignoring automatic macOS minor update download deferral due to current deadline workflow."
			fi
		fi
		...
		{ [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_installer_minor_update_target}" != "FALSE" ]]; } && log_super "Status: Previously downloaded ${macos_installer_title} ${macos_installer_version}-${macos_installer_build} installer is available at: /Applications/Install ${macos_installer_title}.app"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_download_macos_error is: ${workflow_download_macos_error}"
	
	# If (no errors) and ${test_mode} then simulate dialogs and notifications for potential user authenticated download dialog.
	if [[ "${workflow_download_macos_error}" == "FALSE" ]] && [[ "${test_mode}" == "TRUE" ]] && [[ $macos_version_major -ge 14 ]] && [[ "${mac_cpu_architecture}" == "arm64" ]] && [[ "${workflow_macos_auth}" == "USER" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_password_policies is: ${current_user_password_policies}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: current_user_system_password_policies is: ${current_user_system_password_policies}"
		if [[ "${current_user_password_policies}" == "TRUE" ]] || [[ "${current_user_system_password_policies}" == "TRUE" ]]; then
			...
			set_pref_main "WorkflowDownloadMacOSAuthRequired" "TRUE"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_download_macos_auth_required is: ${workflow_download_macos_auth_required}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_install_now_active is: ${workflow_install_now_active}"
	...
	if [[ "${workflow_download_macos_error}" == "FALSE" ]] && [[ "${macos_msu_download_required}" == "TRUE" ]] && [[ "${workflow_download_macos_auth_required}" == "TRUE" ]]; then
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_macos_auth is: ${workflow_macos_auth}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_mdm_failover_to_user_status is: ${auth_mdm_failover_to_user_status}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_credential_failover_to_user is: ${auth_credential_failover_to_user}"
		...
		if [[ "${workflow_macos_auth}" == "JAMF" ]]; then
			...
			if [[ "${auth_error_mdm}" == "TRUE" ]]; then
				if [[ "${auth_mdm_failover_to_user_status}" == "TRUE" ]] || [[ $(echo "${auth_mdm_failover_to_user}" | grep -c 'ERROR') -gt 0 ]] || { [[ "${workflow_install_now_active}" == "TRUE" ]] && [[ $(echo "${auth_mdm_failover_to_user}" | grep -c 'INSTALLNOW') -gt 0 ]]; }; then
		...
		if [[ "${workflow_download_macos_error}" == "FALSE" ]] && [[ "${workflow_macos_auth}" != "FALSE" ]]; then
			if [[ "${workflow_macos_auth}" == "LOCAL" ]] || [[ "${workflow_macos_auth}" == "JAMF" ]]; then
				...
				if [[ "${auth_error_saved}" == "TRUE" ]]; then
					if [[ "${auth_credential_failover_to_user}" == "TRUE" ]] || [[ "${auth_ask_user_to_save_password}" == "TRUE" ]]; then
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_download_macos_error is: ${workflow_download_macos_error}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_download_macos_check_user is: ${workflow_download_macos_check_user}"
...
install_non_system_msu() {
	...
	previous_ifs="${IFS}"
	IFS=$' '
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: non_system_msu_labels_array is:\n${non_system_msu_labels_array[*]}"
	...
	local install_msu_pid
	if [[ $macos_version_major -ge 12 ]]; then
		if [[ "${current_user_account_name}" == "FALSE" ]]; then
			sudo -i softwareupdate --install "${non_system_msu_labels_array[@]}" --force --agree-to-license >> "${MSU_WORKFLOW_LOG}" 2>&1 &
			install_msu_pid="$!"
		else # Local user is logged in.
			launchctl asuser "${current_user_id}" sudo -u "${current_user_account_name}" softwareupdate --install "${non_system_msu_labels_array[@]}" --force --agree-to-license >> "${MSU_WORKFLOW_LOG}" 2>&1 &
			install_msu_pid="$!"
		fi
	else # macOS 11
		softwareupdate --install "${non_system_msu_labels_array[@]}" --force --agree-to-license >> "${MSU_WORKFLOW_LOG}" 2>&1 &
		install_msu_pid="$!"
	fi
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_non_system_msu_timeout_seconds is: ${install_non_system_msu_timeout_seconds}"
	while read -t "${install_non_system_msu_timeout_seconds}" -r log_line; do
		# log_super "Debug Mode: Function ${FUNCNAME[0]}: log_line is:\n${log_line}"
		if [[ $(echo "${log_line}" | grep -c "Can’t connect") -gt 0 ]] || [[ $(echo "${log_line}" | grep -c "Couldn't communicate") -gt 0 ]] || [[ $(echo "${log_line}" | grep -c 'No such update') -gt 0 ]] || [[ $(echo "${log_line}" | grep -c 'No updates are available') -gt 0 ]]; then
	...
	if [[ "${install_non_system_msu_start_timeout}" == "FALSE" ]] && [[ "${install_non_system_msu_start_error}" == "FALSE" ]] && [[ "${install_non_system_msu_timeout_error}" == "FALSE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_non_system_msu_installed_titles_array is:\n${install_non_system_msu_installed_titles_array[*]}"
		...
		kill -9 "${install_msu_pid}" >/dev/null 2>&1
		kick_softwareupdated
	fi
	IFS="${previous_ifs}"
}

# Install only a Safari update via the softwareupdate command, and also save responses to ${SUPER_LOG} and ${MSU_WORKFLOW_LOG}.
install_safari_update_msu() {
	install_safari_update_msu_error="TRUE"
	log_super "softwareupdate: Starting Safari ${non_system_msu_safari_target} update installation workflow, check ${MSU_WORKFLOW_LOG} for more detail."
	log_status "Running: softwareupdate: Starting Safari ${non_system_msu_safari_target} update installation workflow."
	log_msu "**** S.U.P.E.R.M.A.N. ${SUPER_VERSION} - INSTALL SAFARI ${non_system_msu_safari_target} UPDATE VIA SOFTWAREUPDATE START ****"
	
	# The update process is backgrounded and is watched via a while loop later on. Also note the different requirements between macOS versions.
	local install_msu_pid
	if [[ $macos_version_major -ge 12 ]]; then
		if [[ "${current_user_account_name}" == "FALSE" ]]; then
			sudo -i softwareupdate --install --safari-only --force --agree-to-license >> "${MSU_WORKFLOW_LOG}" 2>&1 &
			install_msu_pid="$!"
		else # Local user is logged in.
			launchctl asuser "${current_user_id}" sudo -u "${current_user_account_name}" softwareupdate --install --safari-only --force --agree-to-license >> "${MSU_WORKFLOW_LOG}" 2>&1 &
			install_msu_pid="$!"
		fi
	else # macOS 11
		softwareupdate --install --safari-only --force --agree-to-license >> "${MSU_WORKFLOW_LOG}" 2>&1 &
		install_msu_pid="$!"
	fi
	
	# Watch ${MSU_WORKFLOW_LOG} while waiting for the softwareupdate installation workflow to complete.
	# Note this while read loop has a timeout based on ${TIMEOUT_START_SECONDS} then changes to ${TIMEOUT_NON_SYSTEM_MSU_SECONDS}.
	local install_safari_update_msu_start_timeout
	install_safari_update_msu_start_timeout="TRUE"
	local install_safari_update_msu_start_error
	install_safari_update_msu_start_error="TRUE"
	local install_safari_update_msu_timeout_error
	install_safari_update_msu_timeout_error="TRUE"
	local install_safari_update_msu_timeout_seconds
	install_safari_update_msu_timeout_seconds="${TIMEOUT_START_SECONDS}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_safari_update_msu_timeout_seconds is: ${install_safari_update_msu_timeout_seconds}"
	while read -t "${install_safari_update_msu_timeout_seconds}" -r log_line; do
		# log_super "Debug Mode: Function ${FUNCNAME[0]}: log_line is:\n${log_line}"
		if [[ $(echo "${log_line}" | grep -c "Can’t connect") -gt 0 ]] || [[ $(echo "${log_line}" | grep -c "Couldn't communicate") -gt 0 ]] || [[ $(echo "${log_line}" | grep -c 'No such update') -gt 0 ]] || [[ $(echo "${log_line}" | grep -c 'No updates are available') -gt 0 ]]; then
			install_safari_update_msu_start_timeout="FALSE"
			break
		elif [[ $(echo "${log_line}" | grep -c 'Downloading') -gt 0 ]]; then
			log_super "softwareupdate: Safari ${non_system_msu_safari_target} update is downloading..."
			log_msu "**** TIMESTAMP ****"
			install_safari_update_msu_timeout_seconds="${TIMEOUT_NON_SYSTEM_MSU_SECONDS}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_safari_update_msu_timeout_seconds is: ${install_safari_update_msu_timeout_seconds}"
			install_safari_update_msu_start_timeout="FALSE"
			install_safari_update_msu_start_error="FALSE"
		elif [[ $(echo "${log_line}" | grep -c 'Downloaded') -gt 0 ]]; then
			log_super "softwareupdate: Safari ${non_system_msu_safari_target} update download complete."
			install_safari_update_msu_timeout_seconds="${TIMEOUT_NON_SYSTEM_MSU_SECONDS}"
			log_msu "**** TIMESTAMP ****"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_safari_update_msu_timeout_seconds is: ${install_safari_update_msu_timeout_seconds}"
			install_safari_update_msu_start_timeout="FALSE"
			install_safari_update_msu_start_error="FALSE"
		elif [[ $(echo "${log_line}" | grep -c 'Done with Safari') -gt 0 ]]; then
			log_super "softwareupdate: Safari ${non_system_msu_safari_target} update installed."
			log_msu "**** TIMESTAMP ****"
			install_safari_update_msu_start_timeout="FALSE"
			install_safari_update_msu_start_error="FALSE"
		elif [[ $(echo "${log_line}" | grep -c 'Done.') -gt 0 ]]; then
			log_msu "**** TIMESTAMP ****"
			install_safari_update_msu_timeout_error="FALSE"
			break
		fi
	done < <(tail -n1 -F "${MSU_WORKFLOW_LOG}")
	
	# If the softwareupdate installation workflow completed, then validate and collect information.
	if [[ "${install_safari_update_msu_start_timeout}" == "FALSE" ]] && [[ "${install_safari_update_msu_start_error}" == "FALSE" ]] && [[ "${install_safari_update_msu_timeout_error}" == "FALSE" ]]; then
		log_msu "**** S.U.P.E.R.M.A.N. ${SUPER_VERSION} - INSTALL SAFARI ${non_system_msu_safari_target} UPDATE VIA SOFTWAREUPDATE COMPLETED ****"
		install_safari_update_msu_error="FALSE"
	else # The softwareupdate installation workflow failed.
		log_msu "**** S.U.P.E.R.M.A.N. ${SUPER_VERSION} - INSTALL SAFARI ${non_system_msu_safari_target} UPDATE VIA SOFTWAREUPDATE FAILED ****"
		if [[ "${install_safari_update_msu_start_timeout}" == "TRUE" ]]; then
			log_msu "Error: Installation of Safari ${non_system_msu_safari_target} update failed to start after waiting for ${install_safari_update_msu_timeout_seconds} seconds."
			log_super "Error: Installation of Safari ${non_system_msu_safari_target} update failed to start after waiting for ${install_safari_update_msu_timeout_seconds} seconds."
		elif [[ "${install_safari_update_msu_start_error}" == "TRUE" ]]; then
			log_msu "Error: Unable to reach macOS software update server."
			log_super "Error: Unable to reach macOS software update server."
		elif [[ "${install_safari_update_msu_timeout_error}" == "TRUE" ]]; then
			log_msu "Error: Installation of Safari ${non_system_msu_safari_target} update failed, as indicated by no progress after waiting for ${install_safari_update_msu_timeout_seconds} seconds."
			log_super "Error: Installation of Safari ${non_system_msu_safari_target} update failed, as indicated by no progress after waiting for ${install_safari_update_msu_timeout_seconds} seconds."
		fi
		kill -9 "${install_msu_pid}" >/dev/null 2>&1
		kick_softwareupdated
	fi
...
install_macos_msu() {
	...
	if [[ "${test_mode}" == "TRUE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Calling reset_workflow() function."
		reset_workflow
		return 0
	fi
	...
	while read -t "${install_macos_msu_timeout_seconds}" -r log_line; do
		...
		if [[ $(echo "${log_line}" | grep -c "Can’t connect") -gt 0 ]] || [[ $(echo "${log_line}" | grep -c "Couldn't communicate") -gt 0 ]] || [[ $(echo "${log_line}" | grep -c 'No such update') -gt 0 ]] || [[ $(echo "${log_line}" | grep -c 'Failed to download') -gt 0 ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_msu_complete_percent is: ${install_macos_msu_complete_percent}"
...
install_macos_app() {
	...
	if [[ "${test_mode}" == "TRUE" ]]; then
		[[ "${workflow_scheduled_install_active}" == "TRUE" ]] && delete_pref_main "WorkflowScheduledInstall"
		log_super "Test Mode: Skipping the macOS insaller workflow."
		if [[ "${current_user_account_name}" != "FALSE" ]]; then
			log_super "Test Mode: Pausing ${test_mode_timeout_seconds} seconds for the macOS insaller preparation notification..."
			sleep "${test_mode_timeout_seconds}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_app_timeout_seconds is: ${install_macos_app_timeout_seconds}"
	while read -t "${install_macos_app_timeout_seconds}" -r log_line; do
		...
		if [[ $(echo "${log_line}" | grep -c 'Preparing to run') -gt 0 ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_app_complete_percent is: ${install_macos_app_complete_percent}"
	...
	if [[ "${install_macos_app_start_error}" == "FALSE" ]] && [[ "${install_macos_app_start_timeout}" == "FALSE" ]] && [[ "${install_macos_app_timeout}" == "FALSE" ]]; then
		...
		if [[ "${install_macos_app_start_error}" == "TRUE" ]]; then
			log_installer "Error: Installation of macOS installer application failed to start."
			log_super "Error: Installation of macOS installer application failed to start."
		elif [[ "${install_macos_app_start_timeout}" == "TRUE" ]]; then
			log_installer "Error: Installation of macOS installer application failed to start preparing after waiting for ${install_macos_app_timeout_seconds} seconds."
			log_super "Error: Installation of macOS installer application failed to start preparing after waiting for ${install_macos_app_timeout_seconds} seconds."
		else # "${install_macos_app_timeout}" == "TRUE"
			log_installer "Error: Installation of macOS installer application failed to prepare, as indicated by no progress after waiting for ${install_macos_app_timeout_seconds} seconds."
			log_super "Error: Installation of macOS installer application failed to prepare, as indicated by no progress after waiting for ${install_macos_app_timeout_seconds} seconds."
		fi
		
		# Handle workflow failure options.
		if [[ "${workflow_install_now_active}" == "TRUE" ]]; then # Install now workflow mode.
			log_installer "Error: Installation of macOS installer application failed, install now workflow can not continue."
			log_super "Error: Installation of macOS installer application failed, install now workflow can not continue."
			log_status "Inactive Error: Installation of macOS installer application failed, install now workflow can not continue."
			[[ "${current_user_account_name}" != "FALSE" ]] && notification_install_now_failed
			exit_error
		else # Default super workflow.
			deferral_timer_minutes=$deferral_timer_error_minutes
			log_installer "Error: Installation of macOS installer application failed, trying again in ${deferral_timer_minutes} minutes."
			log_super "Error: Installation of macOS installer application failed, trying again in ${deferral_timer_minutes} minutes."
			log_status "Pending: Installation of macOS installer application failed, trying again in ${deferral_timer_minutes} minutes."
			[[ "${current_user_account_name}" != "FALSE" ]] && notification_failed
			set_auto_launch_deferral
		fi
	fi
...
push_macos_mdm() {
	...
	if [[ "${test_mode}" == "TRUE" ]] && [[ "${push_macos_mdm_workflow}" == "DOWNLOAD" ]]; then
		...
		push_macos_mdm_download_error="FALSE"
		return 0
	elif [[ "${test_mode}" == "TRUE" ]]; then # [[ "${push_macos_mdm_workflow}" == "INSTALL" ]]
		[[ "${workflow_scheduled_install_active}" == "TRUE" ]] && delete_pref_main "WorkflowScheduledInstall"
	...
	# Start with log and status updates.
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_workflow is: ${push_macos_mdm_workflow}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_download_required is: ${macos_msu_download_required}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_mdm_failover_to_user_status is: ${auth_mdm_failover_to_user_status}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_target_type is: ${push_macos_mdm_target_type}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_target_version is: ${push_macos_mdm_target_version}"
	
	# Validate Jamf Pro API token and update workflow.
	check_jamf_api_access_token
	check_jamf_api_update_workflow
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_error_jamf is: ${auth_error_jamf}"
	...
	if [[ "${auth_error_jamf}" == "FALSE" ]]; then
		...
		if [[ "${jamf_api_update_workflow}" == "NEW" ]]; then # Jamf Pro API new managed update workflow.
			...
			if { [[ "${push_macos_mdm_target_type}" == "INSTALLER" ]] || [[ "${push_macos_mdm_target_type}" == "UPGRADE" ]]; } && { [[ "${macos_beta_program}" == "TRUE" ]] || [[ "${macos_major_upgrade_latest}" == "TRUE" ]] || [[ "${macos_minor_update_latest}" == "TRUE" ]]; }; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: jamf_api_update_workflow is: ${jamf_api_update_workflow}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: jamf_api_update_url is: ${jamf_api_update_url}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: jamf_api_update_json is: ${jamf_api_update_json}"
		local curl_response
		curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --output /dev/null --write-out "%{http_code}" --location --request POST "${jamf_api_update_url}" --header "Authorization: Bearer ${jamf_access_token}" --header 'accept: application/json' --header 'content-type: application/json' --data "${jamf_api_update_json}")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: curl_response is:\n${curl_response}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_api_error is: ${push_macos_mdm_api_error}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_start_error is: ${push_macos_mdm_start_error}"
	...
	if [[ "${auth_error_jamf}" == "FALSE" ]] && [[ "${push_macos_mdm_api_error}" == "FALSE" ]] && [[ "${push_macos_mdm_start_error}" == "FALSE" ]]; then
		...
		if [[ "${current_user_account_name}" != "FALSE" ]] && [[ "${push_macos_mdm_workflow}" == "INSTALL" ]]; then
			if [[ "${push_macos_mdm_target_type}" == "INSTALLER" ]]; then
				[[ "${mac_cpu_architecture}" == "arm64" ]] && display_string_prepare_time_estimate="15-20"
				[[ "${mac_cpu_architecture}" == "i386" ]] && display_string_prepare_time_estimate="20-30"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_start_error is: ${push_macos_mdm_start_error}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_start_timeout is: ${push_macos_mdm_start_timeout}"
	...
	if [[ "${auth_error_jamf}" == "FALSE" ]] && [[ "${push_macos_mdm_api_error}" == "FALSE" ]] && [[ "${push_macos_mdm_start_error}" == "FALSE" ]] && [[ "${push_macos_mdm_start_timeout}" == "FALSE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_target_type is: ${push_macos_mdm_target_type}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_timeout_seconds is: ${push_macos_mdm_timeout_seconds}"
		if [[ "${push_macos_mdm_target_type}" == "UPGRADE" ]] || [[ "${push_macos_mdm_target_type}" == "UPDATE" ]]; then
			while read -t "${push_macos_mdm_timeout_seconds}" -r log_line; do
				...
				if [[ $(echo "${log_line}" | grep -c 'phase:PREFLIGHT') -gt 0 ]]; then
					...
					[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_complete_percent is: ${push_macos_mdm_complete_percent}"
			...
			if [[ "${push_macos_mdm_timeout_error}" == "FALSE" ]] && [[ "${current_user_account_name}" != "FALSE" ]]; then
				[[ "${mac_cpu_architecture}" == "arm64" ]] && display_string_prepare_time_estimate="10-15"
				[[ "${mac_cpu_architecture}" == "i386" ]] && display_string_prepare_time_estimate="15-25"
				notification_prepare
			fi
			if [[ "${push_macos_mdm_timeout_error}" == "FALSE" ]]; then
				push_macos_mdm_timeout_error="TRUE"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_timeout_error is: ${push_macos_mdm_timeout_error}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_phase is: ${push_macos_mdm_phase}"
	...
	if [[ "${auth_error_jamf}" == "FALSE" ]] && [[ "${push_macos_mdm_api_error}" == "FALSE" ]] && [[ "${push_macos_mdm_start_error}" == "FALSE" ]] && [[ "${push_macos_mdm_start_timeout}" == "FALSE" ]] && [[ "${push_macos_mdm_timeout_error}" == "FALSE" ]]; then
		...
		if [[ "${push_macos_mdm_workflow}" == "DOWNLOAD" ]]; then
			...
			if [[ $macos_version_major -ge 13 ]]; then
				...
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_mdm_prepared_version_extra is: ${macos_mdm_prepared_version_extra}"
			fi
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_mdm_version_prepared is: ${macos_mdm_version_prepared}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_version is: ${macos_msu_version}"
	...
	if [[ "${auth_error_jamf}" == "TRUE" ]] || [[ "${push_macos_mdm_api_error}" == "TRUE" ]] || [[ "${push_macos_mdm_start_error}" == "TRUE" ]] || [[ "${push_macos_mdm_start_timeout}" == "TRUE" ]] || [[ "${push_macos_mdm_timeout_error}" == "TRUE" ]] || [[ "${push_macos_mdm_download_validation_error}" == "TRUE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: push_macos_mdm_download_error is: ${push_macos_mdm_download_error}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_download_required is: ${macos_msu_download_required}"
		...
		if [[ "${workflow_install_now_active}" == "TRUE" ]]; then # Install now workflow mode.
			if [[ "${auth_mdm_failover_to_user_status}" == "TRUE" ]]; then # User authentication MDM failover option enabled.
				...
				if [[ "${push_macos_mdm_workflow}" == "DOWNLOAD" ]]; then
					...
					unset install_jamf_policy_triggers
...
# This is the install workflow for all non-system software updates
workflow_install_non_system_msu() {
	...
	# Install non-system updates and check to make sure all updates are complete.
	install_non_system_msu
	check_software_status_required="TRUE"
	workflow_check_software_status
	
	# If we had installation failures or there are still more non-system updates, then try again.
	...
	if [[ "${install_non_system_msu_error}" == "FALSE" ]] && [[ "${non_system_msu_targets}" == "FALSE" ]]; then
		log_super_audit "Status: Completed installation of all non-system macOS software updates!"
		killall -9 "IBM Notifier" "IBM Notifier Popup" >/dev/null 2>&1
		# For computers managed via Jamf Pro, submit inventory.
		if [[ $jamf_version_normalized != "FALSE" ]]; then
			if [[ "${auth_error_jamf}" != "TRUE" ]]; then
		...
		if [[ "${workflow_install_now_active}" == "TRUE" ]]; then
			...
			{ [[ "${current_user_account_name}" != "FALSE" ]] && [[ "${workflow_install_now_suppress_notifications}" != "TRUE" ]]; } && notification_install_now_failed
			exit_error
		else
			deferral_timer_minutes=$deferral_timer_error_minutes
...
# This is the install workflow for the Safari software update.
workflow_install_safari_update_msu() {
	{ [[ "${current_user_account_name}" != "FALSE" ]] && [[ "${workflow_target_safari_active}" == "TRUE" ]]; } && notification_safari_update
	
	# If ${test_mode} then it's not necessary to continue this function.
	if [[ "${test_mode}" == "TRUE" ]]; then
		log_super "Test Mode: Skipping the install Safari update workflow."
		log_super "Test Mode: Pausing ${test_mode_timeout_seconds} seconds for the Safari update notification..."
		sleep "${test_mode_timeout_seconds}"
		killall -9 "IBM Notifier" "IBM Notifier Popup" >/dev/null 2>&1
		return 0
	fi
	
	# Install Safari update and check to make sure the Safari update is complete.
	install_safari_update_msu
	check_software_status_required="TRUE"
	workflow_check_software_status
	
	# If we had installation failures or there are a Safari update, then try again.
	if [[ "${install_safari_update_msu_error}" == "TRUE" ]] || [[ "${non_system_msu_safari_target}" != "FALSE" ]]; then
		log_super "Warning: Failed to install Safari update, re-trying installation workflow."
		install_safari_update_msu
		check_software_status_required="TRUE"
		workflow_check_software_status
	fi
	
	# Log status of Safari update completion.
	[[ "${workflow_scheduled_install_active}" == "TRUE" ]] && delete_pref_main "WorkflowScheduledInstall"
	if [[ "${install_safari_update_msu_error}" == "FALSE" ]] && [[ "${non_system_msu_safari_target}" == "FALSE" ]]; then
		log_super_audit "Status: Completed installation of Safari update!"
		if [[ "${current_user_account_name}" != "FALSE" ]] && [[ "${workflow_target_safari_active}" == "TRUE" ]]; then
			killall -9 "IBM Notifier" "IBM Notifier Popup" >/dev/null 2>&1
			sudo -u "${current_user_account_name}" open "/Applications/Safari.app" &
		fi
		# For computers managed via Jamf Pro, submit inventory.
		if [[ $jamf_version_normalized != "FALSE" ]]; then
			if [[ "${auth_error_jamf}" != "TRUE" ]]; then
		...
		deferral_timer_minutes=$deferral_timer_error_minutes
		log_super "Warning: Safari update did not complete, trying again in ${deferral_timer_minutes} minutes."
		log_status "Pending: Safari update did not complete, trying again in ${deferral_timer_minutes} minutes."
		if [[ "${current_user_account_name}" != "FALSE" ]] && [[ "${workflow_target_safari_active}" == "TRUE" ]]; then
			notification_failed
			sudo -u "${current_user_account_name}" open "/Applications/Safari.app" &
		fi
...
workflow_install_no_user() {
	...
	if [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_installer_minor_update_target}" != "FALSE" ]] || [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_msu_minor_update_target}" != "FALSE" ]]; then
		...
		# If (no errors) a target is a macOS installer and a download is required.
		if [[ "${workflow_install_macos_no_user_error}" == "FALSE" ]] && { [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_installer_minor_update_target}" != "FALSE" ]]; } && [[ "${macos_installer_download_required}" == "TRUE" ]]; then
			download_macos_installer
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_install_macos_no_user_error is: ${workflow_install_macos_no_user_error}"
		...
		workflow_install_non_system_msu
	elif [[ "${workflow_target}" == "Safari Update" ]]; then
		workflow_install_safari_update_msu
...
workflow_install_active_user() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_install_macos_active_user_error is: ${workflow_install_macos_active_user_error}"
...
workflow_restart_validate() {
	...
	if [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]] && [[ "${macos_installer_minor_update_target}" == "FALSE" ]] && [[ "${macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${macos_msu_minor_update_target}" == "FALSE" ]] && [[ "${non_system_msu_targets}" == "FALSE" ]]; then
		...
		log_super_audit "Warning: Some macOS software updates/upgrades did not complete after last restart, continuing workflow."
	fi
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Main preference file after workflow_restart_validate: ${SUPER_MAIN_PLIST}:\n$(defaults read "${SUPER_MAIN_PLIST}" 2>/dev/null)"
}

# MARK: *** Jamf Pro ***
################################################################################

# Validate connectivity to Jamf Pro service and set ${jamf_version_normalized}, ${jamf_management_url}, and ${jamf_api_url}.
check_jamf_management_framework() {
	jamf_version_normalized="FALSE"
	auth_error_jamf="FALSE"
	if [[ -f "${JAMF_PRO_BINARY}" ]]; then
		jamf_version_number_full=$("${JAMF_PRO_BINARY}" -version | sed -e 's/version=//' -e 's/-.*//')
		jamf_version_normalized=$(echo "${jamf_version_number_full}" | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') # Expected output: 112001
		if [[ "${mac_cpu_architecture}" == "arm64" ]] && [[ $jamf_version_normalized -lt 103800 ]]; then
			log_super "Error: super requires Jamf Pro version 10.38 or later, the currently installed version of Jamf Pro ${jamf_version_number_full} is not supported."
			auth_error_jamf="TRUE"
		elif [[ "${mac_cpu_architecture}" != "arm64" ]] && [[ $jamf_version_normalized -lt 100000 ]]; then
			log_super "Error: super requires Jamf Pro version 10.00 or later, the currently installed version of Jamf Pro ${jamf_version_number_full} is not supported."
			auth_error_jamf="TRUE"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: jamf_version_normalized is: ${jamf_version_normalized}"
	
	# Check for ${auth_jamf_custom_url} and sets ${jamf_api_url} accordingly.
	if [[ $jamf_version_normalized != "FALSE" ]] && [[ "${auth_error_jamf}" != "TRUE" ]] && [[ -n "${auth_jamf_custom_url}" ]]; then
		local curl_response
		curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --head --location "${auth_jamf_custom_url}" | head -n 1)
		if [[ $(echo "${curl_response}" | grep -c 'HTTP') -gt 0 ]] && [[ $(echo "${curl_response}" | grep -c -e '400' -e '40[4-9]' -e '4[1-9][0-9]' -e '5[0-9][0-9]') -eq 0 ]]; then
...
# Use managed preferences or a jamf recon to resolve the computer's ${jamf_computer_id}.
get_jamf_api_computer_id() {
	jamf_computer_id=$(get_pref_managed "AuthJamfComputerID")
	[[ -z "${jamf_computer_id}" ]] && jamf_computer_id=$(get_pref "AuthJamfComputerID")
	# Resolve the computer's Jamf Pro ID via jamf recon.
	if [[ -z "${jamf_computer_id}" ]]; then
		...
		if [[ $jamf_return -eq 0 ]]; then
			...
			set_pref_main "AuthJamfComputerID" "${jamf_computer_id}"
		else
			log_super "Error: Jamf Pro inventory collection failed."
			auth_error_jamf="TRUE"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: jamf_response is:\n${jamf_response}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: jamf_return is: ${jamf_return}"
		fi
	fi
	[[ ! "${jamf_computer_id}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]] && log_super "Error: Unable to resolve Jamf Pro Computer ID." && auth_error_jamf="TRUE"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: jamf_computer_id is: ${jamf_computer_id}"
}
...
get_jamf_api_access_token() {
	...
	if [[ -n "${auth_jamf_client}" ]]; then
		curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --location --request POST "${jamf_api_url}api/oauth/token" --header "Content-Type: application/x-www-form-urlencoded" --data-urlencode "client_id=${auth_jamf_client}" --data-urlencode "grant_type=client_credentials" --data-urlencode "client_secret=${auth_jamf_secret}")
	else # Legacy ${auth_jamf_account} authentication.
		curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --location --request POST "${jamf_api_url}api/v1/auth/token" --user "${auth_jamf_account}:${auth_jamf_password}")
	fi
	if [[ $(echo "${curl_response}" | grep -c 'token') -gt 0 ]]; then
		...
		if [[ -n "${auth_jamf_client}" ]]; then
			...
			auth_error_jamf="TRUE"
			[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_client is: ${auth_jamf_client}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_secret is: ${auth_jamf_secret}"
		else # Legacy ${auth_jamf_account} authentication.
			...
			auth_error_jamf="TRUE"
			[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_account is: ${auth_jamf_account}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_jamf_password is: ${auth_jamf_password}"
		fi
		auth_error_jamf="TRUE"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: jamf_api_url is: ${jamf_api_url}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: curl_response is:\n${curl_response}"
	fi
}
...
send_jamf_api_blank_push() {
	jamf_blank_push_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --output /dev/null --write-out "%{http_code}" --location --request POST "${jamf_api_url}JSSResource/computercommands/command/BlankPush/id/${jamf_computer_id}" --header "Authorization: Bearer ${jamf_access_token}")
}
...
check_jamf_api_credentials() {
	...
	if [[ "${jamf_blank_push_response}" != 201 ]]; then
		...
		auth_error_jamf="TRUE"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: jamf_blank_push_response is:\n${jamf_blank_push_response}"
	fi
}
...
check_jamf_api_access_token() {
	...
	if [[ -n "${jamf_access_token}" ]]; then
		curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --output /dev/null --write-out "%{http_code}" --location --request GET "${jamf_api_url}api/v1/auth" --header "Authorization: Bearer ${jamf_access_token}")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: curl_response is:\n${curl_response}"
	fi
...
check_jamf_api_update_workflow() {
	...
	if [[ $jamf_version_normalized -ge 104800 ]]; then
		local curl_response
		curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --write-out "%{http_code}" --location --request GET "${jamf_api_url}api/v1/managed-software-updates/plans/feature-toggle" --header "Authorization: Bearer ${jamf_access_token}" --header "Content-Type: application/json")
		if [[ $(echo "${curl_response}" | grep -c '200') -gt 0 ]]; then
			...
			auth_error_jamf="TRUE"
			[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: curl_response is: ${curl_response}"
		fi
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: jamf_api_update_workflow is: ${jamf_api_update_workflow}"
}
...
delete_jamf_api_access_token() {
	local curl_response
	curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --output /dev/null --write-out "%{http_code}" --location --request POST --url "${jamf_api_url}api/v1/auth/invalidate-token" --header "Authorization: Bearer ${jamf_access_token}")
	if [[ $curl_response -eq 204 ]]; then
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Jamf Pro API access token successfully invalidated."
		unset jamf_access_token
	elif [[ $curl_response -eq 401 ]]; then
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Jamf Pro API access token already invalid."
		unset jamf_access_token
	else
...
# Install any optional ${install_jamf_policy_triggers}.
run_jamf_policy_triggers() {
	...
	read -r -a jamf_policy_triggers_array <<<"${install_jamf_policy_triggers}"
	for jamf_policy_trigger in "${jamf_policy_triggers_array[@]}"; do
		...
		if [[ "${test_mode}" != "TRUE" ]]; then
			...
			if [[ $jamf_return -ne 0 ]]; then
				...
				log_super_audit "Status: Jamf Pro Policy with Trigger \"${jamf_policy_trigger}\" was successful."
			fi
		else
	...
	if [[ "${jamf_policy_error}" != "TRUE" ]]; then
		log_super_audit "Status: All Jamf Pro Policies completed."
...
set_display_strings_language() {
	...
	#### This code generates the ${display_string_workflow_title} variable to include the appropriate installation workflow title .
	if [[ "${workflow_target}" == "Non-system Software Updates" ]]; then
		display_string_workflow_title="Apple Software Updates"
	elif [[ "${workflow_target}" == "Safari Update" ]]; then
		display_string_workflow_title="Safari ${non_system_msu_safari_target} Update"
	...
	display_string_non_system_updates_body="Required macOS software updates are now installing.\n\nThe computer will not restart but some Apple applications (like Safari) may automatically restart.\n\nThis should only take a few minutes, but please do not restart or sleep the computer as it will interfere with the update process."
	
	#### Language for notification_safari_update(), a non-interactive notification informing the user that the workflow is installing a Safari update.
	display_string_safari_update_title="Installing Safari ${non_system_msu_safari_target} Update (No Restart)"
	display_string_safari_update_body="Required Safari update is now installing and Safari will automatically restart when the update is complete.\n\nThis should only take a few moments, but please do not restart or sleep the computer as it will interfere with the update process."
...
# This also adjusts ${deferral_timer_minutes} given the ${schedule_workflow_active}.
set_deferral_button() {
	# If needed adjust the ${deferral_timer_minutes} for the ${schedule_workflow_active} and also write to log.
	if [[ -n "${schedule_workflow_active}" ]]; then
...
# This also adjusts ${deferral_timer_menu_minutes} given the ${schedule_workflow_active}.
set_deferral_menu() {
	local previous_ifs
	previous_ifs="${IFS}"
	IFS=','
	...
	# If needed adjust the ${deferral_timer_menu_minutes} for the ${schedule_workflow_active}.
	if [[ -n "${schedule_workflow_active}" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_timer_menu_adjusted is: ${deferral_timer_menu_adjusted}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: deferral_time_menu_adjusted_array is: ${deferral_time_menu_adjusted_array[*]}"
	...
	if [[ -n "${display_help_button_string}" ]]; then
		if [[ $(echo "${display_help_button_string}" | grep -c '^http://\|^https://\|^mailto:\|^jamfselfservice://') -gt 0 ]]; then
			if [[ $(echo "${display_help_button_string_option}" | grep -c '^http://\|^https://') -gt 0 ]]; then
				curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --head --location "${display_help_button_string_option}" | head -1)
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: curl_response is: ${curl_response}"
	...
	if [[ -n "${display_warning_button_string}" ]]; then
		if [[ $(echo "${display_warning_button_string}" | grep -c '^http://\|^https://\|^mailto:\|^jamfselfservice://') -gt 0 ]]; then
			if [[ $(echo "${display_warning_button_string_option}" | grep -c '^http://\|^https://') -gt 0 ]]; then
				curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --head --location "${display_warning_button_string_option}" | head -1)
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: curl_response is: ${curl_response}"
				if [[ $(echo "${curl_response}" | grep -c '200') -gt 0 ]] || [[ $(echo "${curl_response}" | grep -c '302') -gt 0 ]]; then
...
open_notification_ibm_notifier() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: ibm_notifier_array is:\n${ibm_notifier_array[*]}"
	...
	notification_return="$?"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: notification_response is: ${notification_response}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: notification_return is: ${notification_return}"
}
...
open_dialog_ibm_notifier() {
	...
	local curl_response
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_content is: ${display_accessory_content}"
	if [[ -n "${display_accessory_content}" ]] && [[ "${dialog_type}" != "ALERT" ]] && [[ "${dialog_type}" != "SCHEDULE" ]]; then
		if [[ $(echo "${display_accessory_content}" | grep -c '^http://\|^https://') -gt 0 ]]; then
			if [[ "${display_accessory_type}" =~ ^TEXTBOX$|^HTMLBOX$|^HTML$ ]]; then
				display_accessory_payload=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --fail --location "${display_accessory_content}")
				...
				curl_response=$(curl --user-agent "${SUPER_USER_AGENT}" --silent --head --location "${display_accessory_content}" | head -1)
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: curl_response is: ${curl_response}"
				if [[ $(echo "${curl_response}" | grep -c '200') -gt 0 ]] || [[ $(echo "${curl_response}" | grep -c '302') -gt 0 ]]; then
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_enabled is: ${display_accessory_enabled}"
	
	# Append any additional display accessory options to the ${ibm_notifier_array}.
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_type is: ${dialog_type}"
	if [[ "${dialog_type}" == "ALERT" ]]; then
		if [[ "${display_notifications_centered_status}" == "TRUE" ]] || [[ "${display_notifications_centered_status}" == "TEMP" ]]; then
	...
	dialog_return="$?"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ "${dialog_type}" != "AUTH" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_response is: ${dialog_response}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ "${dialog_type}" == "AUTH" ]]; } && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_response is: ${dialog_response}"
...
notification_install_now_start() {
	...
	# Handle the ${display_notifications_centered_status} option.
	if [[ "${display_notifications_centered_status}" == "TRUE" ]] || [[ "${display_notifications_centered_status}" == "TEMP" ]]; then
		ibm_notifier_array+=(-buttonless)
...
notification_install_now_up_to_date() {
	set_display_strings_language
	unset display_notifications_centered_status
...
notification_jamf_pro_policy() {
	...
	if [[ "${workflow_scheduled_install_now}" == "TRUE" ]]; then
		log_super "IBM Notifier: Running Jamf Pro Policy scheduled installation notification."
...
notification_prepare() {
	...
	# Variations for the main body text of the preparing update notification.
	if [[ "${workflow_scheduled_install_now}" == "TRUE" ]]; then
		log_super "IBM Notifier: Preparing update/upgrade scheduled installation notification showing a ${display_string_prepare_time_estimate} minute estimate."
...
notification_restart() {
	...
	if [[ "${workflow_scheduled_install_now}" == "TRUE" ]]; then
		log_super "IBM Notifier: Restart scheduled installation notification."
...
# Display a non-interactive notification informing the user that the workflow is installing non-system macOS software updates.
...
# Display a non-interactive notification informing the user that the workflow is installing a Safari update.
notification_safari_update() {
	set_display_strings_language
	log_super "IBM Notifier: Installing Safari update notification."
	ibm_notifier_array=(-type popup -bar_title "${display_string_safari_update_title}" -subtitle "${display_string_safari_update_body}" -icon_path "${display_icon}" -icon_width "${display_icon_size}" -icon_height "${display_icon_size}")
...
dialog_insufficient_storage() {
	...
	if [[ "${workflow_scheduled_install_now}" == "TRUE" ]]; then
		[[ -n "${dialog_timeout_seconds}" ]] && log_super "IBM Notifier: Insufficient storage scheduled installation dialog with a ${dialog_timeout_seconds} second timeout."
		[[ -z "${dialog_timeout_seconds}" ]] && log_super "IBM Notifier: Insufficient storage scheduled installation dialog with no timeout."
	...
	# Manage ${display_unmovable}, ${display_hide_background}, ${display_silently}, and ${display_notifications_centered}.
	{ [[ "${display_unmovable_status}" != "TRUE" ]] && [[ $(echo "${display_unmovable}" | grep -c 'ERROR') -gt 0 ]]; } && display_unmovable_status="TEMP"
	...
	{ [[ "${display_silently_status}" != "TRUE" ]] && [[ $(echo "${display_silently}" | grep -c 'ERROR') -gt 0 ]]; } && display_silently_status="TEMP"
	{ [[ "${display_notifications_centered_status}" != "TRUE" ]] && [[ $(echo "${display_notifications_centered}" | grep -c 'ERROR') -gt 0 ]]; } && display_notifications_centered_status="TEMP"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: storage_ready is: ${storage_ready}"
	
	# Reset temporary ${display_unmovable}, ${display_hide_background}, ${display_silently}, and ${display_notifications_centered}.
	...
	[[ "${display_notifications_centered_status}" == "TEMP" ]] && unset display_notifications_centered_status
...
dialog_power_required() {
	...
	if [[ "${workflow_scheduled_install_now}" == "TRUE" ]]; then
		[[ -n "${dialog_timeout_seconds}" ]] && log_super "IBM Notifier: Power required scheduled installation dialog with a ${dialog_timeout_seconds} second timeout."
		[[ -z "${dialog_timeout_seconds}" ]] && log_super "IBM Notifier: Power required scheduled installation dialog with no timeout."
	...
	# Manage ${display_unmovable}, ${display_hide_background}, ${display_silently}, and ${display_notifications_centered}.
	{ [[ "${display_unmovable_status}" != "TRUE" ]] && [[ $(echo "${display_unmovable}" | grep -c 'ERROR') -gt 0 ]]; } && display_unmovable_status="TEMP"
	{ [[ "${display_hide_background_status}" != "TRUE" ]] && [[ $(echo "${display_hide_background}" | grep -c 'ERROR') -gt 0 ]]; } && display_hide_background_status="TEMP"
	{ [[ "${display_silently_status}" != "TRUE" ]] && [[ $(echo "${display_silently}" | grep -c 'ERROR') -gt 0 ]]; } && display_silently_status="TEMP"
	{ [[ "${display_notifications_centered_status}" != "TRUE" ]] && [[ $(echo "${display_notifications_centered}" | grep -c 'ERROR') -gt 0 ]]; } && display_notifications_centered_status="TEMP"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: POWER_REQUIRED_RECHECK_SECONDS is: ${POWER_REQUIRED_RECHECK_SECONDS}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: power_required_charger_connected is: ${power_required_charger_connected}"
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_power_required_error is: ${dialog_power_required_error}"
}
...
dialog_user_choice() {
	...
	if [[ "${workflow_target}" == "Non-system Software Updates" ]] || [[ "${workflow_target}" == "Safari Update" ]] || [[ "${workflow_target}" == "Jamf Pro Policy Triggers Without Restarting" ]]; then
	...
	# Provide logging for workflow conditions related to the ${scheduled_install_user_choice}.
	if [[ "${scheduled_install_user_choice}" == "TRUE" ]] && { [[ "${workflow_target}" != "Non-system Software Updates" ]] && [[ "${workflow_target}" != "Safari Update" ]] && [[ "${workflow_target}" != "Jamf Pro Policy Triggers Without Restarting" ]] && [[ "${workflow_target}" != "Jamf Pro Policy Triggers With Restart" ]] && [[ "${workflow_target}" != "Restart Without Updates" ]]; } && { [[ "${workflow_auth_error}" == "TRUE" ]] || [[ "${workflow_macos_auth}" == "USER" ]]; } && { [[ ! "${auth_ask_user_to_save_password}" == "TRUE" ]] && [[ "${auth_user_account_saved}" == "FALSE" ]]; }; then
		...
		scheduled_install_user_choice="FALSE"
	fi
	
	# Set the user choice buttons including handling of ${scheduled_install_user_choice}.
	if [[ "${scheduled_install_user_choice}" == "TRUE" ]]; then
	...
	# Manage ${display_unmovable}, ${display_hide_background}, and ${display_silently} options.
	{ [[ "${display_unmovable_status}" != "TRUE" ]] && [[ $(echo "${display_unmovable}" | grep -c 'DIALOG') -gt 0 ]]; } && display_unmovable_status="TEMP"
	{ [[ "${display_hide_background_status}" != "TRUE" ]] && [[ $(echo "${display_hide_background}" | grep -c 'DIALOG') -gt 0 ]]; } && display_hide_background_status="TEMP"
	{ [[ "${display_silently_status}" != "TRUE" ]] && [[ $(echo "${display_silently}" | grep -c 'DIALOG') -gt 0 ]]; } && display_silently_status="TEMP"
	...
	# Reset temporary ${display_unmovable}, ${display_hide_background}, and ${display_silently} options.
	[[ "${display_unmovable_status}" == "TEMP" ]] && unset display_unmovable_status
	[[ "${display_hide_background_status}" == "TEMP" ]] && unset display_hide_background_status
	[[ "${display_silently_status}" == "TEMP" ]] && unset display_silently_status
...
dialog_user_schedule() {
	...
	if [[ "${mac_cpu_architecture}" == "arm64" ]] && [[ "${auth_ask_user_to_save_password}" == "TRUE" ]] && [[ "${auth_user_account_saved}" == "FALSE" ]] && [[ "${workflow_target}" != "Non-system Software Updates" ]] && [[ "${workflow_target}" != "Safari Update" ]] && [[ "${workflow_target}" != "Jamf Pro Policy Triggers Without Restarting" ]] && [[ "${workflow_target}" != "Jamf Pro Policy Triggers With Restart" ]] && [[ "${workflow_target}" != "Restart Without Updates" ]]; then
	...
	display_accessory_datepicker_start_epoch=$((workflow_time_epoch + (DIALOG_USER_SCHEDULE_MINIMUM_SELECTION_MINUTES * 60) + 1))
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_datepicker_start_date is: ${display_accessory_datepicker_start_date}"
	...
	0)
		...
		dialog_user_schedule_minimum_selection_epoch=$((workflow_time_epoch + 121))
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_user_schedule_selection is: ${dialog_user_schedule_selection}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_user_schedule_selection_epoch is: ${dialog_user_schedule_selection_epoch}"
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_user_schedule_minimum_selection_epoch is: ${dialog_user_schedule_minimum_selection_epoch}"
...
dialog_schedule_reminder() {
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_final_reminder is: ${scheduled_install_final_reminder}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_user_choice_active is: ${scheduled_install_user_choice_active}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: scheduled_install_user_choice_adjusted is: ${scheduled_install_user_choice_adjusted}"
	if [[ "${scheduled_install_final_reminder}" == "TRUE" ]]; then
		...
		# Create initial ${ibm_notifier_array} settings for the dialog.
		if [[ "${scheduled_install_user_choice}" == "TRUE" ]] && [[ "${scheduled_install_user_choice_active}" == "TRUE" ]]; then
...
dialog_user_auth() {
	...
	if [[ "${dialog_user_auth_type}" == "DOWNLOAD" ]]; then
		...
		[[ -n "${dialog_timeout_seconds}" ]] && log_super "IBM Notifier: User authentication scheduled installation dialog with a ${dialog_timeout_seconds} second timeout."
		[[ -z "${dialog_timeout_seconds}" ]] && log_super "IBM Notifier: User authentication scheduled installation dialog with no timeout."
	...
	if [[ "${dialog_user_auth_valid}" == "TRUE" ]]; then
		...
		if [[ "${auth_ask_user_to_save_password}" == "TRUE" ]]; then
			...
			[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_user_save_password_response is: ${auth_user_save_password_response}"
			local auth_user_password_keychain
			auth_user_password_keychain=$(launchctl asuser "${current_user_id}" sudo -u "${current_user_account_name}" security find-generic-password -w -a "super_auth_user_password" "/Users/${current_user_account_name}/Library/Keychains/login.keychain" 2>&1)
			[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_local_password is: ${auth_local_password}"
			[[ "${verbose_mode}" == "TRUE" ]] && log_echo "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: auth_user_password_keychain is: ${auth_user_password_keychain}"
		...
		elif [[ "${mdm_enrolled}" == "TRUE" ]] && [[ "${auth_error_bootstrap_token}" == "TRUE" ]] && { [[ "${auth_mdm_failover_to_user_status}" == "TRUE" ]] || [[ $(echo "${auth_mdm_failover_to_user}" | grep -c 'ERROR') -gt 0 ]]; }; then
			...
			if [[ "${bootstrap_token_attempt_fix}" == "TRUE" ]]; then
				...
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: profiles_response is: ${profiles_response}"
				check_bootstrap_token_escrow
			fi
		fi
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: dialog_user_auth_error is: ${dialog_user_auth_error}"
}
...
main() {
	...
	# If restarting from a macOS update/upgrade the restart validation workflow starts before all others.
	if [[ "${workflow_restart_validate_active}" == "TRUE" ]]; then
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Calling reset_workflow() function."
		reset_workflow
		# Handle the ${workflow_reset_super_after_completion_active} or ${alternate_temporary_plist} workflow options.
		if [[ "${workflow_reset_super_after_completion_active}" == "TRUE" ]] || [[ -n "${alternate_temporary_plist}" ]]; then
			if [[ "${workflow_reset_super_after_completion_active}" == "TRUE" ]]; then
				delete_pref "WorkflowResetSuperAfterCompletion"
				set_pref_main "WorkflowResetSuperAfterCompletionNow" "TRUE"
				log_super "Status: Workflow complete, restarting via LaunchDeamon to fully reset super..."
			elif [[ -n "${alternate_temporary_plist}" ]]; then
				delete_pref_main "AlternateTemporaryPlist"
				log_super "Status: Alternate temporary workflow complete, restarting super to reload workflow settings..."
			fi
			restart_super_sleep_seconds=5
			restart_super
		fi
	...
	if [[ "${workflow_disable_update_check}" == "TRUE" ]]; then # Skip software updates/upgrade mode option.
		...
		non_system_msu_targets="FALSE"
		non_system_msu_safari_target="FALSE"
		{ [[ "${workflow_disable_relaunch}" != "TRUE" ]] && [[ -z "${install_jamf_policy_triggers}" ]] && [[ "${workflow_restart_without_updates}" != "TRUE" ]]; } && log_super "Warning: When using the --workflow-disable-update-check option consider also using the --workflow-disable-relaunch option to prevent super from unecissarily re-launching."
		if [[ "${test_mode}" == "TRUE" ]]; then
			log_super "Test Mode: Simulating skip updates workflow."
			if [[ "${install_jamf_policy_triggers_without_restarting}" == "TRUE" ]]; then
				log_super "Test Mode: Simulating install Jamf Pro Policy Triggers without restarting workflow."
			elif [[ "${workflow_restart_without_updates}" == "TRUE" ]]; then
		...
		if [[ "${test_mode}" != "TRUE" ]]; then # Default workflow starts by checking for updates/upgrades.
			...
			macos_msu_major_upgrade_target="FALSE"
			macos_msu_minor_update_target="FALSE"
			if [[ "${workflow_restart_without_updates}" == "TRUE" ]]; then
				log_super "Test Mode: Simulating restart without updates workflow."
			elif [[ "${install_jamf_policy_triggers_without_restarting}" == "TRUE" ]]; then
				log_super "Test Mode: Simulating install Jamf Pro Policy Triggers without restarting workflow."
			elif [[ "${install_non_system_updates_without_restarting}" == "TRUE" ]]; then
				non_system_msu_targets="TRUE"
				log_super "Test Mode: Simulating install non-system updates workflow."
			elif [[ "${install_safari_update_without_restarting}" == "TRUE" ]]; then
				non_system_msu_targets="TRUE"
				non_system_msu_safari_target="Safari 18.x"
				log_super "Test Mode: Simulating install Safari update workflow."
			elif [[ "${install_macos_major_upgrades}" == "TRUE" ]]; then
				if [[ $macos_version_normalized -lt 120300 ]]; then
					macos_installer_major_upgrade_target="Title:${macos_title},Version:${macos_version_major}.${macos_version_minor},Build:${macos_build},Size:14633014395" 
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: non_system_msu_targets is: ${non_system_msu_targets}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: non_system_msu_safari_target is: ${non_system_msu_safari_target}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_prioritize_non_restart_updates is: ${install_prioritize_non_restart_updates}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_jamf_policy_triggers_without_restarting is: ${install_jamf_policy_triggers_without_restarting}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_restart_without_updates is: ${workflow_restart_without_updates}"
	...
	if [[ "${install_prioritize_non_restart_updates}" == "TRUE" ]] && { { [[ "${non_system_msu_targets}" == "TRUE" ]] && [[ "${install_non_system_updates_without_restarting}" == "TRUE" ]]; } || { [[ "${non_system_msu_safari_target}" != "FALSE" ]] && [[ "${install_safari_update_without_restarting}" == "TRUE" ]]; }; }; then
		[[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] && log_super "Warning: Non-system update workflow is being prioritized over ${macos_installer_title} ${macos_installer_version}-${macos_build} major upgrade via installer."
		[[ "${macos_installer_minor_update_target}" != "FALSE" ]] && log_super "Warning: Non-system update workflow is being prioritized over ${macos_installer_title} ${macos_installer_version}-${macos_build} minor update via installer."
		[[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] && log_super "Warning: Non-system update workflow is being prioritized over ${macos_msu_title} ${macos_msu_version}-${macos_msu_build} major upgrade via softwareupdate."
		...
		macos_msu_major_upgrade_target="FALSE"
		macos_msu_minor_update_target="FALSE"
		if [[ "${install_non_system_updates_without_restarting}" == "TRUE" ]]; then
			workflow_target="Non-system Software Updates"
			workflow_target_log_string="NON-SYSTEM SOFTWARE UPDATES PRIORITY"
			[[ -n "${display_accessory_non_system_updates}" ]] && display_accessory_content="${display_accessory_non_system_updates}"
			[[ "${workflow_only_download_active}" == "TRUE" ]] && log_super "Warning: The --install-non-system-updates-without-restarting option is currently overriding the --only-download option."
		fi
		if [[ "${install_safari_update_without_restarting}" == "TRUE" ]] && [[ "${non_system_msu_safari_target}" != "FALSE" ]]; then
			workflow_target="Safari Update"
			workflow_target_log_string="SAFARI UPDATE PRIORITY"
			[[ -n "${display_accessory_safari_update}" ]] && display_accessory_content="${display_accessory_safari_update}"
			[[ "${workflow_only_download_active}" == "TRUE" ]] && log_super "Warning: The --install-safari-update-without-restarting option is currently overriding the --only-download option."
		fi
		workflow_macos_auth="FALSE"
		[[ "${workflow_only_download_active}" == "TRUE" ]] && workflow_only_download_active="FALSE"
	elif [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_installer_minor_update_target}" != "FALSE" ]]; then
		workflow_target="${macos_installer_title} ${macos_installer_version}-${macos_build}"
		if [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]]; then
			workflow_target_log_string="${workflow_target} MAJOR UPGRADE VIA INSTALLER"
			[[ -n "${display_accessory_macos_major_upgrade}" ]] && display_accessory_content="${display_accessory_macos_major_upgrade}"
		else # ${macos_installer_minor_update_target}
			workflow_target_log_string="${workflow_target} MINOR UPDATE VIA INSTALLER"
			[[ -n "${display_accessory_macos_minor_update}" ]] && display_accessory_content="${display_accessory_macos_minor_update}"
		fi
		[[ "${non_system_msu_targets}" == "TRUE" ]] && log_super "Status: The workflow will automatically re-check for non-system updates immediately after the system restarts from the pending macOS major upgrade."
		...
		if [[ $(echo "${macos_msu_minor_update_target}" | grep -c '(') -gt 0 ]] && [[ "${install_rapid_security_responses}" != "TRUE" ]]; then
		...
		[[ "${non_system_msu_targets}" == "TRUE" ]] && log_super "Status: The workflow will automatically re-check for non-system updates immediately after the system restarts from the pending macOS minor update."
	elif [[ "${non_system_msu_targets}" == "TRUE" ]]; then
		if [[ "${install_non_system_updates_without_restarting}" != "TRUE" ]] || [[ "${install_safari_update_without_restarting}" != "TRUE" ]]; then
			log_super "Warning: The default workflow ignores non-system software updates if there is no macOS update/upgrade available. Use the --install-non-system-updates-without-restarting or the --install-safari-update-without-restarting option to install non-system items even if there is no macOS update/upgrade availalbe."
		else
			if [[ "${install_non_system_updates_without_restarting}" == "TRUE" ]]; then
				workflow_target="Non-system Software Updates"
				workflow_target_log_string="NON-SYSTEM SOFTWARE UPDATES PRIORITY"
		...
		[[ -n "${display_accessory_jamf_policy_triggers}" ]] && display_accessory_content="${display_accessory_jamf_policy_triggers}"
		workflow_macos_auth="FALSE"
		...
		if [[ -n "${install_jamf_policy_triggers}" ]]; then 
			...
			[[ -n "${display_accessory_restart_without_updates}" ]] && display_accessory_content="${display_accessory_restart_without_updates}"
		fi
		workflow_macos_auth="FALSE"
		if [[ "${workflow_only_download_active}" == "TRUE" ]]; then
	...
	{ [[ -z "${display_accessory_content}" ]] && [[ -n "${display_accessory_default}" ]]; } && display_accessory_content="${display_accessory_default}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_target is: ${workflow_target}"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_accessory_content is: ${display_accessory_content}"
	
	# Log any changes to the workflow target.
	workflow_target_previous=$(get_pref "WorkflowTarget")
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_target_previous is: ${workflow_target_previous}"
	{ [[ -z "${workflow_target_previous}" ]] && [[ "${workflow_target}" != "FALSE" ]]; } && log_super_audit "Status: Setting new workflow target to ${workflow_target}".
	{ [[ -n "${workflow_target_previous}" ]] && [[ "${workflow_target}" != "${workflow_target_previous}" ]]; } && log_super_audit "Warning: Previous workflow target of ${workflow_target_previous} has been changed to ${workflow_target}."
	
	# If the workflow target is only Safari, check to see if Safari is active or in test mode. If not, force an Install Now workflow.
	if [[ "${workflow_target}" == "Safari Update" ]]; then
		if [[ "${current_user_account_name}" != "FALSE" ]] && { [[ $(pgrep '^Safari$') ]] || [[ "${test_mode}" == "TRUE" ]]; }; then
			workflow_target_safari_active="TRUE"
		else
			log_super "Status: Safari is not currently in use and can be installed immediately, forcing the Install Now workflow."
			workflow_target_safari_active="FALSE"
			workflow_install_now_active="TRUE"
			workflow_install_now_suppress_notifications="TRUE"
		fi
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: workflow_target_safari_active is: ${workflow_target_safari_active}"
	...
	# Check for zero date, scheduled installation, and log workflow.
	if [[ "${workflow_target}" != "FALSE" ]]; then
		...
		set_pref_main "WorkflowTarget" "${workflow_target}"
	else # No software updates/upgrade needed so reset any leftovers.
		{ [[ "${workflow_target_previous}" != "FALSE" ]] && [[ "${verbose_mode}" == "TRUE" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Calling reset_workflow() function."
		[[ "${workflow_target_previous}" != "FALSE" ]] && reset_workflow
	fi
	
	# If needed, handle ${schedule_deferred_start_days_active} before the any installation or restart workflow starts.
	if [[ "${workflow_target}" != "FALSE" ]] && [[ "${schedule_deferred_start_days_active}" == "TRUE" ]]; then
		if [[ "${workflow_install_now_active}" == "TRUE" ]]; then
			log_super "Warning: The --workflow-install-now option is currently overriding the schedule deferred start."
		elif [[ "${workflow_scheduled_install_active}" == "TRUE" ]]; then
			[[ "${workflow_scheduled_install_now}" == "TRUE" ]] && log_super "Warning: A scheduled installation is currently overriding the schedule deferred start."
			[[ "${workflow_scheduled_install_now}" != "TRUE" ]] && log_super "Warning: A scheduled installation reminder is currently overriding the schedule deferred start."
		else # Schedule deferred start should force a deferral.
			deferral_timer_minutes=$(((schedule_zero_date_deferred_start_epoch - workflow_time_epoch) / 60 ))
			log_super "Status: Automatic deferred start until ${schedule_zero_date_deferred_start_date}, deferring for ${deferral_timer_minutes} minutes."
			log_status "Pending: Automatic deferred start until ${schedule_zero_date_deferred_start_date}, deferring for ${deferral_timer_minutes} minutes."
			set_auto_launch_deferral
		fi
	fi
	
	# If set, handle the ${schedule_workflow_active} before the any installation or restart workflow starts.
	if [[ "${workflow_target}" != "FALSE" ]] && [[ -n "${schedule_workflow_active}" ]]; then
		if [[ "${schedule_workflow_active_deferral}" == "TRUE" ]]; then
			if [[ "${workflow_install_now_active}" == "TRUE" ]]; then
	...
	# Double-check for local user, and handle the ${workflow_require_active_user}.
	[[ "${current_user_account_name}" == "FALSE" ]] && check_current_user
	if [[ "${current_user_account_name}" == "FALSE" ]] && [[ "${workflow_require_active_user}" == "TRUE" ]]; then
		deferral_timer_minutes=$deferral_timer_error_minutes
		log_super "Error: No currently logged in user and workflow require active user is enabled, trying again in ${deferral_timer_minutes} minutes."
		log_status "Pending: No currently logged in user and workflow require active user is enabled, trying again in ${deferral_timer_minutes} minutes."
		set_auto_launch_deferral
	fi
	...
	if [[ "${workflow_target}" != "FALSE" ]]; then
		...
		if [[ "${workflow_install_now_active}" == "TRUE" ]]; then
			if [[ "${current_user_account_name}" == "FALSE" ]]; then # A normal user is not logged in, start installations immediately.
				...
				[[ $(echo "${auth_mdm_failover_to_user}" | grep -c 'INSTALLNOW') -gt 0 ]] && auth_mdm_failover_to_user_status="TRUE"
				...
				{ [[ "${workflow_target}" != "Non-system Software Updates" ]] && [[ "${workflow_target}" != "Safari Update" ]] && [[ "${workflow_target}" != "Jamf Pro Policy Triggers Without Restarting" ]] && [[ "${workflow_target}" != "Jamf Pro Policy Triggers With Restart" ]] && [[ "${workflow_target}" != "Restart Without Updates" ]]; } && workflow_download_macos # This function only downloads if needed and includes internal storage checks and test mode logic.
				workflow_install_active_user # This function includes internal power, storage, and Apple silicon authentication checks. Sub-functions include test mode logic.
			fi
		elif [[ "${workflow_scheduled_install_active}" == "TRUE" ]]; then
			if [[ "${workflow_scheduled_install_now}" == "TRUE" ]]; then
			...
			if [[ "${current_user_account_name}" == "FALSE" ]] && [[ "${macos_msu_automatic_download_deferral}" == "FALSE" ]]; then # A normal user is not logged in and the system can not automatically download the update, the only download workflow can not continue.
				deferral_timer_minutes=$deferral_timer_error_minutes
				log_super "Error: No current active user and automatic macOS update downloads are disabled, due to limitations in macOS the only download workflow can not continue, trying again in ${deferral_timer_minutes} minutes."
				log_status "Pending: No current active user and automatic macOS update downloads are disabled, due to limitations in macOS the only download workflow can not continue, trying again in ${deferral_timer_minutes} minutes."
				set_auto_launch_deferral
			else # A normal user is currently logged in or automatic macOS update downloads is possible.
			...
			# Double-check for local user, and if recently started up then wait until later.
			[[ "${current_user_account_name}" == "FALSE" ]] && check_current_user
			if [[ "${current_user_account_name}" == "FALSE" ]] && [[ $((workflow_time_epoch - kernel_boot_epoch)) -lt $RECENT_STARTUP_AUTO_DEFERRAL_SECONDS ]]; then
				deferral_timer_minutes=$deferral_timer_error_minutes
				log_super "Error: The system started up less than $RECENT_STARTUP_AUTO_DEFERRAL_SECONDS seconds ago so it's unsafe to assume current active user status, trying again in ${deferral_timer_minutes} minutes."
				log_status "Pending: The system started up less than $RECENT_STARTUP_AUTO_DEFERRAL_SECONDS seconds ago so it's unsafe to assume current active user status, trying again in ${deferral_timer_minutes} minutes."
				set_auto_launch_deferral
			fi
			# At this point pick a workflow based on if the user is active.
			if [[ "${current_user_account_name}" == "FALSE" ]]; then # A normal user is not logged in, start installations immediately.
				workflow_install_no_user # This function includes internal power, storage, and Apple silicon authentication checks. Sub-functions include test mode logic.
			else # A normal user is currently logged in so check for deadlines and if necessary download macOS update/upgrade.
				check_deadlines_days_date
				{ [[ "${deadline_date_status}" == "FALSE" ]] && [[ "${deadline_days_status}" == "FALSE" ]]; } && check_user_focus
				{ [[ "${deadline_date_status}" != "FALSE" ]] || [[ "${deadline_days_status}" != "FALSE" ]]; } && user_focus_active="FALSE"
				check_deadlines_count
				{ [[ "${workflow_target}" != "Non-system Software Updates" ]] && [[ "${workflow_target}" != "Safari Update" ]] && [[ "${workflow_target}" != "Jamf Pro Policy Triggers Without Restarting" ]] && [[ "${workflow_target}" != "Jamf Pro Policy Triggers With Restart" ]] && [[ "${workflow_target}" != "Restart Without Updates" ]]; } && workflow_download_macos # This function only downloads if needed and includes internal storage checks and test mode logic.
				if [[ "${workflow_download_macos_check_user}" == "TRUE" ]]; then # The download may have taken a while so another check to make sure the user is still logged in.
					check_current_user
					workflow_time_epoch=$(date +%s)
					[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Setting new workflow_time_epoch to: ${workflow_time_epoch}"
					if [[ "${current_user_account_name}" == "FALSE" ]]; then
						deferral_timer_minutes=$deferral_timer_error_minutes
						log_super "Error: The active user logged out while the update was downloading, trying again in ${deferral_timer_minutes} minutes."
						log_status "Pending: The active user logged out while the update was downloading, trying again in ${deferral_timer_minutes} minutes."
						set_auto_launch_deferral
					else
						check_deadlines_days_date
						{ [[ "${deadline_date_status}" == "FALSE" ]] && [[ "${deadline_days_status}" == "FALSE" ]]; } && check_user_focus
				...
				if [[ "${deadline_date_status}" == "HARD" ]] || [[ "${deadline_days_status}" == "HARD" ]] || [[ "${deadline_count_status}" == "HARD" ]] || [[ "${deadline_date_status}" == "SOFT" ]] || [[ "${deadline_days_status}" == "SOFT" ]] || [[ "${deadline_count_status}" == "SOFT" ]]; then
					[[ $(echo "${display_unmovable}" | grep -c 'DEADLINE') -gt 0 ]] && display_unmovable_status="TRUE"
					[[ $(echo "${display_hide_background}" | grep -c 'DEADLINE') -gt 0 ]] && display_hide_background_status="TRUE"
					[[ $(echo "${display_silently}" | grep -c 'DEADLINE') -gt 0 ]] && display_silently_status="TRUE"
					[[ $(echo "${display_notifications_centered}" | grep -c 'DEADLINE') -gt 0 ]] && display_notifications_centered_status="TRUE"
					[[ $(echo "${display_notifications_centered}" | grep -c 'DEADLINE') -gt 0 ]] && display_notifications_centered_status="TRUE"
					[[ $(echo "${auth_mdm_failover_to_user}" | grep -c 'DEADLINE') -gt 0 ]] && auth_mdm_failover_to_user_status="TRUE"
				fi
	...
	if [[ $(/usr/libexec/PlistBuddy -c "Print :WorkflowRestartValidate" "${SUPER_MAIN_PLIST}.plist" 2> /dev/null) == "true" ]]; then # An update/upgrade is about to restart the computer.
		...
		{ [[ "${workflow_target}" != "FALSE" ]] && [[ "${verbose_mode}" == "TRUE" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: Calling reset_workflow() function."
		[[ "${workflow_target}" != "FALSE" ]] && reset_workflow
		# Wrap-up for the ${workflow_install_now_active} if there were no updates/upgrades.
		if [[ "${workflow_install_now_active}" == "TRUE" ]]; then
			{ [[ "${current_user_account_name}" != "FALSE" ]] && [[ "${workflow_install_now_suppress_notifications}" != "TRUE" ]]; } && notification_install_now_up_to_date
			if [[ "${test_mode}" == "TRUE" ]]; then
		...
		# Logic for ${workflow_disable_relaunch} and ${deferral_timer_workflow_relaunch_minutes}.
		if [[ "${workflow_disable_relaunch}" == "TRUE" ]]; then
			log_super_audit "Status: Full super workflow complete! Automatic relaunch is disabled."
			...
			log_super_audit "Status: Full super workflow complete! The super workflow is scheduled to automatically relaunch in ${deferral_timer_minutes} minutes."
